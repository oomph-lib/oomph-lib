...
public:
  ...
  {
    Vector<typename TIMESTEPPER::NodeInitialConditionFctPt> initial_value_fct(
      1);
    Vector<typename TIMESTEPPER::NodeInitialConditionFctPt> initial_veloc_fct(
      1);
    Vector<typename TIMESTEPPER::NodeInitialConditionFctPt> initial_accel_fct(
      1);
    ...
    for (unsigned ibound = 0; ibound < num_bound; ibound++)
    {
      ...
      for (unsigned inod = 0; inod < num_nod; inod++)
      {
        ...
        if (use_direct_assignment)
        {
          ...
          nod_pt->set_value(
            0, TanhSolnForLinearWave::exact_u(time_pt()->time(), x));
        }
        else
        {
          // Get timestepper
...
void LinearWaveProblem<ELEMENT, TIMESTEPPER>::set_initial_condition()
{
  ...
  else
  {
    ...
    for (unsigned jnod = 0; jnod < num_nod; jnod++)
    {
      ...
      double u_exact = TanhSolnForLinearWave::exact_u(time_pt()->time(), x);
      double dudt_exact =
...
void LinearWaveProblem<ELEMENT, TIMESTEPPER>::doc_solution(DocInfo& doc_info)
{
  ...
  some_file.open(filename);
  mesh_pt()->output_fct(
    some_file, npts, time_pt()->time(), TanhSolnForLinearWave::get_exact_u);
  some_file.close();
...
int main(int argc, char* argv[])
{
  ...
  // ------------------------
  {
    ...
    // Build problem
    LinearWaveProblem<QLinearWaveElement<2, 3>, Newmark<1>> problem(
      n_x, n_y, impulsive_start, &TanhSolnForLinearWave::get_source);
