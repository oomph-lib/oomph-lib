...
// Exercise: New namespace for tanh source
namespace TanhSolnOneDPoisson
{
	...
	void get_exact_u(const Vector<double>& x, Vector<double>& u)
	{
		u[0]=double(Sign)*(tanh(10*(x[0]-0.5)));
	}
	
	// Source function (ie the second derivative)
	void source_function(const Vector<double>& x, double& source)
	{
		source=double(Sign)*(-2*100*tanh(10*(x[0]-0.5))*1/((cosh(10*(x[0]-0.5)))*(cosh(10*(x[0]-0.5)))));
	}
...
int main()
{
 ...
 OneDPoissonProblem<QPoissonElement<1,2> > //Element type as template parameter
  problem(n_element,TanhSolnOneDPoisson::source_function);
