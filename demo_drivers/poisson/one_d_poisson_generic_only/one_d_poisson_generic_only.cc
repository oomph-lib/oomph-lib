...
public:
  ...
  void get_residuals(Vector<double> &residuals)
  {
    ...
    for (unsigned ipt = 0; ipt < n_intpt; ipt++)
    {
      ...
      double interpolated_x = 0.0, interpolated_u = 0.0,
...
public:
  ...
  OneDimMesh(const unsigned &n_element)
  {
    ...
    Node_pt.push_back(
      finite_element_pt(n_element - 1)->construct_boundary_node(n_node - 1));
...
public:
  ...
  void actions_before_newton_solve()
  {
    ...
    {
      throw OomphLibError(
        "Trouble! Check error messages and fix the problems.\n",
