  ...
  DemoPoissonProblem(const int& sign=1) : Sign(sign)
   {
    ...
    Problem::mesh_pt() = new OneDimMesh<TwoNodePoissonElement>(50);
	...
	// Exercise addition: Set u(0)=1
	// mesh_pt()->boundary_node_pt(0,0)->set_value(0,1);
	// Exercise addition finished
  ...
  void actions_after_newton_solve() 
   {
    ...
    mesh_pt()->output(file);
	// Exercise addition, output the analytical results
	ofstream file2 ("analytical.dat");
	int numelan=31;
	double xarr[numelan-1];
	double analyt_res[numelan];
	for (int ii=0; ii<=30; ii++)
	{
		xarr[ii]=ii*1/double(numelan);
		analyt_res[ii]=double(Sign)*((sin(sqrt(30))-1)*xarr[ii]-sin(sqrt(30)*xarr[ii]));
		file2 << xarr[ii] << " ";
		file2 << analyt_res[ii] << "\n";
	}
	// Exercise addition finished
