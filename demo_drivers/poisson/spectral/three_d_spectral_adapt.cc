...
// LIC//
// LIC//====================================================================
// Driver for a 3D poisson problem using spectral elements
// Note that the current implementation is not efficient, but has
// ben used to implement non-uniform refinement for elements with non-uniformly
// spaced nodes.
  ...
  void get_exact_u(const Vector<double>& x, Vector<double>& u)
  {
    u[0] = tanh(Alpha *
                ((x[0] - X_0) * N_x / sqrt(N_x * N_x + N_y * N_y + N_z * N_z) +
                 (x[1] - Y_0) * N_y / sqrt(N_x * N_x + N_y * N_y + N_z * N_z) +
                 (x[2] - Z_0) * N_z / sqrt(N_x * N_x + N_y * N_y + N_z * N_z)));
  }
      ...
      (1.0 -
       pow(tanh(Alpha *
                ((x[0] - X_0) * N_x / sqrt(N_x * N_x + N_y * N_y + N_z * N_z) +
                 ...
                 (x[2] - Z_0) * N_z / sqrt(N_x * N_x + N_y * N_y + N_z * N_z))),
           2.0)) *
      Alpha * Alpha * N_x * N_x / (N_x * N_x + N_y * N_y + N_z * N_z);
    s3 =
      -2.0 *
      tanh(Alpha *
           ((x[0] - X_0) * N_x / sqrt(N_x * N_x + N_y * N_y + N_z * N_z) +
      ...
      (1.0 -
       pow(tanh(Alpha *
                ((x[0] - X_0) * N_x / sqrt(N_x * N_x + N_y * N_y + N_z * N_z) +
                 ...
                 (x[2] - Z_0) * N_z / sqrt(N_x * N_x + N_y * N_y + N_z * N_z))),
           2.0)) *
      Alpha * Alpha * N_y * N_y / (N_x * N_x + N_y * N_y + N_z * N_z);
    s4 =
      -2.0 *
      tanh(Alpha *
           ((x[0] - X_0) * N_x / sqrt(N_x * N_x + N_y * N_y + N_z * N_z) +
      ...
      (1.0 -
       pow(tanh(Alpha *
                ((x[0] - X_0) * N_x / sqrt(N_x * N_x + N_y * N_y + N_z * N_z) +
                 ...
                 (x[2] - Z_0) * N_z / sqrt(N_x * N_x + N_y * N_y + N_z * N_z))),
           2.0)) *
      Alpha * Alpha * N_z * N_z / (N_x * N_x + N_y * N_y + N_z * N_z);
    s2 = s3 + s4;
    source = s1 + s2;
  }
  ...
  Source_fct_pt(source_fct_pt)
{
  ...
  if (CommandLineArgs::Argc > 1)
  {
    ...
    // first step
    mesh_pt()->max_permitted_error() = 0.4;
    mesh_pt()->min_permitted_error() = 0.2;
  }
  else
  {
...
int main(int argc, char* argv[])
{
  ...
  // source function
  EighthSpherePoissonProblem<RefineableQSpectralPoissonElement<3, 4>> problem(
