...
public: 
 ...
 SimpleRefineableRectangularQuadMesh(const unsigned &Nx,
  ...
  {
   //To Check rotations, we should rotate a central element
   FiniteElement* el_pt = this->finite_element_pt(Nx + 1);
   ...
   {
    elemental_nodes_pt[n] = el_pt->node_pt(n);
   }
   
   //We now merely permute the nodes
 //   unsigned n_p = el_pt->nnode_1d();
//    for(unsigned j=0;j<n_p;j++)
//    {
//     for(unsigned i=0;i<n_p;i++)
//      {
//       el_pt->node_pt(j*n_p + i)
...
public:
 ...
 SimpleRefineableRectangularQuadMesh<ELEMENT>* mesh_pt() 
  {
   return dynamic_cast<SimpleRefineableRectangularQuadMesh<ELEMENT>*>(Problem::mesh_pt());
  }
...
int main()
{
 ...
 RefineablePoissonProblem<RefineableQSpectralPoissonElement<2,5> > 
 ...
 problem.mesh_pt()->output("initial.dat");
