  ...
  void output(ostream& outfile, const unsigned& nplot)
  {
    ...
    for (unsigned iplot = 0; iplot < num_plot_points; iplot++)
    {
      ...
      dshape_dbasis_and_dtest_eulerian(
        s, psi, dpsidx, basis, dbasisdx, test, dtestdx);
...
protected:
                                                 ...
                                                 unsigned flag)
  {
    ...
    for (unsigned ipt = 0; ipt < n_intpt; ipt++)
    {
      ...
      // Call the derivatives of the shape and test functions
      double J = dshape_dbasis_and_dtest_eulerian_at_knot(
        ipt, psi, dpsidx, basis, dbasisdx, test, dtestdx);
      ...
      for (unsigned l = 0; l < n_basis; l++)
      {
        ...
        if (local_eqn >= 0)
        {
          ...
          {
            residuals[local_eqn] +=
              diff * interpolated_dudx[k] * dtestdx(l, k) * W;
          }
  ...
  RankThreeTensor<double>& dresidual_dnodal_coordinates)
{
  ...
  Shape psi(n_node), basis(n_basis), test(n_basis);
  DShape dpsidx(n_node, DIM), dbasisdx(n_basis, DIM), dtestdx(n_basis, DIM);
  DShape dpsidx_pls(n_node, DIM), dbasisdx_pls(n_basis, DIM),
    dtestdx_pls(n_basis, DIM);
  ...
  for (unsigned ipt = 0; ipt < n_intpt; ipt++)
  {
    ...
    // Call the derivatives of the shape and test functions
    double J = this->dshape_dbasis_and_dtest_eulerian_at_knot(
      ipt, psi, dpsidx, basis, dbasisdx, test, dtestdx);
         ...
         it++)
    {
      ...
      for (unsigned ii = 0; ii < DIM; ii++)
      {
        ...
        // Call the derivatives of the shape and test functions
        // at advanced level
        double J_pls = this->dshape_dbasis_and_dtest_eulerian_at_knot(
          ipt, psi, dpsidx_pls, basis, dbasisdx_pls, test, dtestdx_pls);
        ...
        for (unsigned i = 0; i < DIM; i++)
        {
          ...
          {
            d_dpsidx_dX(ii, jj, j, i) =
              (dpsidx_pls(j, i) - dpsidx(j, i)) / eps_fd;
            d_dtestdx_dX(ii, jj, j, i) =
              (dtestdx_pls(j, i) - dtestdx(j, i)) / eps_fd;
          }
        }
        ...
        for (unsigned i = 0; i < DIM; i++)
        {
          ...
          {
            aux +=
              nodal_value(j_nod, u_nodal_index) * d_dpsidx_dX(ii, jj, j_nod, i);
          }
                                                ...
                                                const unsigned& initial_p_order)
{
  ...
  if (father_pt != 0)
  {
          ...
          dynamic_cast<PRefineableQElement<1, 2>*>(father_pt->object_pt()))
    {
      ...
      if (this->internal_data_pt(0)->nvalue() <=
          this->p_order() - this->nnode())
      {
      ...
      if (this->ninternal_data() == 0)
        delete this->internal_data_pt(0);
...
public:
  ...
  {
    // Set integration scheme
    // (To avoid memory leaks in pre-build and p-refine where new integration
    // schemes are created)
    ...
    double& norm)
  {
    ...
    for (unsigned ipt = 0; ipt < n_intpt; ipt++)
    {
      ...
      outfile << exact_grad[0] << " " << exact_grad[0] - dudx_fe[0]
              << std::endl;
                                                  ...
                                                  DShape& dtestdx) const
  {
    ...
    // Loop over the nodes and set the first nodal shapes equal to the basis
    // functions
...
//========================================================================
template<class ELEMENT>
PRefineableOneDPoissonProblem<ELEMENT>::PRefineableOneDPoissonProblem(
  PoissonEquations<1>::PoissonSourceFctPt source_fct_pt) :
  Source_fct_pt(source_fct_pt)
{
  ...
  Problem::mesh_pt() = new RefineableOneDMesh<ELEMENT>(n, length);
...
void PRefineableOneDPoissonProblem<ELEMENT>::doc_solution(DocInfo& doc_info)
{
  ...
  mesh_pt()->compute_error(
    some_file, ArcTanSolnForPoisson::get_exact_u, error, norm);
  some_file.close();
...
int main()
{
  ...
  PRefineableOneDPoissonProblem<PRefineableQPoissonElement<1>> nodal_problem(
    &ArcTanSolnForPoisson::get_source);
