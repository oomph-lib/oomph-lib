...
class ElasticRefineableTwoDAnnularMesh
  : public virtual RefineableTwoDAnnularMesh<ELEMENT>,
...
public:
  ...
  virtual ~ElasticRefineableTwoDAnnularMesh<ELEMENT>()
  {
...
private:
  ...
  /// Bulk mesh
  ElasticRefineableTwoDAnnularMesh<ELEMENT>* Bulk_mesh_pt;
...
void ShellProblem<ELEMENT>::complete_problem_setup()
{
  ...
  for (unsigned e = 0; e < n_element_bulk; e++)
  {
    ...
    // Cast to element with pressure dofs
    RefineableQAxisymmetricCylindricalPVDWithPressureElement* el_with_pres_pt =
      dynamic_cast<RefineableQAxisymmetricCylindricalPVDWithPressureElement*>(
        el_pt);

    // Cast will only succeed if we're using the pressure formulation
    // Can set compressible/incompressible and pin any redundant solid pressures
    // here
    if (el_with_pres_pt != 0)
    {
      ...
      el_with_pres_pt->pin_elemental_redundant_nodal_solid_pressures();
    }
