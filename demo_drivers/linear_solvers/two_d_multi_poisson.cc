...
MultiPoissonProblem<ELEMENT>::MultiPoissonProblem()
{
  ...
  else if (CommandLineArgs::command_line_flag_has_been_set("--two_plus_three"))
  {
...
void MultiPoissonProblem<ELEMENT>::doc_solution(DocInfo& doc_info)
{
  ...
  mesh_pt()->compute_error(
    some_file, TanhSolnForMultiPoisson::get_exact_u, error, norm);
  some_file.close();
...
int main(int argc, char* argv[])
{
  ...
  CommandLineArgs::specify_command_line_flag("--upper_triangular");

  CommandLineArgs::specify_command_line_flag("--two_plus_three");
  ...
  CommandLineArgs::specify_command_line_flag("--coarse_two_plus_two_plus_one");

  CommandLineArgs::specify_command_line_flag("--one_plus_four_with_two_coarse");
  ...
  oomph_info << "\n\nSolving for TanhSolnForMultiPoisson::Alpha="
             << TanhSolnForMultiPoisson::Alpha << std::endl
             << std::endl;
