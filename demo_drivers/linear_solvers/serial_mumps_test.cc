...
void OneDPoissonProblem<ELEMENT>::doc_solution(DocInfo& doc)
{
  ...
  sprintf(filename, "%s/soln_serial_mumps.dat", doc.directory().c_str());
  ...
  sprintf(filename, "%s/exact_soln.dat", doc.directory().c_str());
  some_file.open(filename);
  ...
  double error, norm;
  sprintf(filename, "%s/error.dat", doc.directory().c_str());
  some_file.open(filename);
