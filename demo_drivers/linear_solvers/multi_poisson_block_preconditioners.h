...
namespace oomph
{
  ...
  /// on a compound block with more than two DOF types then they must be
  /// coarsened via a parameter to the master preconditioner's member function
  ...
  /// on a compound block with more than one DOF type then they must be
  /// coarsened via a parameter to the master preconditioner's member function
  /// turn_into_subsidiary_block_preconditioner(...).
  //=============================================================================
  template<typename MATRIX>
  ...
  void CoarseTwoIntoOne<MATRIX>::setup()
  {
    ...
    std::cout << "CoarseTwoIntoOne nblock_types: " << n_block_types
              << std::endl;
    ...
    // types. Since we have two DOF types, we must coarsen DOF types.
    {
      ...
      block_prec_pt->turn_into_subsidiary_block_preconditioner(
        this, dof_map, doftype_coarsening);
                                                     ...
                                                     DoubleVector& z)
  {
    ...
    for (int i = n_block - 1; i > -1; i--)
    {
      ...
      // Solve on the block
      this->Block_preconditioner_pt[i]->preconditioner_solve(block_r[i],
                                                             block_z[i]);
    } // End for over i
  ...
  void UpperTriangular<MATRIX>::clean_up_my_memory()
  {
    ...
    for (unsigned i = 0, ni = Off_diagonal_matrix_vector_product_pt.nrow();
         i < ni;
         i++)
    {
      for (unsigned j = 0, nj = Off_diagonal_matrix_vector_product_pt.ncol();
           j < nj;
           j++)
      {
                                                  ...
                                                  DoubleVector& z)
  {
    ...
    // Split up rhs vector into sub-vectors, arranged to match the matrix
    // blocks.
    Vector<DoubleVector> block_r;
    this->get_block_vectors(r, block_r);
  ...
  //=============================================================================
  template<typename MATRIX>
  class TwoPlusThreeUpperTriangular : public BlockPreconditioner<MATRIX>
  {
  public:
    /// Constructor.
  ...
  void TwoPlusThreeUpperTriangular<MATRIX>::setup()
  {
    ...
    // next setup the off diagonal mat vec operators
    {
      ...
      this->setup_matrix_vector_product(Off_diagonal_matrix_vector_product_pt,
                                        &block_matrix,
                                        block_column_index);
    }
  }
    ...
    const DoubleVector& r, DoubleVector& z)
  {
    ...
    // Split up rhs vector into sub-vectors, rarranged to match the matrix
    // blocks.
    Vector<DoubleVector> block_r;
    this->get_block_vectors(r, block_r);
  ...
  /// \short Upper block triangular with two levels of subsidiary
  /// preconditioners for a system with 5 dof types.
  //=============================================================================
  template<typename MATRIX>
    ...
    for (unsigned i = 0; i < n_dof_types; i++)
    {
      for (unsigned j = 0; j < n_dof_types; j++)
      {
        if (i != j)
        {
          ...
          } // done -- quite wasteful, we're actually storing lots of zeroes,
          ...
          else
          {
            ...
            // Need one row start per row, and one for the nnz, all of which are
            // 0. There are no rows, so this is a Vector of size 1.
  ...
  //============================================================================
  template<typename MATRIX>
  void TwoPlusThreeUpperTriangularWithReplace<MATRIX>::clean_up_my_memory()
  {
    // Clean up subsidiary preconditioners.
    if (First_subsidiary_preconditioner_pt != 0)
    {
  ...
  //=============================================================================
  template<typename MATRIX>
  class CoarseTwoPlusTwoPlusOne : public BlockPreconditioner<MATRIX>
  {
  public:
  ...
  void CoarseTwoPlusTwoPlusOne<MATRIX>::setup()
  {
    ...
    // Second subsidiary preconditioner is also a block preconditioner
    {
      ...
      // combined/coarsened into dof type
      // dof_type_in_coarsed_block_preconditioner
  ...
  //===========================================================================
  template<typename MATRIX>
  void CoarseTwoPlusTwoPlusOne<MATRIX>::clean_up_my_memory()
  {
    // Delete diagonal preconditioners (approximate solvers)
    if (First_subsidiary_preconditioner_pt != 0)
    {
  ...
  //=============================================================================
  template<typename MATRIX>
  class OnePlusFourWithTwoCoarse : public BlockPreconditioner<MATRIX>
  {
  public:
  ...
  //===========================================================================
  template<typename MATRIX>
  void OnePlusFourWithTwoCoarse<MATRIX>::clean_up_my_memory()
  {
    // Delete diagonal preconditioners (approximate solvers)
    if (First_subsidiary_preconditioner_pt != 0)
    {
