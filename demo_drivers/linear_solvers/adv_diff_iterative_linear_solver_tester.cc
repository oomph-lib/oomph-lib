...
namespace TanhSolnForAdvectionDiffusion
{
  ...
  void source_function(const Vector<double>& x_vect, double& source)
  {
    ...
    double y = x_vect[1];
    source =
      2.0 * tanh(-0.1E1 + Alpha * (TanPhi * x - y)) *
...
//========================================================================
template<class ELEMENT>
void TwoMeshFluxAdvectionDiffusionProblem<
  ELEMENT>::actions_before_newton_solve()
{
  ...
  DocInfo& doc_info)
{
  ...
  cout << "Ultimate min/max. refinement levels in bulk mesh : "
       << min_refinement_level << " " << max_refinement_level << std::endl;
  ...
  Bulk_mesh_pt->output_fct(
    some_file, npts, TanhSolnForAdvectionDiffusion::get_exact_u);
  some_file.close();
  ...
  Bulk_mesh_pt->compute_error(
    some_file, TanhSolnForAdvectionDiffusion::get_exact_u, error, norm);
  some_file.close();
...
void TwoMeshFluxAdvectionDiffusionProblem<ELEMENT>::actions_after_adapt()
{
  ...
  Bulk_mesh_pt->get_refinement_levels(min_refinement_level,
                                      max_refinement_level);
  cout << "Min/max. refinement levels in bulk mesh: " << min_refinement_level
...
void run_it(LinearSolver* linear_solver_pt)
{
  ...
  TwoMeshFluxAdvectionDiffusionProblem<
    ...
    problem(&TanhSolnForAdvectionDiffusion::source_function,
  ...
  if (it_solver_pt != 0)
  {
    it_solver_pt->open_convergence_history_file_stream(
      "RESLT/resolve_convergence.dat");
  }
  ...
  error_file << "Error for resolve: " << sqrt(error / double(n_dof))
             << std::endl;
  std::cout << "Error for resolve: " << sqrt(error / double(n_dof))
            << std::endl;
  error_file.close();
  ...
  {
    it_solver_pt->open_convergence_history_file_stream(
      "RESLT/la_solve_convergence.dat");
  }
  ...
  error_file2 << "Error for LA solve: " << sqrt(error / double(n_dof))
              << std::endl;
  std::cout << "Error for LA solve: " << sqrt(error / double(n_dof))
            << std::endl;
  error_file2.close();
  ...
  while (doit)
  {
    ...
    switch (solver_flag)
    {
      ...
      case 0:
        ...
        std::cout << "Solving with SuperLU direct solver" << std::endl;
        break;
      ...
      case 1:
        ...
        it_linear_solver_pt->open_convergence_history_file_stream(
