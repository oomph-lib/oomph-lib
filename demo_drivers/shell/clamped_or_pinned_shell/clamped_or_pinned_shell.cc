...
namespace Global_Physical_Variables
{
                  ...
                  Vector<double> &load)
  {
    ...
    {
      load[i] =
        (external_pressure() - Pcos * pow(0.05, 3) / 12.0 * cos(2.0 * xi[1])) *
        N[i];
    }
  }
                                    ...
                                    const unsigned &problem_id)
{
  ...
  // Now create the mesh
  Problem::mesh_pt() =
    new CircularCylindricalShellMesh<ELEMENT>(nx, ny, lx, ly);
  ...
  std::cout << "Displacement control applied at xi = (" << xi[0] << ", "
            << xi[1] << ")" << std::endl;
  std::cout << "Corresponding to                x  = (" << x[0] << ", " << x[1]
            << ", " << x[2] << ")" << std::endl;
...
void ShellProblem<ELEMENT>::create_bc_elements(const unsigned &b)
{
  ...
  std::cout << "Creating " << n_element << " boundary condition elements"
            << std::endl;
...
void ShellProblem<ELEMENT>::solve(string &dir_name)
{
  ...
  for (unsigned i = 1; i < 11; i++)
  {
    ...
    trace_file << Global_Physical_Variables::external_pressure() /
                    (pow(0.05, 3) / 12.0)
               << " " << Trace_node_pt->x(1) << " " << Trace_node2_pt->x(0)
               << std::endl;
