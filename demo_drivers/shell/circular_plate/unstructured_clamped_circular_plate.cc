...
namespace oomph
{
  ...
  public:
    ...
    inline Vector<double> interpolated_u_shell(const Vector<double>& s) const
    {
      ...
      this->my_interpolated_u_normal(
        s, u, interpolated_dudxi, interpolated_d2udxi);
  ...
  //======================================================================
  /// C1CurvedShellElement elements are with subparametric interpolation for the
  /// function.
  //======================================================================
  template<unsigned DIM, unsigned NNODE_1D>
...
namespace oomph
{
                                                ...
                                                const unsigned& flag)
  {
    ...
    DShape dpsidxi1(3, n_position_type, DIM), dtestdxi(n_node, DIM),
      dpsidxi(n_node, DIM);
    DShape d2psidxi1(3, n_position_type, 3), d2testdxi(n_node, 3),
      d2psidxi(n_node, 3);
    ...
    for (unsigned ipt = 0; ipt < n_intpt; ipt++)
    {
      ...
      // direcion--> Lagrange shape function
      ...
      // direction--> Bell shape function for straight triangular element
      ...
      Vector<double> ut(2, 0.0);
      DenseMatrix<double> interpolated_dutdxi(2, 2, 0.0);
      this->my_interpolated_u_tangential(
        integration_point, ut, interpolated_dutdxi);
      ...
      // compute for the derivative of the based vectors in the tangential and
      // normal directions -> Christoffel symbols \Gamma^{m}_{i,alpha} =
      // dtdxi(i,alpha,m) = (dadxi(i,alpha,m)t(m,k))
      ...
      for (unsigned i = 0; i < n_dim; i++)
      {
        ...
        if (i == 2)
        {
              ...
              for (unsigned p = 0; p < n_position_type; p++)
              {
                ...
                for (unsigned al = 0; al < 2; al++)
                {
                  for (unsigned be = 0; be < 2; be++)
                  {
                    ...
                    delta_kappa(al, be) -=
                      (dtdxi(2, be, 2)) * dpsidxi1(l, p, al);

                    // for the first derivative with respect to \be
                    delta_kappa(al, be) -=
                      (dtdxi(2, al, 2)) * dpsidxi1(l, p, be);
                  }
          ...
          else if (bd_element == 1)
          {
            ...
            // The shape functions are complete polynomials of degree 7 with 36
            // dofs
            Shape psi_curve(36);
            DShape dpsi_curve(36, 2), d2psi_curve(36, 3);
            this->d2basis_eulerian_curve(
              integration_point, psi_curve, dpsi_curve, d2psi_curve);

            //----------------
            // Rearrange nodes to be in the right position
            DenseMatrix<double> D(21, 21), B(21, 36), position(3, 2),
              bd_position(20, 2);
            Vector<double> x(2, 0.0);
            this->get_value_transform_matrix(
                ...
                for (unsigned l = 0; l < 36; l++)
                {
                  ...
                  for (unsigned al = 0; al < 2; al++)
                  {
                    for (unsigned be = 0; be < 2; be++)
                    {
                      // compute for variations of the strain tensor
                      delta_gamma(al, be) =
                        (dtdxi(2, be, al) + dtdxi(2, al, be)) / 2.0 *
                        psi_curve[l];
                      ...
                      delta_kappa(al, be) +=
                        -1.0 / adet * dtdxi(al, be, 0) *
                        (dtdxi(2, 1, 2) * tensor_a[0][1] * psi_curve[l] -
                         dtdxi(2, 0, 2) * tensor_a[1][1] * psi_curve[l]);

                      delta_kappa(al, be) +=
                        -1.0 / adet * dtdxi(al, be, 1) *
                        (-1 * dtdxi(2, 1, 2) * tensor_a[0][0] * psi_curve[l] +
                         dtdxi(2, 0, 2) * tensor_a[1][0] * psi_curve[l]);

                      delta_kappa(al, be) +=
                        -1.0 / adet * dtdxi(al, be, 2) *
                        (dtdxi(2, 1, 1) * tensor_a[0][0] * psi_curve[l] -
                         dtdxi(2, 1, 0) * tensor_a[0][1] * psi_curve[l] +
                         dtdxi(2, 0, 0) * tensor_a[1][1] * psi_curve[l] -
                         dtdxi(2, 0, 1) * tensor_a[1][0] * psi_curve[l]);
                      ...
                      delta_kappa(al, be) +=
                        -1.0 / adet * dtdxi(al, be, 0) *
                        (tensor_a[0][1] * dpsi_curve(l, 1) -
                         tensor_a[1][1] * dpsi_curve(l, 0));

                      delta_kappa(al, be) +=
                        -1.0 / adet * dtdxi(al, be, 1) *
                        (-1 * tensor_a[0][0] * dpsi_curve(l, 1) +
                         tensor_a[1][0] * dpsi_curve(l, 0));

                      // for the zero derivative
                      for (unsigned k = 0; k < n_dim; k++)
                      {
                        delta_kappa(al, be) -=
                          (dtdxi(2, al, k) * dtdxi(k, be, 2)) * psi_curve[l];
                      }

                      // for the first derivative with respect to \al
                      delta_kappa(al, be) -=
                        (dtdxi(2, be, 2)) * dpsi_curve(l, al);

                      // for the first derivative with respect to \be
                      delta_kappa(al, be) -=
                        (dtdxi(2, al, 2)) * dpsi_curve(l, be);
                    }
                  } // end of compute for variations
                  ...
                  {
                    // add in external forcing
                    residuals[local_eqn] -= (1.0 / H) * source[i] *
                                            psi_curve[l] * W1 * sqrt(adet) *
                                            M(row, l);
                    // Loop over the Greek indicies
                    for (unsigned al = 0; al < 2; al++)
                    {
...
int main(int argc, char* argv[])
{
  ...
  MyLinearisedShellProblem<C1CurvedShellElement<2, 3>, 2, 3> problem(
