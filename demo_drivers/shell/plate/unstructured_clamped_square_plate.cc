...
namespace oomph
{
  ...
  public:
    ...
    virtual inline unsigned u_index_shell() const
    {
    ...
    inline Vector<double> interpolated_u_shell(const Vector<double> &s) const
    {
      ...
      double adet =
        tensor_a[0][0] * tensor_a[1][1] - tensor_a[0][1] * tensor_a[1][0];
      ...
      // /// displacement component in y direction is identical with normal
      // dirention double x_dir; double y_dir; double z_dir;

      // x_dir = interpolated_u[0]*t(0,0) + interpolated_u[1]*t(1,0) +
      // interpolated_u[2]*unit_n[0]; y_dir = interpolated_u[0]*t(0,1) +
      // interpolated_u[1]*t(1,1) + interpolated_u[2]*unit_n[1]; z_dir =
      // interpolated_u[0]*t(0,2) + interpolated_u[1]*t(1,2) +
      // interpolated_u[2]*unit_n[2];
  ...
  //======================================================================
  /// BellShellElement elements are with subparametric interpolation for the
  /// function.
  //======================================================================
  template<unsigned DIM, unsigned NNODE_1D>
                                              ...
                                              DShape &d2testdx) const
  {
    ...
    const double J = this->d2shape_and_d2test_eulerian_at_knot(
      ipt, psi, dpsidx, d2psidx, test, dtestdx, d2testdx);
...
namespace oomph
{
                                                ...
                                                const unsigned &flag)
  {
    ...
    Shape psi1(3, n_position_type), test1(3, n_position_type), test(n_node),
      psi(n_node);
    DShape dpsidxi1(3, n_position_type, DIM),
      dtestdxi1(3, n_position_type, DIM), dtestdxi(n_node, DIM),
      dpsidxi(n_node, DIM);
    DShape d2psidxi1(3, n_position_type, 3), d2testdxi1(3, n_position_type, 3),
      d2testdxi(n_node, 3), d2psidxi(n_node, 3);
    ...
    for (unsigned ipt = 0; ipt < n_intpt; ipt++)
    {
      ...
      // Call the derivatives of the c0-shape and test functions for tangential
      // direcion
      double J = dshape_and_dtest_eulerian_at_knot_shell(
        ipt, psi, dpsidxi, test, dtestdxi);

      // Call the derivatives of the c1-shape and test functions for normal
      // direction
      double J1 = d2shape_and_d2test_eulerian_at_knot_shell(
        ipt, psi1, dpsidxi1, d2psidxi1, test, dtestdxi, d2testdxi);
      ...
      for (unsigned i = 0; i < n_lagrangian; i++)
      {
        vec[0] = (dadxi(0, i, 1) * t(1, 2) - dadxi(0, i, 2) * t(1, 1)) -
                 (dadxi(1, i, 1) * t(0, 2) - dadxi(1, i, 2) * t(0, 1));
        vec[1] = (dadxi(0, i, 2) * t(1, 0) - dadxi(0, i, 0) * t(1, 2)) -
                 (dadxi(1, i, 2) * t(0, 0) - dadxi(1, i, 0) * t(0, 2));
        vec[2] = (dadxi(0, i, 0) * t(1, 1) - dadxi(0, i, 1) * t(1, 0)) -
                 (dadxi(1, i, 0) * t(0, 1) - dadxi(1, i, 1) * t(0, 0));
      ...
      // compute for the tangential gradient in tangential and normal directions
      // -> Christoffel symbol \Gamma^{m}_{i,alpha} = dtdxi(i,alpha,m) =
      // (dadxi(i,alpha,m)t(m,k))
      ...
      for (unsigned al = 0; al < 2; al++)
      {
        ...
        {
          gamma[al][be] =
            (interpolated_dudxi_tn(al, be) + interpolated_dudxi_tn(be, al)) /
            2.0;
        }
      }
      ...
      for (unsigned c = 0; c < 3; c++)
      {
        ...
        for (unsigned k = 0; k < n_dim; k++)
        {
          ...
          for (unsigned j = 0; j < n_dim; j++)
          {
            interpolated_d2udxi_tn(k, c) +=
              interpolated_dudxi_tn(j, al) * dtdxi(j, be, k);
          }
          for (unsigned i = 0; i < n_dim; i++)
          {
            interpolated_d2udxi_tn(k, c) +=
              interpolated_dudxi(i, be) * dtdxi(i, al, k);
          }
        }
      }
      ...
      L[0] = tensor_a[0][1] * interpolated_dudxi_tn(2, 1) -
             tensor_a[1][1] * interpolated_dudxi_tn(2, 0);
      L[1] = -1.0 * tensor_a[0][0] * interpolated_dudxi_tn(2, 1) +
             tensor_a[1][0] * interpolated_dudxi_tn(2, 0);
      L[2] = tensor_a[0][0] * interpolated_dudxi_tn(1, 1) -
             tensor_a[0][1] * interpolated_dudxi_tn(0, 1) +
             tensor_a[1][1] * interpolated_dudxi_tn(0, 0) -
             tensor_a[1][0] * interpolated_dudxi_tn(1, 0);
            ...
            for (unsigned p = 0; p < n_position_type; p++)
            {
              ...
              for (unsigned al = 0; al < 2; al++)
              {
                for (unsigned be = 0; be < 2; be++)
                {
                  ...
                  delta_gamma(al, be) =
                    (dtdxi(2, be, al) + dtdxi(2, al, be)) / 2.0 * psi1(l, p);
                  ...
                  delta_kappa(al, be) +=
                    -1.0 / adet * dtdxi(al, be, 0) *
                    (dtdxi(2, 1, 2) * tensor_a[0][1] * psi1(l, p) -
                     dtdxi(2, 0, 2) * tensor_a[1][1] * psi1(l, p));

                  delta_kappa(al, be) +=
                    -1.0 / adet * dtdxi(al, be, 1) *
                    (-1 * dtdxi(2, 1, 2) * tensor_a[0][0] * psi1(l, p) +
                     dtdxi(2, 0, 2) * tensor_a[1][0] * psi1(l, p));

                  delta_kappa(al, be) +=
                    -1.0 / adet * dtdxi(al, be, 2) *
                    (dtdxi(2, 1, 1) * tensor_a[0][0] * psi1(l, p) -
                     dtdxi(2, 1, 0) * tensor_a[0][1] * psi1(l, p) +
                     dtdxi(2, 0, 0) * tensor_a[1][1] * psi1(l, p) -
                     dtdxi(2, 0, 1) * tensor_a[1][0] * psi1(l, p));

                  delta_kappa(al, be) +=
                    dtdxi(al, be, 2) / adet / adet *
                    (dtdxi(2, 1, 1) * tensor_a[0][0] * psi1(l, p) -
                     dtdxi(2, 1, 0) * tensor_a[0][1] * psi1(l, p) +
                     dtdxi(2, 0, 0) * tensor_a[1][1] * psi1(l, p) -
                     dtdxi(2, 0, 1) * tensor_a[1][0] * psi1(l, p));

                  delta_kappa(al, be) += -1.0 / adet * dtdxi(al, be, 0) *
                                         (tensor_a[0][1] * dpsidxi1(l, p, 1) -
                                          tensor_a[1][1] * dpsidxi1(l, p, 0));

                  delta_kappa(al, be) +=
                    -1.0 / adet * dtdxi(al, be, 1) *
                    (-1 * tensor_a[0][0] * dpsidxi1(l, p, 1) +
                     tensor_a[1][0] * dpsidxi1(l, p, 0));

                  // for the zero derivative
                  for (unsigned k = 0; k < n_dim; k++)
                  {
                    delta_kappa(al, be) -=
                      (dtdxi(2, al, k) * dtdxi(k, be, 2)) * psi1(l, p);
                  }
              ...
              if (local_eqn >= 0)
              {
                ...
                residuals[local_eqn] -=
                  (1.0 / H) * source[i] * psi1(l, p) * W1 * sqrt(adet);
                  ...
                  for (unsigned be = 0; be < 2; be++)
                  {
                    for (unsigned ga = 0; ga < 2; ga++)
                    {
                      for (unsigned de = 0; de < 2; de++)
                      {
                        residuals[local_eqn] +=
                          Et[al][be][ga][de] *
                          (gamma[al][be] * delta_gamma(ga, de) +
                           1.0 / 12.0 * H * H * kappa[al][be] *
                             delta_kappa(ga, de)) *
                          W1 * sqrt(adet);
                      }
                    }
        ...
        else /// find residuals for the first and second directions
        {
          ...
          for (unsigned l = 0; l < n_node; l++)
          {
            ...
            if (i == 0)
            {
              ...
              delta_gamma(0, 1) =
                dpsidxi(l, 1) / 2.0 +
                (dtdxi(i, 1, 0) + dtdxi(i, 0, 1)) / 2.0 * psi[l];
              delta_gamma(1, 0) =
                dpsidxi(l, 1) / 2.0 +
                (dtdxi(i, 1, 0) + dtdxi(i, 0, 1)) / 2.0 * psi[l];
            ...
            for (unsigned al = 0; al < 2; al++)
            {
              for (unsigned be = 0; be < 2; be++)
              {
                ...
                if (i == 0)
                {
                  delta_kappa(al, be) +=
                    -1.0 / adet * dtdxi(al, be, 0) *
                    (dtdxi(0, 1, 2) * tensor_a[0][1] * psi[l] -
                     dtdxi(0, 0, 2) * tensor_a[1][1] * psi[l]);

                  delta_kappa(al, be) +=
                    -1.0 / adet * dtdxi(al, be, 1) *
                    (-1 * dtdxi(0, 1, 2) * tensor_a[0][0] * psi[l] +
                     dtdxi(0, 0, 2) * tensor_a[1][0] * psi[l]);

                  delta_kappa(al, be) +=
                    -1.0 / adet * dtdxi(al, be, 2) *
                    (dtdxi(0, 1, 1) * tensor_a[0][0] * psi[l] -
                     dtdxi(0, 1, 0) * tensor_a[0][1] * psi[l] +
                     dtdxi(0, 0, 0) * tensor_a[1][1] * psi[l] -
                     dtdxi(0, 0, 1) * tensor_a[1][0] * psi[l]);
                  ...
                  delta_kappa(al, be) += -1.0 / adet * dtdxi(al, be, 2) *
                                         (-1 * tensor_a[0][1] * dpsidxi(l, 1) +
                                          tensor_a[1][1] * dpsidxi(l, 0));

                  delta_kappa(al, be) += dtdxi(al, be, 2) / adet / adet *
                                         (-1 * tensor_a[0][1] * dpsidxi(l, 1) +
                                          tensor_a[1][1] * dpsidxi(l, 0));
                }
                else if (i == 1)
                {
                  delta_kappa(al, be) +=
                    -1.0 / adet * dtdxi(al, be, 0) *
                    (dtdxi(1, 1, 2) * tensor_a[0][1] * psi[l] -
                     dtdxi(1, 0, 2) * tensor_a[1][1] * psi[l]);

                  delta_kappa(al, be) +=
                    -1.0 / adet * dtdxi(al, be, 1) *
                    (-1 * dtdxi(1, 1, 2) * tensor_a[0][0] * psi[l] +
                     dtdxi(1, 0, 2) * tensor_a[1][0] * psi[l]);

                  delta_kappa(al, be) +=
                    -1.0 / adet * dtdxi(al, be, 2) *
                    (dtdxi(1, 1, 1) * tensor_a[0][0] * psi[l] -
                     dtdxi(1, 1, 0) * tensor_a[0][1] * psi[l] +
                     dtdxi(1, 0, 0) * tensor_a[1][1] * psi[l] -
                     dtdxi(1, 0, 1) * tensor_a[1][0] * psi[l]);
                  ...
                  delta_kappa(al, be) += -1.0 / adet * dtdxi(al, be, 2) *
                                         (tensor_a[0][0] * dpsidxi(l, 1) -
                                          tensor_a[1][0] * dpsidxi(l, 0));
                ...
                {
                  delta_kappa(al, be) -=
                    (dtdxi(i, al, k) * dtdxi(k, be, 2)) * psi[l];
                }
            ...
            if (local_eqn >= 0)
            {
              // add in external forcing
              residuals[local_eqn] -=
                (1.0 / H) * source[i] * psi[l] * W * sqrt(adet);
              // Loop over the Greek indicies
              for (unsigned al = 0; al < 2; al++)
              {
                for (unsigned be = 0; be < 2; be++)
  ...
  Source_fct_pt(source_fct_pt)
{
  ...
  Problem::mesh_pt() = new TriangleMesh<ELEMENT>(
...
//========================================================================
template<class ELEMENT, unsigned DIM, unsigned NNODE_1D>
void MyLinearisedShellProblem<ELEMENT, DIM, NNODE_1D>::
...
int main(int argc, char *argv[])
{
  ...
  if (argc != 4)
  {
    ...
    error_message += "Must specify the following file names  \n";
    error_message += "filename.node then filename.ele then filename.poly\n";

    throw OomphLibError(error_message, "main()", OOMPH_EXCEPTION_LOCATION);
  }
  ...
  // Set up the problem:
  MyLinearisedShellProblem<BellShellElement<2, 3>,
                           2,
                           3> // Element type as template parameter
