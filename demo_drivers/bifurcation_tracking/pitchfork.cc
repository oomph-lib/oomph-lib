                        ...
                        TimeStepper* time_stepper_pt)
{
  ...
  for (unsigned l1 = 1; l1 < (Np - 1); l1++)
  {
    ...
    Node_pt[node_count] =
      finite_element_pt(Nx - 1)->construct_node(l1, time_stepper_pt);
    // Set the position of the node
    ...
    for (unsigned ipt = 0; ipt < n_intpt; ipt++)
    {
      ...
      for (unsigned l = 0; l < n_node; l++)
      {
        ...
        if (local_eqn >= 0)
        {
          ...
          // Viscous term
          residuals[local_eqn] -= interpolated_dfdx[1] * dtestfdx(l, 0) * W;
          // Inertial term
          residuals[local_eqn] +=
            Re *
            (interpolated_f[0] * interpolated_dfdx[1]
             /*Comment out the next term for Iain's equations*/
             - interpolated_f[1] * interpolated_dfdx[0]) *
            testf[l] * W;
            ...
            for (unsigned l2 = 0; l2 < n_node; l2++)
            {
              ...
              if (local_unknown >= 0)
              {
                jacobian(local_eqn, local_unknown) +=
                  Re *
                  (psif[l2] * interpolated_dfdx[1]
                   /*Comment out the next term for Iain's equations*/
                   - interpolated_f[1] * dpsifdx(l2, 0)) *
                  testf[l] * W;
              }
              ...
              if (local_unknown >= 0)
              {
                ...
                jacobian(local_eqn, local_unknown) +=
                  Re *
                  (interpolated_f[0] * dpsifdx(l2, 0)
                   /*Comment out the next term for Iains' equations*/
                   - psif[l2] * interpolated_dfdx[0]) *
                  testf[l] * W;
              }
            }
        ...
        if (local_eqn >= 0)
        {
          residuals[local_eqn] += (interpolated_f[1] * testf[l] +
                                   interpolated_dfdx[0] * dtestfdx(l, 0)) *
                                  W;
...
int main()
{
  ...
  for (unsigned i = 0; i < 6; i++)
  {
    ...
    trace << Re << " " << Test_pt->interpolated_f(s, 1) << " "
  ...
  std::cout << "The slack parameter is " << problem.dof(2 * n_dof + 1)
            << std::endl;
