...
int main()
{
  ...
  // Create Problem
  FlowAroundCylinderProblem<RefineableQCrouzeixRaviartElement<2>> problem(
  ...
  // the data file
  problem.activate_hopf_tracking(
    &Global_Parameters::Re, frequency, eigenvectors[0], eigenvectors[1]);
  // Solve the problem
  problem.newton_solve();
  ...
  trace << Global_Parameters::Re << " " << problem.dof(problem.ndof() - 1)
        << "\n";
  trace.close();
}
