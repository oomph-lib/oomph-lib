...
public:
                                             ...
                                             unsigned flag)
  {
    ...
    if (flag)
    {
      ...
      if (flag == 2)
      {
        jacobian(local_eqn, local_unknown) = -1.0 - p0 * exp_u2;
      }
      else
      {
    ...
    residuals[local_eqn] =
      -du_dt[1] - u[1] + p0 * (1.0 - u[0] - p4 * u[1]) * exp_u2;
    ...
    if (flag)
    {
      ...
      if (flag == 2)
      {
        jacobian(local_eqn, local_unknown) = -1.0 - p0 * p4 * exp_u2;
      }
      else
      {
        jacobian(local_eqn, local_unknown) =
    ...
    residuals[local_eqn] = -du_dt[2] - u[2] - p2 * u[2] +
    ...
    if (flag)
    {
      ...
      {
        jacobian(local_eqn, local_unknown) =
          -1.0 - p2 + p0 * p3 * (1.0 - u[0] + p1 * p4 * u[1]) * exp_u2;
      }
      else
      {
        jacobian(local_eqn, local_unknown) =
                        ...
                        const double& time = 0.0)
  {
    ...
    this->internal_data_pt(Internal_index)
      ->time_stepper_pt()
      ->time_derivative(0, internal_data_pt(Internal_index), u);
    this->internal_data_pt(Internal_index)
      ->time_stepper_pt()
...
void ABCProblem<ELEMENT, TIMESTEPPER>::solve()
{
  ...
  std::cout << "Hopf bifurcation found at " << Global_Physical_Variables::P[0]
            << " " << dof(ndof() - 1) << " "
            << mesh_pt()->element_pt(0)->internal_data_pt(0)->value(0) << " "
            << mesh_pt()->element_pt(0)->internal_data_pt(0)->value(1) << " "
            << mesh_pt()->element_pt(0)->internal_data_pt(0)->value(2)
  ...
  assembly_handler_pt() = new PeriodicOrbitAssemblyHandler<7>(
    this, n_time_element, initial_guess, 8.0 * atan(1.0) / period);
  ...
  std::cout << "Orbit found with period " << 1.0 / this->dof(this->ndof() - 1)
            << "\n";
