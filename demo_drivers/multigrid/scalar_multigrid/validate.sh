...
#-------------------------------------------------------
compare_length="$OOMPH_ROOT_DIR/scripts/compare_file_length_with_tolerance.bash"

# The address of the bash script to compare file data
#----------------------------------------------------
compare_data="$OOMPH_ROOT_DIR/scripts/fpdiff.py"
...
for code in ${code_stem[@]}; do
	...
	for nnode in ${nnode_1d_list[@]}; do
		for pre in ${presmoother_list[@]}; do
			for post in ${postsmoother_list[@]}; do
				for adapt in ${use_adapt[@]}; do
					# Make it
					#--------
