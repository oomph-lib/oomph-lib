...
void UnitCubePoissonMGProblem<ELEMENT, MESH>::doc_solution()
{
  ...
  mesh_pt()->compute_error(
    some_file, TanhSolnForPoisson::get_exact_u, error, soln);
...
int main(int argc, char *argv[])
{
  ...
  CommandLineArgs::specify_command_line_flag("--n_adapt",
                                             &Global_Parameters::N_adaptations);
  ...
  if (Global_Parameters::Use_adaptation_flag)
  {
    ...
    // Refine once at least otherwise the V-cycle only uses SuperLU and
    // converges instantly
