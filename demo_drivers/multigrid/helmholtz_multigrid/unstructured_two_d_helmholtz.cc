...
PMLHelmholtzMGProblem<ELEMENT>::PMLHelmholtzMGProblem()
{
  ...
  Bulk_mesh_pt =
    new RefineableQuadFromTriangleMesh<ELEMENT>(triangle_mesh_parameters);
...
void PMLHelmholtzMGProblem<ELEMENT>::doc_solution()
{
  ...
  Bulk_mesh_pt->output_fct(
    some_file, npts, GlobalParameters::simple_exact_u_pt);
  some_file.close();
  ...
  Bulk_mesh_pt->compute_error(
    some_file, GlobalParameters::simple_exact_u_pt, error, norm);
  some_file.close();
...
void PMLHelmholtzMGProblem<ELEMENT>::create_pml_meshes()
{
  ...
  PML_right_mesh_pt =
    TwoDimensionalPMLHelper::create_right_pml_mesh<PMLLayerElement<ELEMENT>>(
      Bulk_mesh_pt, right_boundary_id, n_x_right_pml, width_x_right_pml);
  PML_top_mesh_pt =
    TwoDimensionalPMLHelper::create_top_pml_mesh<PMLLayerElement<ELEMENT>>(
      Bulk_mesh_pt, top_boundary_id, n_y_top_pml, width_y_top_pml);
  PML_left_mesh_pt =
    TwoDimensionalPMLHelper::create_left_pml_mesh<PMLLayerElement<ELEMENT>>(
      Bulk_mesh_pt, left_boundary_id, n_x_left_pml, width_x_left_pml);
  PML_bottom_mesh_pt =
    TwoDimensionalPMLHelper::create_bottom_pml_mesh<PMLLayerElement<ELEMENT>>(
      Bulk_mesh_pt, bottom_boundary_id, n_y_bottom_pml, width_y_bottom_pml);
  ...
  PML_top_right_mesh_pt = TwoDimensionalPMLHelper::create_top_right_pml_mesh<
    ...
    PML_right_mesh_pt, PML_top_mesh_pt, Bulk_mesh_pt, right_boundary_id);

  PML_bottom_right_mesh_pt =
    TwoDimensionalPMLHelper::create_bottom_right_pml_mesh<
      PMLLayerElement<ELEMENT>>(
      PML_right_mesh_pt, PML_bottom_mesh_pt, Bulk_mesh_pt, right_boundary_id);

  PML_top_left_mesh_pt =
    TwoDimensionalPMLHelper::create_top_left_pml_mesh<PMLLayerElement<ELEMENT>>(
      PML_left_mesh_pt, PML_top_mesh_pt, Bulk_mesh_pt, left_boundary_id);

  PML_bottom_left_mesh_pt =
    TwoDimensionalPMLHelper::create_bottom_left_pml_mesh<
      PMLLayerElement<ELEMENT>>(
      PML_left_mesh_pt, PML_bottom_mesh_pt, Bulk_mesh_pt, left_boundary_id);
...
int main(int argc, char** argv)
{
  ...
  CommandLineArgs::specify_command_line_flag("--n_adapt",
                                             &GlobalParameters::N_adaptations);
  ...
  CommandLineArgs::specify_command_line_flag("--k_sq",
                                             &GlobalParameters::K_squared);

  // Choose the value of the shift in the CSLP
  CommandLineArgs::specify_command_line_flag("--alpha",
                                             &GlobalParameters::Alpha_shift);
