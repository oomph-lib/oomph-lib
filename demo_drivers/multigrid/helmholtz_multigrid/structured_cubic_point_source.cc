...
PMLStructuredCubicHelmholtz<ELEMENT>::PMLStructuredCubicHelmholtz()
{
  ...
  //-----------------------------------------------
  // Build the "bulk" mesh
  Bulk_mesh_pt = new RefineableSimpleCubicMesh<ELEMENT>(GlobalParameters::Nx,
                                                        GlobalParameters::Ny,
                                                        GlobalParameters::Nz,
                                                        GlobalParameters::Lx,
                                                        GlobalParameters::Ly,
...
void PMLStructuredCubicHelmholtz<ELEMENT>::enable_pmls()
{
  ...
  double right_boundary =
    GlobalParameters::Lx - GlobalParameters::Pml_thickness;
...
{
  // Tell the user
  oomph_info << "\nDocumentation step: " << GlobalParameters::Doc_info.number()
             << std::endl;
  ...
  for (unsigned e = 0; e < n_element; e++)
  {
    ...
    if (el_pt != 0)
    {
      ...
      el_pt->compute_error(
        some_file, GlobalParameters::get_simple_exact_u, el_error, el_norm);
  ...
  oomph_info << "\nSolution norm : " << norm << "\nAbsolute error: " << error
             << "\nRelative error: " << error / norm << std::endl;
