...
//LIC//====================================================================
//
//  Linearised poro-elastic fsi problem -- flow in a cylindrical poroelastic
//  tube
...
namespace Global_Physical_Variables
{
  ...
  /// Define the poroelasticity inertia parameter -- dependent parameter;
  /// compute from Re, Q and density ratio
  ...
  void update_dependent_parameters()
  {
    ...
    // Ratio of the pore fluid density to the compound density
    Rho_f_over_rho =
      Density_ratio_poro / (1.0 + Porosity * (Density_ratio_poro - 1.0));

    // Poro-elastic timescale ratio
    Lambda_sq =
      Poro_elastic_to_navier_stokes_fluid_density * Re * Q / Rho_f_over_rho;
  ...
  void doc_dependent_parameters()
  {
    ...
    oomph_info << "==================" << std::endl;
    oomph_info << "Reynolds number (Re)                           : " << Re
               << std::endl;
    oomph_info << "Strouhal number (St)                           : " << St
               << std::endl;
    oomph_info << "Womersley number (ReSt)                        : " << Wo
               << std::endl;
    oomph_info << "FSI parameter (Q)                              : " << Q
               << std::endl;
    ...
    oomph_info << "Biot parameter (alpha)                         : " << Alpha
               << std::endl;
    oomph_info << "Porosity                                       : "
               << Porosity << std::endl;
    ...
    oomph_info << "Fluid density ratio (rho_f/rho_f_nst)          : "
               << Poro_elastic_to_navier_stokes_fluid_density << std::endl;
    oomph_info << "rho_fluid/rho_compound                         : "
               << Rho_f_over_rho << std::endl;
    oomph_info << "Poisson's ratio (Nu)                           : " << Nu
               << std::endl;
    oomph_info << "First Lame parameter (Lambda_lame)             : "
               << Lambda_lame << std::endl;
    oomph_info << "Second Lame parameter (Mu_lame)                : " << Mu_lame
               << std::endl;
    ...
    // For information:
    {
      ...
      oomph_info << "Timestep required to sub-divide pulse-wave propagation \n"
                 << "along tube length into " << nt << " steps: " << dt
                 << std::endl;
    }
  }
...
public:
  ...
  void actions_before_implicit_timestep()
  {
    ...
    Global_Physical_Variables::P_inlet_const =
      ...
      Global_Physical_Variables::P_inlet_step * 0.5 *
        (1.0 + tanh(Global_Physical_Variables::Alpha_tanh *
...
//============================================================================
template<class FLUID_ELEMENT, class POROELASTICITY_ELEMENT>
PressureWaveFSIProblem<FLUID_ELEMENT,
                       POROELASTICITY_ELEMENT>::PressureWaveFSIProblem()
{
  // Create timesteppers
  ...
  fluid_triangle_mesh_parameters.internal_open_curves_pt() =
    bl_open_boundary_pt;
  ...
  unsigned nplot_r = 5;
  unsigned nplot_z =
    unsigned(double(nplot_r) * Global_Physical_Variables::Length /
             Global_Physical_Variables::Inner_radius);
  ...
  {
    x[0] = double(ir) / double(nplot_r - 1) *
           Global_Physical_Variables::Inner_radius;
  ...
  temp_inner_coord[1][1] = Global_Physical_Variables::Solid_mesh_bl_thickness;
  ...
  // Use the TriangleMeshParameters object for gathering all
  // the necessary arguments for the TriangleMesh object
  TriangleMeshParameters poro_triangle_mesh_parameters(poro_outer_boundary_pt);
  ...
  // Extract points
  nplot_r = 5;
  nplot_z = unsigned(double(nplot_r) * Global_Physical_Variables::Length /
...
//========================================================================
template<class FLUID_ELEMENT, class POROELASTICITY_ELEMENT>
void PressureWaveFSIProblem<FLUID_ELEMENT,
                            POROELASTICITY_ELEMENT>::create_poro_face_elements()
{
  // FSI boundary
  //-------------
...
void PressureWaveFSIProblem<FLUID_ELEMENT, POROELASTICITY_ELEMENT>::setup_fsi()
{
  ...
  Multi_domain_functions::
    setup_bulk_elements_adjacent_to_face_mesh<POROELASTICITY_ELEMENT, 2>(
  ...
  doc_solution(DocInfo& doc_info)
{
  ...
  for (unsigned i = 0; i < npts; i++)
  {
    ...
    file << x[0] << " " << x[1] << " " << veloc[0] << " " << veloc[1] << " "
         << veloc[2] << " " << el_pt->interpolated_p_axi_nst(s) << " "
         << std::endl;
  }
  file.close();
  ...
  for (unsigned i = 0; i < npts; i++)
  {
    ...
    file << x[0] << " " << x[1] << " " << el_pt->interpolated_u(s, 0) << " "
         << el_pt->interpolated_u(s, 1) << " " << el_pt->interpolated_q(s, 0)
         << " " << el_pt->interpolated_q(s, 1) << " "
         << el_pt->interpolated_p(s) << " " << std::endl;
  }
  file.close();

  // Poroelasticity FSI traction
  sprintf(filename,
          "%s/poro_traction%i.dat",
          doc_info.directory().c_str(),
          doc_info.number());
  file.open(filename);
...
int main(int argc, char** argv)
{
  ...
  CommandLineArgs::specify_command_line_flag(
    "--p_inlet_initial", &Global_Physical_Variables::P_inlet_initial);

  // Step in inlet pressure (applied with tanh profile)
  CommandLineArgs::specify_command_line_flag(
    "--p_inlet_step", &Global_Physical_Variables::P_inlet_step);

  // Non-dim permeability
  CommandLineArgs::specify_command_line_flag(
    "--permeability", &Global_Physical_Variables::Permeability);
  ...
  // Porosity
  CommandLineArgs::specify_command_line_flag(
    "--porosity", &Global_Physical_Variables::Porosity);
  ...
  CommandLineArgs::specify_command_line_flag(
    "--el_area_fluid", &Global_Physical_Variables::Element_area_fluid);

  // Element area for solid mesh
  CommandLineArgs::specify_command_line_flag(
    "--el_area_solid", &Global_Physical_Variables::Element_area_solid);
  ...
  CommandLineArgs::specify_command_line_flag(
    "--alpha_tanh", &Global_Physical_Variables::Alpha_tanh);

  // "Time" for tanh increment of pressure load
  CommandLineArgs::specify_command_line_flag(
    "--t_tanh", &Global_Physical_Variables::T_tanh);
  ...
  // Timestep
  oomph_info << "About to do " << nstep << " timesteps with dt = " << dt
             << std::endl;
