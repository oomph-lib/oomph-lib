...
namespace Flags
{
  ...
  void segregated_doc_flags()
  {
    ...
    std::cout << "-- Convergence tolerance: " << Convergence_tolerance
              << std::endl;

    std::cout << "-- Under-relaxation parameter: " << Omega_under_relax
              << std::endl;
    ...
    std::cout << "-- Reynolds number: " << Global_Physical_Variables::Re
              << std::endl;

    std::cout << "-- FSI parameter Q: " << Global_Physical_Variables::Q
              << std::endl;
  ...
  public virtual SegregatableFSIProblem
{
  ...
  /// \short Vector of pairs of pointers to wall elements
...
public:
  ...
  /// \short Identify the fluid and solid Data and meshes that
  ...
  /// \short Overload actions before the segregated_convergence_check
                                        ...
                                        steady_flag)
{
  ...
  for (unsigned i = 0; i < n_control; i++)
  {
    ...
    this->Wall_geom_object_pt->locate_zeta(
      zeta_ctrl, Control_point_pair[i].first, Control_point_pair[i].second);

    Vector<double> r_ctrl(2);
    Control_point_pair[i].first->position(Control_point_pair[i].second, r_ctrl);
  }
  ...
  ELEMENT>::actions_before_newton_convergence_check()
{
  ...
  if (!Flags::Use_segregated_solver)
  {
    ...
    std::cout << "==================================================\n";
    std::cout << "Iteration             : " << this->Newton_iter << std::endl;
    std::cout << "RMS  change           :       " << rms_change << std::endl;
    std::cout << "Max. change           :       " << max_change << std::endl;
    std::cout << "RMS norm              :       " << rms_norm << std::endl;
    std::cout << "Max. global residual  :       " << max_res << std::endl;
    std::cout << "==================================================\n\n";
...
template<class ELEMENT>
void SegregatedFSICollapsibleChannelProblem<
  ELEMENT>::actions_before_segregated_convergence_check()
{
  ...
  if (!Recheck_convergence_after_pointwise_aitken)
  {
    ++Picard_iter;
  }
  ...
  ELEMENT>::write_zone_info_for_convergence_history(ofstream &convergence_file)
{
  ...
  if (Flags::Use_segregated_solver)
  {
    ...
    convergence_file << "; Tol = " << Flags::Convergence_tolerance;
  }
  else
  {
  ...
  const unsigned &niter)
{
  ...
  sprintf(
    filename, "%s/beam%i.dat", doc_info.directory().c_str(), doc_info.number());
  some_file.open(filename);
  ...
  trace_file << this->Left_node_pt->value(0) << " "
             ...
             << Flags::Use_pointwise_aitken << " " << Flags::Omega_under_relax
             << " " << Flags::Use_irons_and_tuck_extrapolation << " " << cpu
             << " " << niter << " " << std::endl;
...
void SegregatedFSICollapsibleChannelProblem<ELEMENT>::steady_run()
{
  ...
  Global_Physical_Variables::P_ext_data_pt->set_value(
    0, Global_Physical_Variables::Pmin);
  ...
  if (Flags::Use_segregated_solver)
  {
    ...
    trace_file << "; Tol = " << Flags::Convergence_tolerance;
  }
  else
  {
    trace_file << "Newton";
  ...
  double delta_p =
    (Global_Physical_Variables::Pmax - Global_Physical_Variables::Pmin) /
  ...
  double delta_y =
    (y_min - Global_Physical_Variables::Yprescr) / double(Flags::Nsteps - 1);
  ...
  for (unsigned istep = 0; istep < Flags::Nsteps; istep++)
  {
    ...
    if (this->Displ_control)
    {
      std::cout << "Solving for control displ = "
                << Global_Physical_Variables::Yprescr << std::endl;
    }
    else
    {
    ...
    if (Flags::Use_segregated_solver)
    {
      ...
      if (conv_data.cpu_total() != 0.0)
      {
        ...
        std::cout << "PERCENTAGE OF CPU TIME SPENT IN COMPUTATION OF "
                     "NON-ESSENTIAL BITS="
                  << double((conv_data.cpu_total() -
                             conv_data.essential_cpu_total()) /
                            conv_data.cpu_total()) *
                       100.0
                  << " " << conv_data.cpu_total() << " "
                  << conv_data.essential_cpu_total() << " " << std::endl
                  << std::endl
                  << std::endl
                  << std::endl;
      }
  ...
  const double &dummy_dt)
{
  ...
  // jump) are specified
  Global_Physical_Variables::P_ext_data_pt->set_value(
    0, Global_Physical_Variables::Pmax);
  ...
  std::cout << "Pressure before set initial: "
            << Global_Physical_Variables::P_ext_data_pt->value(0) << std::endl;
