...
// If we are using MPI, then only use HYPRE if it has been initialised
      ...
      default:
        ...
        std::ostringstream error_stream;
        error_stream << "Error: Wrong solver_flag: " << solver_flag
                     << std::endl;
        throw OomphLibError(
...
int main(int argc, char* argv[])
{
  ...
  else
  {
    std::cout
      ...
      << "- solver flag [0: direct; 1: exact; 2: simple; 3: Schur/Superlu; 4: "
         "Schur/Hypre]"
      << std::endl
      << "- solver sub flag [0: diag; 1: retain fluid on solid; 2: retain "
         "solid on fluid]"
      << std::endl
