    ...
    if (t < Ramp_period)
    {
      return Min_flux +
             (Max_flux - Min_flux) * 0.5 *
               (1.0 - cos(MathematicalConstants::Pi * t / Ramp_period));
    }
    else
    {
      return Max_flux;
...

{
  ...
  std::cout << "Coordinates of solid control point "
            << Solid_control_node_pt->x(0) << " " << Solid_control_node_pt->x(1)
            << " " << std::endl;
  ...
  // Build fluid mesh
  Fluid_mesh_pt = new RefineableAlgebraicCylinderWithFlagMesh<FLUID_ELEMENT>(
    cylinder_pt,
    top_flag_pt,
    bottom_flag_pt,
    tip_flag_pt,
  ...
  for (unsigned inod = 0; inod < num_nod; inod++)
  {
    ...
    double uy =
      ampl * 6.0 * ycoord / Domain_height * (1.0 - ycoord / Domain_height);
...
//=======================================================================
template<class FLUID_ELEMENT, class SOLID_ELEMENT>
void TurekProblem<FLUID_ELEMENT,
                  SOLID_ELEMENT>::actions_before_newton_convergence_check()
{
...
template<class FLUID_ELEMENT, class SOLID_ELEMENT>
void TurekProblem<FLUID_ELEMENT,
                  SOLID_ELEMENT>::actions_before_implicit_timestep()
{
  // Current time
...
void TurekProblem<FLUID_ELEMENT, SOLID_ELEMENT>::actions_after_adapt()
{
  ...
  PVDEquationsBase<2>::unpin_all_solid_pressure_dofs(
    solid_mesh_pt()->element_pt());
  ...
  DocInfo& doc_info, ofstream& trace_file)
{
  ...
  trace_file << time_pt()->time() << " " << Solid_control_node_pt->x(0) << " "
             << Solid_control_node_pt->x(1) << " "
             << Fluid_control_node_pt->value(2) << " "
             << Global_Parameters::flux(time_pt()->time()) << " " << std::endl;

  cout << "Doced solution for step " << doc_info.number() << std::endl
       << std::endl
       << std::endl;
...
int main(int argc, char* argv[])
{
  ...
  else
  {
    ...
    oomph_info << "Enter none (for default) or one (namely the case id"
               << std::endl;
    oomph_info << "which should be one of: FSI1, FSI2, FSI3, CSM1" << std::endl;
  }
