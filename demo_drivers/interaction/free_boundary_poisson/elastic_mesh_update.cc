...
DeformableFishPoissonProblem<ELEMENT>::DeformableFishPoissonProblem()
{
  ...
  // Doc
  cout << std::endl
       << "Control node is located at: " << Doc_node_pt->x(0) << " "
       << Doc_node_pt->x(1) << std::endl
       << std::endl;
  ...
  // Loop over the elements in the mesh to set Solid parameters/function
  // pointers
  for (unsigned i = 0; i < n_element; i++)
  {
