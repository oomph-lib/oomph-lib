...
namespace Global_Parameters
{
  ...
  std::complex<double> axisym_coefficient()
  {
    ...
    MapleGenVar3 =
      (-2.0 / (2.0 + 2.0 * Nu) * 1.0 +
       2.0 / (2.0 + 2.0 * Nu) * 1.0 * H_coating) /
      (Nu / (1.0 + Nu) / (1.0 - 2.0 * Nu) + 2.0 / (2.0 + 2.0 * Nu) -
       2.0 / (2.0 + 2.0 * Nu) * H_coating +
       1 / (2.0 + 2.0 * Nu) * H_coating * H_coating) /
      2.0;
    MapleGenVar5 =
      -1 /
      (Nu / (1.0 + Nu) / (1.0 - 2.0 * Nu) + 2.0 / (2.0 + 2.0 * Nu) -
    ...
    MapleGenVar6 =
      ...
      (-(-2.0 / (2.0 + 2.0 * Nu) * 1.0 +
       ...
       (2.0 / (2.0 + 2.0 * Nu) * 1.0 -
        ...
        2.0 * 1.0 * H_coating * Nu / (1.0 + Nu) / (1.0 - 2.0 * Nu)) /
         (Nu / (1.0 + Nu) / (1.0 - 2.0 * Nu) + 2.0 / (2.0 + 2.0 * Nu) -
          2.0 / (2.0 + 2.0 * Nu) * H_coating +
          1 / (2.0 + 2.0 * Nu) * H_coating * H_coating) /
         2.0) /
      (HankelH1(1.0, sqrt(K_squared)) * sqrt(K_squared) -
       Q * HankelH1(0.0, sqrt(K_squared)) /
         (Nu / (1.0 + Nu) / (1.0 - 2.0 * Nu) + 2.0 / (2.0 + 2.0 * Nu) -
          2.0 / (2.0 + 2.0 * Nu) * H_coating +
          1 / (2.0 + 2.0 * Nu) * H_coating * H_coating) /
         2.0 +
       Q * HankelH1(0.0, sqrt(K_squared)) *
         (1.0 - 2.0 * H_coating + H_coating * H_coating) /
         (Nu / (1.0 + Nu) / (1.0 - 2.0 * Nu) + 2.0 / (2.0 + 2.0 * Nu) -
          2.0 / (2.0 + 2.0 * Nu) * H_coating +
          1 / (2.0 + 2.0 * Nu) * H_coating * H_coating) /
         2.0);
  ...
  double exact_axisym_radiated_power()
  {
    ...
    double power =
      sqrt(K_squared) * 0.5 * r * 2.0 * MathematicalConstants::Pi *
...
CoatedDiskProblem<ELASTICITY_ELEMENT, HELMHOLTZ_ELEMENT>::CoatedDiskProblem()
{
  ...
  // Build solid mesh
  Solid_mesh_pt = new RefineableTwoDAnnularMesh<ELASTICITY_ELEMENT>(
    periodic,
    azimuthal_fraction_of_coating,
    ntheta_solid,
    nr_solid,
    a,
    Global_Parameters::H_coating);
  ...
  // Build mesh
  Helmholtz_mesh_pt = new RefineableTwoDAnnularMesh<HELMHOLTZ_ELEMENT>(
    periodic,
    azimuthal_fraction_of_coating,
    ntheta_helmholtz,
    nr_helmholtz,
  ...
  Helmholtz_outer_boundary_mesh_pt = new HelmholtzDtNMesh<HELMHOLTZ_ELEMENT>(
...
void CoatedDiskProblem<ELASTICITY_ELEMENT, HELMHOLTZ_ELEMENT>::doc_solution()
{
  ...
  Trace_file << Global_Parameters::Q << " " << Global_Parameters::K_squared
             ...
             << Global_Parameters::Omega_sq << " " << power << " "
             << Global_Parameters::exact_axisym_radiated_power() << " "
             << std::endl;
  ...
  //------------------------------------
  sprintf(filename,
          "%s/exact_helmholtz_soln%i.dat",
          Doc_info.directory().c_str(),
          Doc_info.number());
  some_file.open(filename);
  Helmholtz_mesh_pt->output_fct(
    some_file, n_plot, Global_Parameters::exact_axisym_potential);
  some_file.close();
