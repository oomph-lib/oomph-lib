...
namespace Global_Parameters
{
                     ...
                     Vector<std::complex<double>>& traction)
  {
    ...
    double magnitude =
      exp(-Alpha * pow(phi - 0.25 * MathematicalConstants::Pi, 2));

    unsigned dim = traction.size();
    for (unsigned i = 0; i < dim; i++)
    {
      traction[i] =
        complex<double>(-magnitude * P * n[i], magnitude * P * n[i]);
    }
...
CoatedDiskProblem<ELASTICITY_ELEMENT, HELMHOLTZ_ELEMENT>::CoatedDiskProblem()
{
  ...
  curvilinear_boundary_pt.push_back(new TriangleMeshCurviLine(
  ...
  boundary_id = curvilinear_boundary_pt.size();
  curvilinear_boundary_pt.push_back(new TriangleMeshCurviLine(
    upper_inner_boundary_pt, zeta_start, zeta_end, nsegment, boundary_id));
  ...
  nsegment = 1;
  boundary_id = curvilinear_boundary_pt.size();
  upper_inward_rib_curviline_pt = new TriangleMeshCurviLine(
  ...
  nsegment = 1;
  boundary_id = curvilinear_boundary_pt.size();
  lower_inward_rib_curviline_pt = new TriangleMeshCurviLine(
  ...
  curvilinear_boundary_pt.push_back(
    new TriangleMeshCurviLine(lower_inner_boundary_circle_pt,
                              zeta_start,
                              zeta_end,
                              nsegment,
                              boundary_id));
  ...
  //--------------------------------
  closed_curve_pt = new TriangleMeshClosedCurve(curvilinear_boundary_pt);
  ...
  Helmholtz_mesh_pt =
    new RefineableTwoDAnnularMesh<HELMHOLTZ_ELEMENT>(periodic,
                                                     azimuthal_fraction,
                                                     ntheta_helmholtz,
                                                     nr_helmholtz,
                                                     a,
  ...
  Global_Parameters::E_pt[0] =
    new TimeHarmonicIsotropicElasticityTensor(Global_Parameters::Nu);
  Global_Parameters::E_pt[1] =
    new TimeHarmonicIsotropicElasticityTensor(Global_Parameters::Nu);
...
//========================================================================
template<class ELASTICITY_ELEMENT, class HELMHOLTZ_ELEMENT>
void CoatedDiskProblem<ELASTICITY_ELEMENT,
                       HELMHOLTZ_ELEMENT>::actions_before_adapt()
{
...
template<class ELASTICITY_ELEMENT, class HELMHOLTZ_ELEMENT>
void CoatedDiskProblem<ELASTICITY_ELEMENT,
                       HELMHOLTZ_ELEMENT>::actions_after_adapt()
{
  // Complete problem setup
  complete_problem_setup();
...
template<class ELASTICITY_ELEMENT, class HELMHOLTZ_ELEMENT>
void CoatedDiskProblem<ELASTICITY_ELEMENT,
                       HELMHOLTZ_ELEMENT>::complete_problem_setup()
{
...
template<class ELASTICITY_ELEMENT, class HELMHOLTZ_ELEMENT>
void CoatedDiskProblem<ELASTICITY_ELEMENT,
                       HELMHOLTZ_ELEMENT>::create_solid_traction_elements()
{
...
template<class ELASTICITY_ELEMENT, class HELMHOLTZ_ELEMENT>
void CoatedDiskProblem<ELASTICITY_ELEMENT,
                       HELMHOLTZ_ELEMENT>::create_fsi_traction_elements()
{
...
template<class ELASTICITY_ELEMENT, class HELMHOLTZ_ELEMENT>
void CoatedDiskProblem<ELASTICITY_ELEMENT,
                       HELMHOLTZ_ELEMENT>::create_helmholtz_fsi_flux_elements()
{
                       ...
                       HELMHOLTZ_ELEMENT>::create_helmholtz_ABC_elements()
{
  ...
  for (unsigned e = 0; e < n_element; e++)
  {
    ...
    HelmholtzAbsorbingBCElement<HELMHOLTZ_ELEMENT>* flux_element_pt =
      new HelmholtzAbsorbingBCElement<HELMHOLTZ_ELEMENT>(bulk_elem_pt,
                                                         face_index);
...
template<class ELASTICITY_ELEMENT, class HELMHOLTZ_ELEMENT>
void CoatedDiskProblem<ELASTICITY_ELEMENT,
                       HELMHOLTZ_ELEMENT>::setup_interaction()
{
  ...
  Multi_domain_functions::
    setup_bulk_elements_adjacent_to_face_mesh<ELASTICITY_ELEMENT, 2>(
...
void CoatedDiskProblem<ELASTICITY_ELEMENT, HELMHOLTZ_ELEMENT>::doc_solution()
{
  ...
  some_file.close();
  oomph_info << "Step: " << Doc_info.number() << ": Q=" << Global_Parameters::Q
             << "\n"
  ...
  // Write trace file
  Trace_file << Global_Parameters::Q << " " << Global_Parameters::K_squared
             << " " << Global_Parameters::Density_ratio[0] << " "
             << Global_Parameters::Density_ratio[1] << " "
             << Global_Parameters::Omega_sq[0] << " "
             << Global_Parameters::Omega_sq[1] << " " << power << " "
             << std::endl;

  std::ostringstream case_string;
  case_string << "TEXT X=10,Y=90, T=\"Q=" << Global_Parameters::Q
              << ",  k<sup>2</sup> = " << Global_Parameters::K_squared
              << ",  density ratio = " << Global_Parameters::Density_ratio[0]
              << " and " << Global_Parameters::Density_ratio[1]
              << ",  omega_sq = " << Global_Parameters::Omega_sq[0] << " and "
  ...
  for (unsigned i = 0; i < nstep; i++)
  {
    ...
    some_file.open(filename);
    double phi =
      2.0 * MathematicalConstants::Pi * double(i) / double(nstep - 1);
    unsigned nelem = Helmholtz_mesh_pt->nelement();
    for (unsigned e = 0; e < nelem; e++)
    {
      HELMHOLTZ_ELEMENT* el_pt =
        dynamic_cast<HELMHOLTZ_ELEMENT*>(Helmholtz_mesh_pt->element_pt(e));
