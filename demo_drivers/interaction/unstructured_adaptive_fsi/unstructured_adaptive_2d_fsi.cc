...
public:
  ...
  void actions_after_adapt()
  {
    ...
    // pointers to the meshes. The interaction boundary are boundaries 0, 1 and
    // 2 of the 2D fluid mesh.
  ...
  void output_strain_and_dissipation(std::ostream& trace)
  {
    ...
    trace << Global_Physical_Variables::Q << " " << dissipation << " "
...
private:
  ...
  void delete_fsi_traction_elements()
  {
    ...
    for (unsigned b = 0; b < 3; ++b)
    {
      ...
      delete Solid_fsi_boundary_pt[b];
      Solid_fsi_boundary_pt[b] = 0;
    }
  }
...
UnstructuredFSIProblem<FLUID_ELEMENT, SOLID_ELEMENT>::UnstructuredFSIProblem()
{
  ...
  TriangleMeshParameters triangle_mesh_parameters_solid(solid_closed_curve_pt);

  // Define the maximum element area
  triangle_mesh_parameters_solid.element_area() = uniform_element_area;

  // Create the mesh
  Solid_mesh_pt = new RefineableSolidTriangleMesh<SOLID_ELEMENT>(
  ...
  // Use the TriangleMeshParameters object for gathering all
  // the necessary arguments for the TriangleMesh object
  TriangleMeshParameters triangle_mesh_parameters_fluid(fluid_closed_curve_pt);

  // Define the maximum element area
  triangle_mesh_parameters_fluid.element_area() = uniform_element_area;

  // Create the mesh
  Fluid_mesh_pt = new RefineableSolidTriangleMesh<FLUID_ELEMENT>(
  ...
  {
    Traction_mesh_pt[m] = new SolidMesh;
  }
  this->create_fsi_traction_elements();
  ...
  for (unsigned m = 0; m < 3; m++)
  {
    Lagrange_multiplier_mesh_pt[m] = new SolidMesh;
  }
  this->create_lagrange_multiplier_elements();
