...
public:
  ...
  MacroElementNodeUpdateCollapsibleChannelMesh<ELEMENT>* bulk_mesh_pt()
  {
    ...
    return dynamic_cast<MacroElementNodeUpdateCollapsibleChannelMesh<ELEMENT>*>(
      Bulk_mesh_pt);
  }
  ...
  const double& ly)
{
  ...
  Bulk_mesh_pt = new MacroElementNodeUpdateCollapsibleChannelMesh<ELEMENT>(
    nup,
    ncollapsible,
    ndown,
    ny,
  ...
  // Build bulk (fluid) mesh
  Bulk_mesh_pt =
    new AlgebraicCollapsibleChannelMesh<ELEMENT>(nup,
                                                 ncollapsible,
                                                 ndown,
                                                 ny,
                                                 lup,
...
int main(int argc, char* argv[])
{
  ...
  // Build the problem with QCrouzeixRaviartElements
  FSICollapsibleChannelProblem<AlgebraicElement<QTaylorHoodElement<2>>> problem(
    nup, ncollapsible, ndown, ny, lup, lcollapsible, ldown, ly);

#else

  // Build the problem with QCrouzeixRaviartElements
  FSICollapsibleChannelProblem<AlgebraicElement<QCrouzeixRaviartElement<2>>>
    problem(nup, ncollapsible, ndown, ny, lup, lcollapsible, ldown, ly);
