...
class ElasticCollapsibleChannelMesh :
...
public:
  ...
  ElasticCollapsibleChannelMesh<ELEMENT>* bulk_mesh_pt()
  {
    ...
    return dynamic_cast<ElasticCollapsibleChannelMesh<ELEMENT>*>(Bulk_mesh_pt);
  }
  ...
  const double& ly)
{
  ...
  Bulk_mesh_pt =
    new ElasticCollapsibleChannelMesh<ELEMENT>(nup,
                                               ncollapsible,
                                               ndown,
                                               ny,
                                               lup,
                                                         ...
                                                         ofstream& trace_file)
{
  ...
  // Write trace file
  trace_file << time_pt()->time() << " " << Wall_node_pt->x(1) << " "
             << Left_node_pt->value(0) << " " << Right_node_pt->value(0) << " "
             << Global_Physical_Variables::P_ext << " " << std::endl;
...
//=======================================================================
template<class ELEMENT>
void FSICollapsibleChannelProblem<ELEMENT>::delete_traction_elements(
...
//=======================================================================
template<class ELEMENT>
void FSICollapsibleChannelProblem<
...
template<class ELEMENT>
void FSICollapsibleChannelProblem<
  ELEMENT>::delete_lagrange_multiplier_elements()
{
  // How many surface elements are in the surface mesh
...
int main(int argc, char* argv[])
{
  ...
  FSICollapsibleChannelProblem<
    PseudoSolidNodeUpdateElement<QTaylorHoodElement<2>, QPVDElement<2, 3>>>
    problem(nup, ncollapsible, ndown, ny, lup, lcollapsible, ldown, ly);

#else

  // Build the problem with QCrouzeixRaviartElements
  FSICollapsibleChannelProblem<
    PseudoSolidNodeUpdateElement<QCrouzeixRaviartElement<2>, QPVDElement<2, 3>>>
    problem(nup, ncollapsible, ndown, ny, lup, lcollapsible, ldown, ly);
