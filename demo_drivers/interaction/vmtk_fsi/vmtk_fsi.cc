...
UnstructuredFSIProblem<FLUID_ELEMENT, SOLID_ELEMENT>::UnstructuredFSIProblem()
{
  ...
  Fluid_mesh_pt = new MyFluidTetMesh<FLUID_ELEMENT>(
    node_file_name, element_file_name, face_file_name, split_corner_elements);
  ...
  Solid_mesh_pt = new MySolidTetgenMesh<SOLID_ELEMENT>(
  ...
  for (unsigned i = 0; i < nbound; i++)
  {
    ...
    for (unsigned inod = 0; inod < num_nod; inod++)
    {
      ...
      if (is_in_or_outflow_node)
      {
        ...
        pinned_file << nod_pt->x(0) << " " << nod_pt->x(1) << " "
                    << nod_pt->x(2) << endl;
      }
    }
...
//=======================================================================
template<class FLUID_ELEMENT, class SOLID_ELEMENT>
void UnstructuredFSIProblem<FLUID_ELEMENT,
                            SOLID_ELEMENT>::create_fsi_traction_elements()
{
