...
void run_navier_stokes_outflow()
{
  ...
  womersley_impedance_tube_pt->setup(
    &GlobalPhysicalParameters::Alpha_sq, dt, q_initial);

  // Output Womersley solution
  womersley_impedance_tube_pt->womersley_problem_pt()->doc_solution(
    doc_info, trace_file, xi_hi[0]);
  ...
  for (unsigned istep = 0; istep < nstep; istep++)
  {
    ...
    for (unsigned j = 0; j < nnod; j++)
    {
      ...
      double y = nod_pt->x(1);
      double time =
        womersley_impedance_tube_pt->womersley_problem_pt()->time_pt()->time();
      nod_pt->set_value(
        2, (1.0 - x * x - y * y) * sin(2.0 * MathematicalConstants::Pi * time));
    }
...
void run_impedance_tube()
{
  ...
  womersley_impedance_tube_pt->setup(
    &GlobalPhysicalParameters::Alpha_sq,
    dt,
    GlobalPhysicalParameters::Prescribed_volume_flux);

  // Output solution
  womersley_impedance_tube_pt->womersley_problem_pt()->doc_solution(doc_info,
                                                                    trace_file);
...
void run_prescribed_flux()
{
  ...
  // timestepper)
  WomersleyProblem<QWomersleyElement<2, 4>, 2> problem(
    &GlobalPhysicalParameters::Alpha_sq,
    &GlobalPhysicalParameters::Prescribed_volume_flux,
    time_stepper_pt,
    my_mesh_pt);
...
void run_prescribed_pressure_gradient()
{
  ...
  problem.pressure_gradient_data_pt()->set_value(
    0, GlobalPhysicalParameters::prescribed_pressure_gradient(0.0));
