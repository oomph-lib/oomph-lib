...
class CollapsibleChannelProblem : public Problem
{
 ...
 void unsteady_run(string directory_for_data, bool validation_run=false);
...
void CollapsibleChannelProblem<ELEMENT>::actions_before_newton_solve()
{
 ...
 // For flux control or impedance tube set parallel flow on the outlet
 if ((Outflow_type==0) || (Outflow_type==1))
  {
...
void CollapsibleChannelProblem<ELEMENT>::setup_outflow_flux_control_elements()
{
 ...
 NetFluxControlElement<ELEMENT>* flux_control_el_pt = 
  new NetFluxControlElement<ELEMENT>(
...
void CollapsibleChannelProblem<ELEMENT>::setup_outflow_impedance_elements()
{
 ...
 for (unsigned e=0;e<n_bound_el;e++)
  { 
   ...
   // Build the corresponding prescribed traction element
   NavierStokesImpedanceTractionElement<ELEMENT,
    QWomersleyElement<1,3>,1>* traction_element_pt 
    = new NavierStokesImpedanceTractionElement<ELEMENT,
    QWomersleyElement<1,3>,1>(bulk_elem_pt,face_index);
...
unsteady_run(string directory_for_data, bool validation_run)
{ 
...
int main(int argc, char *argv[])
{
 ...
 if (solver==0)
  {
   ...
   iterative_solver_pt->max_iter() = 100;
 ...
 // run the problem
 problem.unsteady_run(directory_for_data, validation_run);
