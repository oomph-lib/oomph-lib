...
namespace StokesFlowExactWind
{
  ...
  void get_wind(const Vector<double>& x, Vector<double>& wind)
  {
    ...
    double u_r_sph =
      -(-1.0 + 1.5 / (r_sc)-0.5 / (r_sc * r_sc * r_sc)) * cos(theta);
    double u_theta_sph =
      -(1.0 - 0.75 / (r_sc)-0.25 / (r_sc * r_sc * r_sc)) * sin(theta);
  ...
  GeomObject* cylinder_pt, const double& radius, const double& length)
{
  ...
  // Build mesh
  Problem::mesh_pt() = new RefineableHalfRectangleWithHoleMesh<ELEMENT>(
    cylinder_pt, radius, length, 4.0, 2, 4.0, 2, 1.0, 2);
...
int main()
{
  ...
  // Create Problem
  FlowAroundHalfCylinderProblem<RefineableBuoyantQAxisymCrouzeixRaviartElement>
  ...
  for (unsigned i = 0; i < 2; i++)
  {
    ...
    trace << Global_Parameters::Re << " " << Global_Parameters::Pr << " "
          << Global_Parameters::Pe << " " << drag[0] << " " << drag[1] << " "
