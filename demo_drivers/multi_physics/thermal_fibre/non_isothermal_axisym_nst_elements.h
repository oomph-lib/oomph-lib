...
/// existing classes.
/// The QSteadyAxisymAdvectionDiffusionElement<Nnode1d> with bi-quadratic
/// interpolation for the scalar variable (temperature) and
/// QCrouzeixRaviartElement which solves the Navier--Stokes equations using
/// bi-quadratic interpolation for the velocities and a discontinuous bi-linear
/// interpolation for the pressure. Note that we are free to choose the order in
/// which we store the variables at the nodes. In this case we choose to store
/// the variables in the order fluid velocities followed by temperature. We
/// must, therefore, overload the function
/// AxisymAdvectionDiffusionEquations::u_index_axisym_adv_diff() to indicate
/// that the temperature is stored at the 3-th position not the 0-th. We do not
...
public:
  ...
  void fill_in_contribution_to_residuals(Vector<double>& residuals)
  {
    ...
    SteadyAxisymAdvectionDiffusionEquations::fill_in_contribution_to_residuals(
      residuals);
  }
...
//-----------------------------------------------------------------------
                                        ...
                                        DenseMatrix<double>& jacobian)
  {
    ...
    SteadyAxisymAdvectionDiffusionEquations::fill_in_contribution_to_jacobian(
      residuals, jacobian);
        ...
        if (local_eqn >= 0)
        {
          ...
          for (unsigned l2 = 0; l2 < n_node; l2++)
          {
            ...
            for (unsigned i2 = 0; i2 < 2; i2++)
            {
              ...
              if (local_unknown >= 0)
              {
                // Add the contribution to the jacobian matrix (It is necessary
                // multiply by r)
...
namespace oomph
{
  ...
  template<>
  class FaceGeometry<
    FaceGeometry<NonIsothermalAxisymmetricQCrouzeixRaviartElement>> :
