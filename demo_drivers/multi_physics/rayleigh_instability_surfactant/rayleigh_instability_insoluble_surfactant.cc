...
namespace oomph
{
  ...
  protected:
    ...
    double dcdt_surface(const unsigned &l) const
    {
      ...
      if (time_stepper_pt->type() != "Steady")
      {
        ...
        {
          dcdt +=
            time_stepper_pt->weight(1, t) * this->nodal_value(t, l, c_index);
        }
      }
      return dcdt;
    }
                                          ...
                                          DenseMatrix<double> &jacobian)
    {
      ...
      SpineElement<FaceGeometry<ELEMENT>>::fill_in_jacobian_from_geometric_data(
        jacobian);
    }
      ...
      const double &J)
    {
      ...
      {
        d2xds[i] = this->nodal_position(0, i) + this->nodal_position(2, i) -
                   2.0 * this->nodal_position(1, i);
      }
      ...
      {
        k1 +=
          (d2xds[i] / (J_raw * J_raw) - interpolated_tangent[i] *
                                          (interpolated_tangent[0] * d2xds[0] +
                                           interpolated_tangent[1] * d2xds[1]) /
                                          (J_raw * J_raw * J_raw * J_raw)) *
          interpolated_n[i];
      }
      ...
      for (unsigned l = 0; l < n_node; l++)
      {
        ...
        if (local_eqn >= 0)
        {
          ...
          if (Integrated_curvature)
          {
            for (unsigned i = 0; i < 2; i++)
            {
              tmp += (interpolated_u[i] - mesh_velocity[i]) *
                     interpolated_tangent[i];
            }
            // First Advection term
            residuals[local_eqn] +=
              tmp * interpolated_dCds * psif(l) * r * W / J_raw;
            // Additional term from axisymmetric formulation
            residuals[local_eqn] +=
              interpolated_C * interpolated_u[0] * psif(l) * W * J_raw;
            // Second Advection term
            residuals[local_eqn] +=
              interpolated_C *
              (interpolated_duds[0] * interpolated_tangent[0] +
               interpolated_duds[1] * interpolated_tangent[1]) *
              r * W * psif(l) / J_raw;
          }
          ...
          else
          {
            ...
            residuals[local_eqn] -= interpolated_C * (k1 + k2) * psif(l) * r *
                                    W * J_raw *
            ...
            residuals[local_eqn] -=
              ...
              (interpolated_tangent[0] * interpolated_u[0] +
               interpolated_tangent[1] * interpolated_u[1]) *
              dpsifds(l, 0) * r * W / J_raw;
          }

          // Diffusion term
          residuals[local_eqn] +=
            (1.0 / Pe_s) * interpolated_dCds * dpsifds(l, 0) * r * W / J_raw;
            ...
            for (unsigned l2 = 0; l2 < n_node; l2++)
            {
              ...
              if (local_unknown >= 0)
              {
                jacobian(local_eqn, local_unknown) +=
                  time_stepper_pt->weight(1, 0) * psif(l2) * psif(l) * r * W *
                  J_raw;

                if (Integrated_curvature)
                {
                  jacobian(local_eqn, local_unknown) +=
                    ((interpolated_u[0] - mesh_velocity[0]) *
                       interpolated_tangent[0] +
                     (interpolated_u[1] - mesh_velocity[1]) *
                       interpolated_tangent[1]) *
                    dpsifds(l2, 0) * psif(l) * r * W / J_raw;

                  jacobian(local_eqn, local_unknown) +=
                    psif(l2) * interpolated_u[0] * psif(l) * W * J_raw;

                  jacobian(local_eqn, local_unknown) +=
                    psif(l2) *
                    (interpolated_tangent[0] * interpolated_duds[0] +
                     interpolated_tangent[1] * interpolated_duds[1]) *
                    psif(l) * r * W / J_raw;
                }
                else
                {
                  jacobian(local_eqn, local_unknown) -=
                    (mesh_velocity[0] * interpolated_tangent[0] +
                     mesh_velocity[1] * interpolated_tangent[1]) *
                    dpsifds(l2, 0) * psif(l) * r * W / J_raw;

                  jacobian(local_eqn, local_unknown) -=
                    psif(l2) * (k1 + k2) * psif(l) * r * W * J_raw *
                    (interpolated_u[0] * interpolated_n[0] +
                     interpolated_u[1] * interpolated_n[1]);
                ...
                jacobian(local_eqn, local_unknown) +=
                  (1.0 / Pe_s) * dpsifds(l2, 0) * dpsifds(l, 0) * r * W / J_raw;
              }
              ...
              for (unsigned i2 = 0; i2 < ndim; i2++)
              {
                ...
                {
                  // jacobian(local_eqn,local_unknown) +=
                  // time_stepper_pt->weight(1,0)*psif(l2)*PeSt_s*psif(l)*r*W*J_raw;
                  if (Integrated_curvature)
                  {
                    jacobian(local_eqn, local_unknown) +=
                      interpolated_dCds * psif(l2) * interpolated_tangent[i2] *
                      psif(l) * r * W / J_raw;

                    if (i2 == 0)
                    {
                      jacobian(local_eqn, local_unknown) +=
                        interpolated_C * psif(l2) * W * J_raw;
                    }
                  }
                  else
                  {
                    ...
                    jacobian(local_eqn, local_unknown) -=
                      interpolated_C * interpolated_tangent[i2] * psif(l2) *
                      dpsifds(l, 0) * r * W / J_raw;
                  }

                  jacobian(local_eqn, local_unknown) +=
                    interpolated_C * interpolated_tangent[i2] * dpsifds(l2, 0) *
                    psif(l) * r * W / J_raw;
                }
              }
            }
  ...
  public:
    ...
    void output(std::ostream &outfile, const unsigned &n_plot)
    {
      ...
      for (unsigned l = 0; l < n_plot; l++)
      {
        ...
        // Tangent
        double t_vel = (this->interpolated_u(s, 0) * interpolated_tangent[0] +
                        this->interpolated_u(s, 1) * interpolated_tangent[1]) /
                       sqrt(interpolated_tangent[0] * interpolated_tangent[0] +
  ...
  template<class ELEMENT>
  double SpineAxisymmetricMarangoniSurfactantFluidInterfaceElement<
    ELEMENT>::Default_Physical_Constant_Value = 1.0;
  ...
  void deform_free_surface(const double &epsilon)
  {
    ...
    for (unsigned i = 0; i < n_spine; i++)
    {
      ...
      Bulk_mesh_pt->spine_pt(i)->height() =
        ...
        (1.0 + +epsilon * cos(Global_Physical_Variables::Alpha * z_value));
...

{
  ...
  Bulk_mesh_pt = new MyHorizontalSingleLayerSpineMesh<ELEMENT>(
    n_r, n_z, 1.0, l_z, time_stepper_pt());
...
void InterfaceProblem<ELEMENT, TIMESTEPPER>::doc_solution(DocInfo &doc_info)
{
  ...
  Trace_file << time_pt()->time() << " " << Bulk_mesh_pt->spine_pt(0)->height()
  ...
  string file_name =
    "soln" + StringConversion::to_string(doc_info.number()) + ".vtu";
  ParaviewHelper::write_pvd_information(
    Global_Physical_Variables::Pvd_file, file_name, t);
...
int main(int argc, char *argv[])
{
  ...
  // Height of domain
  const double l_z =
    MathematicalConstants::Pi / Global_Physical_Variables::Alpha;
