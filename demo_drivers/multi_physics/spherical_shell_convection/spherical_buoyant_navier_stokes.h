  ...
  public:
    ...
    void output(std::ostream& outfile, const unsigned& nplot)
    {
      ...
      for (unsigned iplot = 0; iplot < num_plot_points; iplot++)
      {
        ...
        outfile << r * sin(theta) << " " << r * cos(theta) << " " << r << " "
                << theta << " ";
                                 ...
                                 Vector<double>& values)
    {
      ...
      RefineableQSphericalCrouzeixRaviartElement::get_interpolated_values(
        s, nst_values);
      ...
      RefineableQSphericalAdvectionDiffusionElement<3>::get_interpolated_values(
        s, advection_values);
    ...
    {
      RefineableQSphericalAdvectionDiffusionElement<3>::rebuild_from_sons(
        mesh_pt);
    ...
    void get_Z2_flux(const Vector<double>& s, Vector<double>& flux)
    {
      ...
      RefineableQSphericalAdvectionDiffusionElement<3>::get_Z2_flux(s,
                                                                    temp_flux);
    ...
    void get_body_force_spherical_nst(const double& time,
                                      const unsigned& ipt,
                                      const Vector<double>& s,
                                      const Vector<double>& x,
      ...
      Vector<double>& residuals, DenseMatrix<double>& jacobian)
    {
      ...
      for (unsigned ipt = 0; ipt < n_intpt; ipt++)
      {
        ...
        double J = this->dshape_and_dtest_eulerian_at_knot_spherical_nst(
          ...
          for (unsigned m = 0; m < n_master; m++)
          {
            ...
            for (unsigned i = 0; i < 3; i++)
            {
              ...
              if (is_node_hanging)
              {
                ...
                local_eqn = this->local_hang_eqn(
                  hang_info_pt->master_node_pt(m), u_nodal_spherical_nst[i]);
              }
              else
              {
                ...
                for (unsigned l2 = 0; l2 < n_node; l2++)
                {
                  ...
                  for (unsigned m2 = 0; m2 < n_master2; m2++)
                  {
                    ...
                    else
                    {
                      ...
                      local_unknown =
                        this->nodal_local_eqn(l2, u_nodal_spherical_adv_diff);
                    ...
                    if (local_unknown >= 0)
                    {
                      ...
                      jacobian(local_eqn, local_unknown) +=
                        -gravity[i] * psif(l2) * Ra * testf(l) * r2 *
            ...
            //------------------------------------------------------
            {
              ...
              if (is_node_hanging)
              {
                // Get the equation number from the master node
                local_eqn = this->local_hang_eqn(
                  hang_info_pt->master_node_pt(m), u_nodal_spherical_adv_diff);
              }
              else
              {
                // Local equation number
                local_eqn =
                  this->nodal_local_eqn(l, u_nodal_spherical_adv_diff);
              }
                  ...
                  for (unsigned m2 = 0; m2 < n_master2; m2++)
                  {
                    ...
                    for (unsigned i2 = 0; i2 < 2; i2++)
                    {
                      ...
                      else
                      {
                        // The local unknown number comes from the node
                        local_unknown =
                          this->nodal_local_eqn(l2, u_nodal_spherical_nst[i2]);
                      }
                      ...
                      if (local_unknown >= 0)
                      {
                        ...
                        if (i2 == 0)
                        {
                          ...
                          jacobian(local_eqn, local_unknown) -=
                            Pe * psif(l2) *
                            interpolated_du_spherical_adv_diff_dx[i2] *
                            testf(l) * r2 * sin_theta * W * hang_weight *
                            hang_weight2;
                        }
                        if (i2 == 1)
                        {
                          ...
                          jacobian(local_eqn, local_unknown) -=
                            Pe * psif(l2) *
                            interpolated_du_spherical_adv_diff_dx[i2] *
                            testf(l) * r * sin_theta * W * hang_weight *
                            hang_weight2;
                        }
                      }
                    }
