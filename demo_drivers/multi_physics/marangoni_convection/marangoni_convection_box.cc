...
namespace oomph
{
  ...
  /// one free surface. In order to implement the constraint in total
  /// generality, one would need to introduce surface elements all round the
  /// mesh and integrate x.n along them. This is not necessary in the special
  /// case described above, because the integrals along the three straight edges
  ...
  public:
    ...
    void set_traded_pressure_data(Data *traded_pressure_data_pt)
    {
      ...
      if (traded_pressure_data_pt->nvalue() != 1)
      {
        ...
        std::string function_name = "SpineVolumConstraintPointElement::\n";
  ...
  template<class ELEMENT>
  double FixedVolumeSpineLineMarangoniFluidInterfaceElement<
...
public:
  ...
  void switch_boundary_conditions()
  {
    ...
    std::cout << "Preparing to timestep: " << assign_eqn_numbers() << "\n";
  }
...
int main(int argc, char **argv)
{
  ...
  ConvectionProblem<SpineElement<BuoyantQCrouzeixRaviartElement<2>>,
                    FixedVolumeSpineLineMarangoniFluidInterfaceElement<
                      SpineElement<BuoyantQCrouzeixRaviartElement<2>>>>
    problem;
