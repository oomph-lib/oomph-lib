 ...
 if(time <= 0.0)
  {
   ...
   for(unsigned n=0;n<n_node;n++)
    {
     mesh_pt()->boundary_node_pt(ibound,n)->set_value(4,1.0);
    }
  }
...
{ 
 ...
 for(unsigned e=0;e<n_element;e++)
  {
   mesh_pt()->bulk_element_pt(e)->output(some_file,npts);
  }
 some_file.close();
 ...
 for(unsigned i=0;i<n_interface;i++)
  {
   mesh_pt()->interface_element_pt(i)->output(some_file,npts);
  }
 some_file.close();

 trace << time_pt()->time() << " " 
       << mesh_pt()->boundary_node_pt(2,8)->value(2) << " "
       << mesh_pt()->boundary_node_pt(2,8)->value(3) << " "
       << mesh_pt()->boundary_node_pt(2,8)->value(4) << std::endl;
