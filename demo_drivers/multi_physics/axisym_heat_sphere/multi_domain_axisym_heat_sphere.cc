...
public:
     ...
     for (unsigned inod=0;inod<num_nod;inod++)
      {
       ...
       //Plug flow on inlet and side boundary
       if(ibound==0)
        {
         nst_mesh_pt()->boundary_node_pt(ibound,inod)->set_value(0,0.0);
         nst_mesh_pt()->boundary_node_pt(ibound,inod)->set_value(1,1.0);
        }
 ...
 AdaptiveHalfRectangleWithHoleMesh<NST_ELEMENT>* nst_mesh_pt() 
  {
   return dynamic_cast<AdaptiveHalfRectangleWithHoleMesh<NST_ELEMENT>*>
    (Nst_mesh_pt);
  }

 /// Access function for the specific mesh
 AdaptiveHalfRectangleWithHoleMesh<AD_ELEMENT>* adv_diff_mesh_pt() 
  {
   return dynamic_cast<AdaptiveHalfRectangleWithHoleMesh<AD_ELEMENT>*>
    (Adv_diff_mesh_pt);
  }
...
{ 
 ...
 // Build meshes
 Nst_mesh_pt =
  new AdaptiveHalfRectangleWithHoleMesh<NST_ELEMENT>(cylinder_pt,radius,length,
                                                     49.0,2,49.0,2,1.0,2);
 Adv_diff_mesh_pt =
  new AdaptiveHalfRectangleWithHoleMesh<AD_ELEMENT>(cylinder_pt,radius,length,
                                                    49.0,2,49.0,2,1.0,2);
 ...
 for(unsigned i=0;i<2;i++)
  {
   ...
   //Open an output file
   sprintf(filename,"vel_soln_Re%g.dat", Global_Parameters::Re);
   //Doc result
   ofstream outfile(filename);
   problem.nst_mesh_pt()->output(outfile,5);
   outfile.close();
   
   sprintf(filename,"conc_soln_Re%g.dat", Global_Parameters::Re);
