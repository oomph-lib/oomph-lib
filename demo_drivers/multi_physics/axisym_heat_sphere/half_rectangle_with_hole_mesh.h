...
namespace oomph
{
      ...
      else
      {
        ...
        if (m < Nup + Ndown + 3)
        {
          ...
          switch (direction)
          {
            case N:
              linear_interpolate(
                Down[0][m_mod + 1], Down[1][m_mod + 1], s[0], f);
              break;
        ...
        else if (m < Nup + Ndown + 3 + (Ncolumn - 1) * (Nup + Ndown + 1))
        {
          ...
          if (m_mod < Nup)
          {
            switch (direction)
            {
              case N:
                linear_interpolate(
                  Up[j_col][m_mod + 1], Up[j_col + 1][m_mod + 1], s[0], f);
                break;

              case S:
                linear_interpolate(
                  Up[j_col][m_mod], Up[j_col + 1][m_mod], s[0], f);
                break;

              case W:
                linear_interpolate(
                  Up[j_col][m_mod], Up[j_col][m_mod + 1], s[0], f);
                break;

              case E:
                linear_interpolate(
                  Up[j_col + 1][m_mod], Up[j_col + 1][m_mod + 1], s[0], f);
                break;
          ...
          else if (m_mod == Nup)
          {
            switch (direction)
            {
              ...
              case E:
                linear_interpolate(
                  Up[j_col + 1][Nup], Down[j_col + 1][0], s[0], f);
                break;
          ...
          else if (m_mod < Nup + Ndown + 1)
          {
            ...
            {
              case N:
                linear_interpolate(Down[j_col][m_mod2 + 1],
                                   Down[j_col + 1][m_mod2 + 1],
                                   s[0],
                                   f);
                break;
              ...
              case S:
                linear_interpolate(
                  Down[j_col][m_mod2], Down[j_col + 1][m_mod2], s[0], f);
                break;

              case W:
                linear_interpolate(
                  Down[j_col][m_mod2], Down[j_col][m_mod2 + 1], s[0], f);
                break;

              case E:
                linear_interpolate(Down[j_col + 1][m_mod2],
                                   Down[j_col + 1][m_mod2 + 1],
                                   s[0],
                                   f);
                break;
  ...
  public:
      ...
      TimeStepper *time_stepper_pt = &Mesh::Default_TimeStepper)
    {
      ...
      if (ncolumn > 1)
      {
        rhs_offset = nup + ndown + 3 + (ncolumn - 2) * (nup + ndown + 1);
      }
      ...
      for (unsigned j = 1; j < ncolumn; j++)
      {
        for (unsigned n = 1; n < np; n++)
        {
          ...
          nod_pt = finite_element_pt(lower_index + nup + ndown)
                     ->node_pt(np * (np - 1) + n);
          convert_to_boundary_node(nod_pt);
          add_boundary_node(2, nod_pt);
        }
  ...
  class RefineableHalfRectangleWithHoleMesh :
    public HalfRectangleWithHoleMesh<ELEMENT>,
