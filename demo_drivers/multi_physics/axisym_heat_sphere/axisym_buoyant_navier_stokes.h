...
namespace oomph
{
  ...
  public:
    ...
    {
      return (
        RefineableQAxisymAdvectionDiffusionElement<3>::required_nvalue(n) +
                                 ...
                                 Vector<double> &values)
    {
      ...
      RefineableAxisymmetricQCrouzeixRaviartElement::get_interpolated_values(
        s, nst_values);
      ...
      RefineableQAxisymAdvectionDiffusionElement<3>::get_interpolated_values(
        s, advection_values);
          ...
          for (unsigned m = 0; m < n_master; m++)
          {
            ...
            for (unsigned i = 0; i < 3; i++)
            {
              ...
              if (is_node_hanging)
              {
                ...
                local_eqn = this->local_hang_eqn(
                  hang_info_pt->master_node_pt(m), u_nodal_axi_nst[i]);
              }
              else
              {
                ...
                for (unsigned l2 = 0; l2 < n_node; l2++)
                {
                  ...
                  for (unsigned m2 = 0; m2 < n_master2; m2++)
                  {
                    ...
                    else
                    {
                      ...
                      local_unknown =
                        this->nodal_local_eqn(l2, u_nodal_axi_adv_diff);
                    ...
                    if (local_unknown >= 0)
                    {
                      ...
                      jacobian(local_eqn, local_unknown) +=
                        -gravity[i] * psif(l2) * Ra * testf(l) * r * W *
            ...
            //------------------------------------------------------
            {
              ...
              if (is_node_hanging)
              {
                // Get the equation number from the master node
                local_eqn = this->local_hang_eqn(
                  hang_info_pt->master_node_pt(m), u_nodal_axi_adv_diff);
              }
              else
              {
                // Local equation number
                  ...
                  for (unsigned m2 = 0; m2 < n_master2; m2++)
                  {
                    ...
                    for (unsigned i2 = 0; i2 < 2; i2++)
                    {
                      ...
                      else
                      {
                        // The local unknown number comes from the node
                        local_unknown =
                          this->nodal_local_eqn(l2, u_nodal_axi_nst[i2]);
                      }
                      ...
                      if (local_unknown >= 0)
                      {
                        ...
                        jacobian(local_eqn, local_unknown) -=
                          Pe * psif(l2) * interpolated_du_axi_adv_diff_dx[i2] *
                          testf(l) * r * W * hang_weight * hang_weight2;
                      }
                    }
                  }
                }
