  ...
  void deform_interface(const double& epsilon, const unsigned& n_periods)
  {
    ...
    for (unsigned i = 0; i < n_spine; i++)
    {
      ...
      Bulk_mesh_pt->spine_pt(i)->height() =
        ...
        epsilon * (cos(n_periods * MathematicalConstants::Pi *
                       (x_value - Global_Physical_Variables::L) /
  ...
  Surface_pinned(pin)
{
  ...
  Bulk_mesh_pt =
    new TwoLayerSpineMesh<ELEMENT>(n_x,
                                   n_y1,
                                   n_y2,
                                   lx,
                                   h1,
  ...
  for (unsigned i = 0; i < n_x; i++)
  {
    ...
    // If it's not periodic, we need to make point elements to balance the
    // surface tension
    if (!Control_Parameters::Periodic_BCs)
    {
  ...
  ofstream& trace)
{
  ...
  trace << time_pt()->time() << " " << monitor_node_pt->x(1) << " "
        << monitor_node_pt->value(2) << " " << monitor_node_pt->value(3) << " "
...
int main(int argc, char** argv)
{
  ...
  SurfactantProblem<
    SpineElement<Hijacked<DoubleBuoyantQCrouzeixRaviartElement<2>>>,
    SpineLineSolubleSurfactantTransportInterfaceElement<
      DoubleBuoyantQCrouzeixRaviartElement<2>>>
    problem;
