...
namespace oomph
{
  ...
  namespace Global_Physical_Variables
  {
    ...
    // Dimensionless position of interface (relative to a total domain height of
    // 1)
    double H0 = 0.2;
    ...
    /// on the viscous scale and so multiplying the normal stress balanced by
    /// the Reynolds number gives a term in the Capillary number only (Ca Re =
    /// We).
    ...
    Vector<double>& residuals, DenseMatrix<double>& jacobian, unsigned flag)
  {
    ...
    if (local_eqn >= 0)
    {
      ...
      if (flag)
      {
        ...
        // There is only one external datum so the first entry is the required
        // number
...
public:
  ...
  SlavePositionPointElement(Node* const& node_pt,
                            Node* const& slave_node_pt,
                            const unsigned& id = 0) :
  ...
  public virtual ElasticRefineableRectangularQuadMesh<ELEMENT>
{
  ...
  // Need to store the tree roots in the upper and lower layers, because the
  // numbers of elements will (of course) change
...
public:
      ...
      nx, ny1 + ny2, lx, h1 + h2, periodic_in_x, time_stepper_pt)
  {
    ...
    // Set the boundary coordinate to be the Lagrangian x coordinate the new
    // boundary
    ...
    // The default position is that the interface will be located half-way
    // between the boundaries, now we need to rescale.
    ...
    for (unsigned n = 0; n < n_node; ++n)
    {
      ...
      if (y < half_height)
      {
        nod_pt->x(1) = h1 * y / half_height;
      }
      else
      {
        nod_pt->x(1) = h1 + h2 * ((y - half_height) / half_height);
      }
    }
    ...
    for (unsigned e = 0; e < Nlower; e++)
    {
      this->Lower_tree_root_pt[e] =
        dynamic_cast<ELEMENT*>(this->element_pt(e))->tree_pt()->root_pt();
    }
    ...
    if (periodic_in_x)
    {
      ...
      {
        left_root_pt[i] = dynamic_cast<ELEMENT*>(this->element_pt(i * nx))
                            ->tree_pt()
                            ->root_pt();
        right_root_pt[i] =
...
public:
  ...
  void unpin_surface()
  {
    ...
    if (Surface_pinned)
    {
      ...
      // Unpin the interfacial surfactant concentrations and Lagrange
      // multipliers
      ...
      for (unsigned i = 0; i < n_interface; i++)
      {
        ...
        // Need to unpin the values of surfactant concentration and Lagrange
        // multiplier
      ...
      // the interface so we pin that at the left-hand-side
      {
        ...
        for (unsigned n = 0; n < n_boundary_node; ++n)
        {
          ...
          if ((!(nod_pt->is_on_boundary(0))) &&
              (!(nod_pt->is_on_boundary(2))) && (!(nod_pt->is_on_boundary(4))))
          {
            unsigned lm_index =
              dynamic_cast<BoundaryNodeBase*>(nod_pt)
      ...
      std::cout << "Surface unpinned to give " << assign_eqn_numbers()
                << " equation numbers\n";
    }
  }
  ...
  void actions_after_adapt()
  {
    ...
    Backed_up_surface_mesh_pt->project_onto_new_mesh(Surface_mesh_pt);
  ...
  void create_slave_position_elements()
  {
    ...
    // We also know that the nodes have already been made periodic so we simply
    // need to create the slave elements to match the vertical positions
    ...
    for (unsigned n = 0; n < n_boundary_node; ++n)
    {
      ...
      if (master_node_pt != 0)
      {
            ...
            !(master_node_pt->is_on_boundary(4)))
        {
          ...
          this->Slave_position_mesh_pt->add_element_pt(
            new SlavePositionPointElement(
              master_node_pt, nod_pt, Periodic_index));
        }
      }
    }
  }
  ...
  void deform_interface(const double& epsilon, const unsigned& n_periods)
  {
    ...
    for (unsigned n = 0; n < n_node; n++)
    {
      ...
      const double new_y_pos =
        current_y_pos + y_scale * epsilon *
  ...
  void interface_min_max(double& min, double& max)
  {
    ...
    if (n_interface > 0)
    {
      ...
      for (unsigned i = 0; i < n_interface; i++)
      {
        ...
        {
          std::cout
            << "Can't do max and min for elements that are not quadratic\n";
          return;
        }
        ...
        if (std::abs(y[0] - 2 * y[1] + y[2]) > 1.0e-10)
        {
          ...
          // quadratic basis functions (This shoudl really be inside the element
          // class)
          Vector<double> extreme_s(
            1, 0.5 * (y[0] - y[2]) / (y[0] - 2.0 * y[1] + y[2]));
          ...
          if (std::abs(extreme_s[0]) <= 1.0)
          {
            ...
            if (extreme_h > local_max)
            {
              local_max = extreme_h;
            }
            if (extreme_h < local_min)
            {
              local_min = extreme_h;
            }
          }
  ...
  Surface_pinned(pin), Periodic_index(50)
{
  ...
  Bulk_mesh_pt =
    new ElasticRefineableTwoLayerMesh<ELEMENT>(n_x,
                                               n_y1,
                                               n_y2,
                                               lx,
                                               h1,
  ...
  // Pin the positions of all nodes and the Lagrange multipliers if the surface
  // is pinned
  if (Surface_pinned)
  {
    unsigned n_node = Bulk_mesh_pt->nnode();
  ...
  for (unsigned ibound = 0; ibound < num_bound; ibound++)
  {
    ...
    if ((ibound == 0) || (ibound == 2))
    {
      ...
      {
        SolidNode* nod_pt =
          static_cast<SolidNode*>(Bulk_mesh_pt->boundary_node_pt(ibound, inod));
        nod_pt->pin(0);
        nod_pt->pin(1);
        nod_pt->pin_position(0);
        nod_pt->pin_position(1);
  ...
  const double& time)
{
  ...
  if (time <= 0.0)
  {
    ...
    const double C_init =
      Gamma_init / (Global_Physical_Variables::K_b * (1.0 - Gamma_init));
    ...
    for (unsigned e = 0; e < n_surface_element; ++e)
    {
      ...
      for (unsigned n = 0; n < n_el_node; ++n)
      {
        Surface_mesh_pt->finite_element_pt(e)->node_pt(n)->set_value(
          4, Gamma_init);
      }
    }
  ...
  ofstream& trace)
{
  ...
  sprintf(
    filename, "%s/int%i.dat", Doc_info.directory().c_str(), Doc_info.number());
  some_file.open(filename);
  ...
  trace << time_pt()->time() << " " << this->Monitor_node_pt->x(1) << " "
        << this->Monitor_node_pt->value(2) << " "
        << this->Monitor_node_pt->value(3) << " "
        << this->Monitor_node_pt->value(4) << " "
        << std::sqrt(this->l2_norm_of_height(Global_Physical_Variables::H0) /
                     (2.0 * Global_Physical_Variables::L))
        << " " << surface << " " << bulk << " " << micelle << " "
        << surface + (bulk + micelle) / (Global_Physical_Variables::Beta_b)
        << " " << min << " " << max
        << " "
        //<< " " << upper_area << " " << lower_area << " " << upper_area +
        // lower_area
        << std::endl;
...
template<class ELEMENT, class INTERFACE_ELEMENT>
double SurfactantProblem<ELEMENT,
                         INTERFACE_ELEMENT>::global_temporal_error_norm()
{
  // Temp
...
int main(int argc, char** argv)
{
  ...
  SurfactantProblem<Hijacked<RefineablePseudoSolidNodeUpdateElement<
                      RefineableDoubleBuoyantQCrouzeixRaviartElement<2>,
