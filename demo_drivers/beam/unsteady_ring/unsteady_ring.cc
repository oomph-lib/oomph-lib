...
void ElasticRingProblem<ELEMENT, TIMESTEPPER>::doc_solution(DocInfo& doc_info)
{
  ...
  Trace_file << time_pt()->time() << " "
             << trace_elem_pt->interpolated_x(s_trace, 1) << " " << global_pot
             << " " << global_kin << " " << global_pot + global_kin
  ...
  // Output solution
  sprintf(
    filename, "%s/ring%i.dat", doc_info.directory().c_str(), doc_info.number());
...

{
  ...
  dump_file << Validation_run_flag << " # Validation run flag" << std::endl;
...
void ElasticRingProblem<ELEMENT, TIMESTEPPER>::restart(ifstream& restart_file)
{
  ...
  Validation_run_flag = unsigned(atof(input_string.c_str()));
...
void ElasticRingProblem<ELEMENT, TIMESTEPPER>::set_initial_conditions()
{
  ...
  else
  {
    ...
    {
      cout << "Have opened " << CommandLineArgs::Argv[2] << " for restart. "
           << std::endl;
    }
    else
    {
...
void ElasticRingProblem<ELEMENT, TIMESTEPPER>::unsteady_run()
{
  ...
  {
    cout << "No command line arguments -- using defaults." << std::endl;
  }
  ...
  Global_Physical_Variables::Pcos =
    1.0e-4 * pow(Global_Physical_Variables::Alpha, 3);
