...
template<class ELEMENT>
RefineableAdvectionDiffusionPipeProblem<
  ELEMENT>::RefineableAdvectionDiffusionPipeProblem()
{
  // Add the timestepper
  ...
  Problem::mesh_pt() = new RefineableRectangularQuadMesh<ELEMENT>(
    n_r, n_z, l_r, l_z, this->time_stepper_pt());
  ...
  {
    mesh_pt()->boundary_node_pt(0, n)->make_periodic(
      mesh_pt()->boundary_node_pt(2, n));
  }
...
//========================================================================
template<class ELEMENT>
void RefineableAdvectionDiffusionPipeProblem<ELEMENT>::set_initial_condition()
{
  ...
  for (unsigned n = 0; n < n_node; n++)
  {
    ...
    double c =
      exp(-10 * r * r - 2.0 * (z - 0.5 * Global_Physical_Variables::Length) *
...
int main()
{
  ...
  RefineableAdvectionDiffusionPipeProblem<
    RefineableQGeneralisedAxisymAdvectionDiffusionElement<3>>
    problem;
