...
namespace GlobalFct
{
                     ...
                     Vector<double>& residuals)
  {
    ...
    for (unsigned j = 0; j < N_primary; j++)
    {
      ...
      residuals[displ_eqn] = Stiffness * u_combined -
                             Force *
                               (2.0 * j + 2.0 + alpha * N + alpha * N * N -
                                2.0 * alpha * j - 2.0 * alpha) /
                               2.0 +
                             fc;
                 ...
                 "--completely_smooth"))
      {
        ...
        double t0 = 768.0 / 77.0 / 0.3141592653589793E1 * phi -
                    2176.0 / 77.0 /
                      (0.3141592653589793E1 * 0.3141592653589793E1) * phi *
                      phi +
                    128.0 / 7.0 /
      ...
      // Single kink
      //------------
      else if (CommandLineArgs::command_line_flag_has_been_set("--single_kink"))
      {
      ...
      // Kuhn Tucker
      //------------
      else if (CommandLineArgs::command_line_flag_has_been_set("--kuhn_tucker"))
      {
  ...
  void set_spurious_solution(Vector<double>& unknowns)
  {
    ...
    oomph_info << "Residual for spurious solution: " << residuals[0] << " "
               << residuals[1] << "\n";
  }
...
int main(int argc, char** argv)
{
  ...
  {
    oomph_info << "\n\nSolving for force: " << GlobalFct::Force << std::endl
               << std::endl;
  ...
  oomph_info << "\nPossibly spurious solution: " << unknowns[0] << " "
             << unknowns[1] << "\n\n";
