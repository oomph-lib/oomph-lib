...
public:
  ...
  void actions_before_implicit_timestep()
  {
    ...
    ProblemParameters::Centre[1] =
      ProblemParameters::Y_c_max -
      amplitude * (1.0 + 0.05 * time) * 0.5 *
        (1.0 - cos(2.0 * MathematicalConstants::Pi * time));
...
private:
  ...
  void complete_problem_setup()
  {
    ...
    // hierher
    // if
    // (!CommandLineArgs::command_line_flag_has_been_set("--proper_elasticity"))
    //  {
...
ContactProblem<ELEMENT>::ContactProblem()
{
  ...
  for (unsigned j = 1; j < npt_contact - 1; j++)
  {
    ...
    contact_bound_coords[j][0] =
      x_ur - (x_ur - x_ll) * double(j) / double(npt_contact - 1);
