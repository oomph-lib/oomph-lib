...
namespace oomph
{
  ...
  public:
    ...
    double knot(const unsigned& i, const unsigned& j) const
    {
      if (i < Gauss<DIM, NPTS_1D>::nweight())
      {
        ...
        unsigned ii = i;
        return (lower +
                (0.5 * (1.0 + Gauss<DIM, NPTS_1D>::knot(ii, j)) * range));
      }
  ...
  public:
                     ...
                     bool& intersection) const
    {
      ...
      // Projection of vector from potential contact point to centre of
      // penetrator onto outer unit normal on potential contact point
    ...
    void output(std::ostream& outfile, const unsigned& nplot) const
    {
      ...
      {
        double phi =
          2.0 * MathematicalConstants::Pi * double(j) / double(nplot - 1);
        outfile << (*Centre_pt)[0] + Radius * cos(phi) << " "
                << (*Centre_pt)[1] + Radius * sin(phi) << std::endl;
      }
    }
                             ...
                             const Vector<Vector<double>>& polygon_vertex)
    {
      ...
      if (sqrt(d0 * d0 + d1 * d1) > 0.0)
      {
        ...
        junk << "First and last point of polygon don't coincide!\n"
             << "First point at: " << polygon_vertex[0][0] << " "
             << polygon_vertex[0][1] << "\n"
             << "Last point at: " << polygon_vertex[nvertex - 1][0] << " "
        ...
        if (point[1] > std::min(p1[1], p2[1]))
        {
          if (point[1] <= std::max(p1[1], p2[1]))
          {
            if (point[0] <= std::max(p1[0], p2[0]))
            {
              ...
              {
                double xintersect =
                  (point[1] - p1[1]) * (p2[0] - p1[0]) / (p2[1] - p1[1]) +
                  p1[0];
  ...
  public:
    ...
    TemplateFreeContactElementBase() :
      Traction_fct_pt(0),
      Use_isoparametric_flag_pt(0),
      Use_collocated_penetration_flag_pt(0),
  ...
  public:
      ...
      FaceGeometry<ELEMENT>(), FaceElement()
    {
      ...
      if (element_pt->dim() != 2)
      {
        // Issue a warning
        throw OomphLibError(
          "This element will almost certainly not work in non-2D problems, "
          "though it should be easy enough to upgrade... Volunteers?\n",
          OOMPH_CURRENT_FUNCTION,
          OOMPH_EXCEPTION_LOCATION);
      }
#endif

      // Always use piecewise Gauss -- it "over-integrates" for isoparametric
      // formulations but is likely to be better for discontinuous basis fcts.
      // Only implemented for 1,3 (1 is from surface of 2D bulk) (3 for
      // quadratic elements)
#ifdef PARANOID
      ...
      {
        // Issue a warning
        throw OomphLibError("Piecwise Gauss used here isn't appropriate for "
                            "non-3-node elements.\n",
                            OOMPH_CURRENT_FUNCTION,
                            OOMPH_EXCEPTION_LOCATION);
      }
      ...
      set_integration_scheme(
        new PiecewiseGauss<1, 3>(this->s_min(), this->s_max()));
    }
    ...
    // fiare: we are currently using FD, but this may be useful if that ever
    // changes some parts of the jacobian are known a priori hierher FD the lot
    // for now
    ...
    fill_in_contribution_to_residuals_surface_contact(Vector<double>& residuals)
  {
    ...
    Vector<double> local_residuals(n_dof, 0.0);
    {
      ...
      for (unsigned ipt = 0; ipt < n_intpt; ipt++)
      {
        ...
        // If we are using collocation for both, then we don't need to set up
        ...
        for (unsigned l = 0; l < n_node; l++)
        {
          ...
          interpolated_lambda_p +=
            this->node_pt(l)->value(first_index) * psi_p[l];
          ...
          for (unsigned k = 0; k < n_position_type; k++)
          {
            ...
            for (unsigned i = 0; i < n_dim; i++)
            {
              ...
              interpolated_x[i] +=
                this->nodal_position_gen(l, this->bulk_position_type(k), i) *
                psi(l, k);
        ...
        for (unsigned l = 0; l < n_node; l++)
        {
          ...
          for (unsigned k = 0; k < n_position_type; k++)
          {
            ...
            {
              local_eqn =
                this->position_local_eqn(l, this->bulk_position_type(k), i);
        ...
        if (!this->use_collocated_penetration_flag())
        {
          ...
          bool intersection = false;
          this->penetration(
            interpolated_x, interpolated_normal, d, intersection);
          if (!intersection)
          {
        ...
        if (local_eqn >= 0)
        {
          ...
          { // If so, we are gonna need information about the node
            ...
            if (this->use_collocated_penetration_flag())
            {
              ...
              // If there is no intersection, d = -max, ie the penetrator is
              // infinitely far away
              if (!intersection)
              {
                d = -DBL_MAX;
          ...
          // Now that we have the appropriate discretisations of penetration and
          // contact pressure, use the semi-smooth residual scheme
    ...
    for (unsigned i = 0; i < n; i++)
    {
      if (this->Penetrator_eq_data_type[i] >= 0)
      {
        switch (unsigned(this->Penetrator_eq_data_type[i]))
        {
          ...
          {
            int local_eqn =
              this->external_local_eqn(this->Penetrator_eq_data_data_index[i],
          ...
          case TemplateFreeContactElementBase::Nodal_position_data:
          {
            ...
            unsigned k = 0;
            int local_eqn =
              this->position_local_eqn(this->Penetrator_eq_data_data_index[i],
                                       k,
                                       this->Penetrator_eq_data_index[i]);
            if (local_eqn >= 0)
            {
          ...
          {
            int local_eqn =
              this->nodal_local_eqn(this->Penetrator_eq_data_data_index[i],
                                    this->Penetrator_eq_data_index[i]);
            if (local_eqn >= 0)
            {
              local_residuals[local_eqn] += contact_force[i];
    ...
    for (unsigned ipt = 0; ipt < n_intpt; ipt++)
    {
      ...
      switch (n_dim)
      {
        ...
        case 3:
          ...
          break;
        default:
          throw OomphLibError("Wrong dimension in SurfaceContactElement",
...

    {
      ...
      for (unsigned ipt = 0; ipt < n_intpt; ipt++)
      {
        ...
        switch (n_dim)
        {
          ...
          default:
            throw OomphLibError("Wrong dimension in SurfaceContactElement",
                                "LinearSurfaceContactElement::fill_in_"
        ...
        //=====LOAD TERMS FROM PRINCIPLE OF VIRTUAL DISPLACEMENTS AND BOUNDARY
        // CONDITIONS========
        ...
        for (unsigned l = 0; l < n_node; l++)
        {
          ...
          for (unsigned i = 0; i < n_dim; i++)
          {
            ...
            if (local_eqn >= 0)
            {
              // Add the loading terms from penetrator and imposed externally to
              // the residuals
