 ...
 void actions_before_implicit_timestep()
  {
   ...
   // (Exponential) growth rate for surface deflection
   double xi_rate=-10.0;

   
   
   bool do_boundary_only=true;
   if (do_boundary_only)
    {
     // How many bulk nodes are on boundary b?
     ...
     for(unsigned j=0;j<n_node;j++)
      {
       ...
       double y=nod_pt->xi(1);
       
       double dy=xi_rate*time_pt()->dt()*y*(1.0-x)*(1.0-x)*x*x;
       oomph_info << "xi0 xi1 dy: " 
                  << x << " "  
                  << y << " "  
                  << dy << "\n";  
       nod_pt->xi(1)=y-dy;
