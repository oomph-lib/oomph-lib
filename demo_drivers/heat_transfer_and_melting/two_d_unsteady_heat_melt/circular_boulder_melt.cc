...
namespace ExactSolution
{
                                                ...
                                                Vector<double> &u)
  {
    ...
    u[0] = U0 + t * t * Y * Y *
                  (Y - 1.0 +
                   Growth_rate * t * t *
                     (1.0 - cos(2.0 * X * 0.3141592653589793E1))) *
                  cos(2.0 * X * 0.3141592653589793E1);
  }
                                               ...
                                               double &source)
  {
    ...
    source =
      -2.0 * t * Y * Y *
        ...
        cos(2.0 * X * 0.3141592653589793E1) -
      2.0 * t * t * t * Y * Y * Growth_rate *
        (1.0 - cos(2.0 * X * 0.3141592653589793E1)) *
        cos(2.0 * X * 0.3141592653589793E1) +
      4.0 * t * t * t * t * Y * Y * Growth_rate *
        pow(cos(2.0 * X * 0.3141592653589793E1), 2.0) * 0.3141592653589793E1 *
        0.3141592653589793E1 -
      8.0 * t * t * t * t * Y * Y * Growth_rate *
        pow(sin(2.0 * X * 0.3141592653589793E1), 2.0) * 0.3141592653589793E1 *
        0.3141592653589793E1 -
      4.0 * t * t * Y * Y *
        (Y - 1.0 +
         Growth_rate * t * t * (1.0 - cos(2.0 * X * 0.3141592653589793E1))) *
        cos(2.0 * X * 0.3141592653589793E1) * 0.3141592653589793E1 *
        0.3141592653589793E1 +
      2.0 * t * t *
        (Y - 1.0 +
         Growth_rate * t * t * (1.0 - cos(2.0 * X * 0.3141592653589793E1))) *
        cos(2.0 * X * 0.3141592653589793E1) +
      4.0 * t * t * Y * cos(2.0 * X * 0.3141592653589793E1);
  }
                                    ...
                                    double &ny)
  {
    ...
    t0 = 2.0 * Growth_rate * t * t * sin(2.0 * X * 0.3141592653589793E1) *
         0.3141592653589793E1 /
         sqrt(1.0 + 4.0 * Growth_rate * Growth_rate * t * t * t * t *
                      pow(sin(2.0 * X * 0.3141592653589793E1), 2.0) *
                      0.3141592653589793E1 * 0.3141592653589793E1);

    nx = t0;

    t0 = 1 / (sqrt(1.0 + 4.0 * Growth_rate * Growth_rate * t * t * t * t *
                           pow(sin(2.0 * X * 0.3141592653589793E1), 2.0) *
                           0.3141592653589793E1 * 0.3141592653589793E1));
                      ...
                      double &flux)
  {
    ...
    flux =
      (2.0 * t * t * t * t * Y * Y * Growth_rate *
         sin(2.0 * X * 0.3141592653589793E1) * 0.3141592653589793E1 *
         cos(2.0 * X * 0.3141592653589793E1) -
       2.0 * t * t * Y * Y *
         (Y - 1.0 +
          Growth_rate * t * t * (1.0 - cos(2.0 * X * 0.3141592653589793E1))) *
      ...
      (2.0 * t * t * Y *
         (Y - 1.0 +
          Growth_rate * t * t * (1.0 - cos(2.0 * X * 0.3141592653589793E1))) *
         cos(2.0 * X * 0.3141592653589793E1) +
       t * t * Y * Y * cos(2.0 * X * 0.3141592653589793E1)) *
        Ny;
  }
                                                    ...
                                                    double &flux)
  {
    ...
    // Add melt flux
    double melt_flux =
      2.0 * Growth_rate * t * (1.0 - cos(2.0 * X * 0.3141592653589793E1)) /
      sqrt(1.0 + 4.0 * Growth_rate * Growth_rate * t * t * t * t *
                   pow(sin(2.0 * X * 0.3141592653589793E1), 2.0) *
                   0.3141592653589793E1 * 0.3141592653589793E1);
...
public:
  ...
  void actions_before_implicit_timestep()
  {
    ...
    ProblemParameters::Centre[1] =
      ProblemParameters::Y_c_initial -
      amplitude * (1.0 + 0.05 * time_pt()->time()) * 0.5 *
        (1.0 - cos(2.0 * MathematicalConstants::Pi * time_pt()->time()));

    oomph_info << "Solving for y_c = " << ProblemParameters::Centre[1]
               << std::endl;
  }
...
private:
  ...
  void complete_problem_setup()
  {
    ...
    // Loop over the contact elements, pass pointer to penetrator and make
    // sticky
    //---------------------------------------------------------------------------
...
MeltContactProblem<ELEMENT>::MeltContactProblem()
{
  ...
  boundary_polyline_pt.push_back(
    new TriangleMeshPolyLine(bound_coords, Left_boundary_id));
  ...
  boundary_polyline_pt.push_back(
    new TriangleMeshPolyLine(bound_coords, Bottom_boundary_id));
  ...
  Right_boundary_id = 2;
  boundary_polyline_pt.push_back(
    new TriangleMeshPolyLine(bound_coords, Right_boundary_id));

  // Half opening angle of "boulder"
  double half_opening_angle =
    acos((ProblemParameters::Centre[1] - 1.0) / ProblemParameters::Radius);
  ...
  {
    double phi =
      1.5 * MathematicalConstants::Pi + half_opening_angle -
      2.0 * half_opening_angle * double(j) / double(nvertex_contact - 1);

    circle_bound_coords[j].resize(2);
    circle_bound_coords[j][0] =
      ProblemParameters::Centre[0] + ProblemParameters::Radius * cos(phi);

    circle_bound_coords[j][1] =
      ProblemParameters::Centre[1] + ProblemParameters::Radius * sin(phi);
  }
  ...
  unsigned nvertex_contact_upper =
    unsigned(double(nvertex_contact) * 0.5 * MathematicalConstants::Pi /
             half_opening_angle);
  ...
  for (unsigned j = 1; j < npt_left - 1; j++)
  {
    ...
    left_melt_bound_coords[j][0] =
      left_penetrator[0] * double(j) / double(npt_left - 1);
    left_melt_bound_coords[j][1] =
      left_penetrator[1] -
  ...
  double maximum_length =
    4.0 * left_penetrator[0] / double(npt_left - 1); // hierher why 4
...
//#define USE_BROKEN
  ...
  // Build mesh
  Bulk_mesh_pt =
    new SolidTriangleMesh<ELEMENT>(triangle_mesh_parameters, time_stepper_pt());
...
int main(int argc, char *argv[])
{
  ...
  // Create penetrator
  ProblemParameters::Penetrator_pt = new CircularPenetrator(
    &ProblemParameters::Centre, ProblemParameters::Radius);
  ...
  while (problem.time_pt()->time() < double(nperiod))
  {
    ...
    double next_dt = problem.doubly_adaptive_unsteady_newton_solve(
      dt,
      epsilon_t,
      max_adapt,
