...
//LIC// but WITHOUT ANY WARRANTY; wiythout even the implied warranty of
...
public:
 ...
 void actions_before_adapt() 
  {
   ...
   // Output solution before adaptation (before flux elements get wiped!)
   // Otherwise they don't get re-populated if no actual adaptation 
   // is performed and the output is empty!
   bool called_from_actions_before_adapt=true;
   doc_solution(called_from_actions_before_adapt);
 ...
 /// Doc the solution
 void doc_solution(const bool& called_from_actions_before_adapt=false);
...
void UnsteadyHeatMeltProblem<ELEMENT>::doc_solution(
 const bool& called_from_actions_before_adapt)
{ 
 ofstream some_file;
 char filename[100];
 ...
 if (called_from_actions_before_adapt)
  {
   ...
   sprintf(filename,"%s/flux_with_melt%i.dat",Doc_info.directory().c_str(),
           Doc_info.number());
   some_file.open(filename);
   unsigned nnod=dynamic_cast<SurfaceMeltElement<ELEMENT>*>(
 ...
 // Only write trace file for proper output
 else
  {
