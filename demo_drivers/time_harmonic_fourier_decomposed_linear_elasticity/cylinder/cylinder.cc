...
namespace Global_Parameters
{
 ...
 /// The traction function at r=rmin
                      ...
                      Vector<std::complex<double> > &result)
 {
  ...
  result[2] = -mu*pow(x[0],2)*(2*pow(x[1],3)+I*double(Fourier_wavenumber)*cos(x[1]));
 }


 /// \short The body force function; returns vector of complex doubles!
 void gravity(const Vector<double> &x,
              Vector<std::complex<double> > &result)
 {
  result[0] = x[0]*(-2.0*I*lambda*double(Fourier_wavenumber)*pow(x[1],3)-cos(x[1])*
                    (lambda*(8.0+3.0*x[0])-
                     mu*(pow(double(Fourier_wavenumber),2)
                         -16.0+x[0]*(x[0]-3.0))+pow(x[0],2)*Omega_sq));
  result[1] = x[0]*sin(x[1])*(mu*(pow(double(Fourier_wavenumber),2)-9.0)+
                              4.0*x[0]*(lambda+mu)+pow(x[0],2)*
                              (lambda+2.0*mu-Omega_sq))-
              3.0*I*double(Fourier_wavenumber)*pow(x[0],2)*pow(x[1],2)*(lambda+mu);
  result[2] = -x[0]*(8.0*mu*pow(x[1],3)-pow(double(Fourier_wavenumber),2)*pow(x[1],3)*
                     (lambda+2.0*mu)+pow(x[0],2)*(pow(x[1],3)*Omega_sq+6.0*mu*x[1])+I*cos(x[1])*
                     double(Fourier_wavenumber)*
 ...
 for(unsigned e=0;e<n_traction;e++)
  {
   ...
   TimeHarmonicFourierDecomposedLinearElasticityTractionElement<ELEMENT> *el_pt = 
    dynamic_cast<TimeHarmonicFourierDecomposedLinearElasticityTractionElement<ELEMENT>* >
