  ...
  Element_area(element_area)
{
  ...
  nsegment = (int)(GlobalParameters::R_b * MathematicalConstants::Pi /
                   sqrt(element_area));
  inner_curvilinear_boundary_pt[0] = new TriangleMeshCurviLine(
    inner_boundary_ellipse_pt, zeta_start, zeta_end, nsegment, Inner_boundary0);
  ...
  inner_curvilinear_boundary_pt[1] = new TriangleMeshCurviLine(
    inner_boundary_ellipse_pt, zeta_start, zeta_end, nsegment, Inner_boundary1);
...
void UnstructuredFvKProblem<ELEMENT>::complete_problem_setup()
{
  ...
  for (unsigned inode = 0; inode < n_boundary3_node; inode++)
  {
    for (unsigned jnode = 0; jnode < n_boundary4_node; jnode++)
    {
          ...
          My_mesh_pt->boundary_node_pt(Inner_boundary4, jnode))
      {
        ...
        done_pin = true;
        oomph_info << "Pinned horizontal/vertical displacement at [ "
  ...
  for (unsigned inode = 0; inode < n_boundary0_node; inode++)
  {
    for (unsigned jnode = 0; jnode < n_boundary5_node; jnode++)
    {
          ...
          My_mesh_pt->boundary_node_pt(Inner_boundary5, jnode))
      {
        ...
        oomph_info << "Pinned horizontal displacement at [ "
  ...
  for (unsigned j = 0; j < n_sample; j++)
  {
    ...
    Mesh_as_geom_object_pt->locate_zeta(
      x, Radial_sample_point_pt[j].first, Radial_sample_point_pt[j].second);
  }
}
...
void UnstructuredFvKProblem<ELEMENT>::doc_solution()
{
  ...
  for (unsigned j = 0; j < nplot; j++)
  {
    ...
    some_file << x[0] << " " << x[1] << " " << el_pt->interpolated_w_fvk(s, 0)
              << " " // w
  ...
  some_file.open(filename);
  My_mesh_pt->compute_error(
    some_file, GlobalParameters::zero, dummy_error, zero_norm);
  some_file.close();

  // Doc L2 error and norm of solution
  oomph_info << "Norm of computed solution: " << sqrt(dummy_error) << std::endl;

  Trace_file << GlobalParameters::Pressure << " " << w_0 << '\n';
...
int main(int argc, char **argv)
{
  ...
  // Problem instance
  UnstructuredFvKProblem<TDisplacementBasedFoepplvonKarmanElement<3>> problem(
    0.01);
