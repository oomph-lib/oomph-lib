...
///Lambda, but note coefficients in swimming & diffusion need to be recomputed separately is this is modified
 double Lambda=2.2;
 ...
 void wind_function(const Vector<double>& x, Vector<double>& wind)
 {
  ...
  //direction
  wind[0] = 0.0;
  wind[1] = 2*(Radius*Radius - x[0]*x[0]);
  wind[2] = 0.0;
 }
...
{ 
 ...
 unsigned n_z=4*Global_Physical_Variables::Length;
 ...
 // conditions here
 // Assume specified side concentration (boundary 1)
 /*{
  unsigned b=1;
 ...
 for(unsigned n=0;n<n_node;n++)
  {
   ...
   //Exponentially decaying in z direction; decay in r is specified equilibrium 
  // double c = exp(-(Global_Physical_Variables::Peclet*Global_Physical_Variables::Lambda/Global_Physical_Variables::Beta)*r*r
    //              -10.0*(z-0.1*Global_Physical_Variables::Length)*
      //            (z-0.1*Global_Physical_Variables::Length));

 //Exponentially decaying in z direction; uniform in r  
   double c = exp(-4*r*r)*
     exp( -(z-0.1*Global_Physical_Variables::Length)*
                  (z-0.1*Global_Physical_Variables::Length)/(0.01*0.01*Global_Physical_Variables::Length*Global_Physical_Variables::Length));


 //Exponentially decaying in  two directions
 //  double c = exp(-10*r*r
   //               -10.0*(z-0.2*Global_Physical_Variables::Length)*
     //             (z-0.2*Global_Physical_Variables::Length));
...
int main()
{
 ...
 bool first = true;
 unsigned max_adapt=6;

// for(unsigned nstep=0;nstep<1;nstep++)
//   {
...
//   }
 ...
 for(unsigned nstep=0;nstep<80;nstep++)
  {
