...
TetmeshPoissonProblem<ELEMENT>::TetmeshPoissonProblem()
{
  ...
  Outer_boundary_pt =
    new CubicTetMeshFacetedSurface(Global_Parameters::Box_half_width,
  ...
  // For sanity check:
  Exact_bounded_volume = 2.0 * Global_Parameters::Box_half_width * 2.0 *
                         Global_Parameters::Box_half_width * 2.0 *
                         Global_Parameters::Box_half_length;
  ...
  // And now build it...
  Bulk_mesh_pt = new RefineableTetgenMesh<ELEMENT>(Outer_boundary_pt,
                                                   Inner_boundary_pt,
...
void TetmeshPoissonProblem<ELEMENT>::apply_boundary_conditions()
{
  ...
  for (unsigned ibound = First_boundary_id_for_outer_boundary;
       ibound < First_boundary_id_for_outer_boundary + 6;
       ibound++)
  {
    pinned_boundary_id.push_back(ibound);
  }
  ...
  for (unsigned bnd = 0; bnd < num_pin_bnd; bnd++)
  {
    ...
    if (num_nod == 0)
    {
      ...
      error_message << "No boundary nodes on boundary " << ibound
                                                  ...
                                                  DocInfo &doc_info)
{
  ...
  //-------------------------------------------
  sprintf(filename,
          "%s/elements_next_to_outer_boundary%i.dat",
          doc_info.directory().c_str(),
          doc_info.number());
       ...
       ibound++)
  {
    ...
    for (unsigned e = 0; e < n_el; e++)
    {
      if (do_bulk_output)
      {
        Bulk_mesh_pt->boundary_element_pt(ibound, e)->output(some_file, nplot);
      }
    }
  }
  some_file.close();
  ...
  //-----------------------------------------------
  sprintf(filename,
          "%s/boundary_coordinates_outer_boundary%i.dat",
          doc_info.directory().c_str(),
          doc_info.number());
  some_file.open(filename);
  for (unsigned ibound = First_boundary_id_for_outer_boundary;
       ...
       ibound++)
  {
    Bulk_mesh_pt->Mesh::template doc_boundary_coordinates<ELEMENT>(ibound,
                                                                   some_file);
  }
  some_file.close();
  ...
  for (unsigned e = 0; e < n_el; e++)
  {
    ...
    volume_in_torus_region +=
      Bulk_mesh_pt->region_element_pt(region_id, e)->size();
  }
  some_file.close();
  ...
  sprintf(filename,
          "%s/soln_in_above_disk_region%i.dat",
          doc_info.directory().c_str(),
          doc_info.number());
  some_file.open(filename);
  ...
  sprintf(filename,
          "%s/soln_in_below_disk_region%i.dat",
          doc_info.directory().c_str(),
          doc_info.number());
  some_file.open(filename);
  region_id = Above_disk_region_id;
  ...
  oomph_info << "Error in total region volume balance: "
             << abs(total_mesh_volume -
                    (volume_in_above_disk_region + volume_in_below_disk_region +
                     volume_in_torus_region + volume_in_region0)) /
                  total_mesh_volume * 100.0
             << " % " << std::endl;

  oomph_info << "Error in above/below disk region volume balance: "
             << abs(volume_in_above_disk_region - volume_in_below_disk_region) /
                  volume_in_above_disk_region * 100.0
             << " % " << std::endl;

  // Attach face elements to boundary of torus
  //------------------------------------------
  sprintf(filename,
          "%s/face_elements_on_boundary_of_torus%i.dat",
          doc_info.directory().c_str(),
          doc_info.number());
  some_file.open(filename);
  ...
  //--------------------------------------------------
  sprintf(filename,
          "%s/face_elements_on_disk_in_torus%i.dat",
          doc_info.directory().c_str(),
          doc_info.number());
  some_file.open(filename);
  ...
  some_file.close();
  oomph_info << "Disk in torus surface area: " << disk_in_torus_surface_area
             << std::endl;

  // Attach face elements to part of disk outside torus
  //--------------------------------------------------
  sprintf(filename,
          "%s/face_elements_on_disk_outside_torus%i.dat",
          doc_info.directory().c_str(),
          doc_info.number());
  some_file.open(filename);
  ...
  //-------------------------------------------
  sprintf(filename,
          "%s/face_elements_on_free_standing_disk%i.dat",
          doc_info.directory().c_str(),
          doc_info.number());
  some_file.open(filename);
  ...
  //---------------------------------------------------------
  sprintf(filename,
          "%s/face_elements_on_disk_upper_layer%i.dat",
          doc_info.directory().c_str(),
          doc_info.number());
  some_file.open(filename);
  ...
  for (unsigned b = First_disk_with_layers_boundary_id;
       b <= Last_disk_with_layers_boundary_id;
       b++)
  {
    unsigned nel = Bulk_mesh_pt->nboundary_element_in_region(b, region_id);
    for (unsigned e = 0; e < nel; e++)
  ...
  //---------------------------------------------------------
  sprintf(filename,
          "%s/face_elements_on_disk_lower_layer%i.dat",
          doc_info.directory().c_str(),
          doc_info.number());
  some_file.open(filename);
  ...
  volumes_and_areas_file << volume_in_above_disk_region << " "
                         << volume_in_below_disk_region << " "
                         << volume_in_torus_region << " " << total_mesh_volume
                         << " " << volume_in_region0 << " "
                         << torus_surface_area << " "
  ...
  //---------------------
  sprintf(
    filename, "%s/norm%i.dat", doc_info.directory().c_str(), doc_info.number());
  some_file.open(filename);
...
int main(int argc, char *argv[])
{
  ...
  CommandLineArgs::specify_command_line_flag(
    "--gmsh_command_line", &Global_Parameters::Gmsh_command_line_invocation);
  ...
  // Build problem
  TetmeshPoissonProblem<ProjectablePoissonElement<TPoissonElement<3, 3>>>
    problem;
