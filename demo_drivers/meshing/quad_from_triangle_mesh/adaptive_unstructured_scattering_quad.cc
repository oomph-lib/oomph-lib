...
namespace GlobalParameters
{
  ...
  void get_exact_u(const Vector<double>& x, Vector<double>& u)
  {
    ...
    Hankel_functions_for_helmholtz_problem::Hankel_first(
      N_fourier, sqrt(K_squared), h_a, hp_a);
...
ScatteringProblem<ELEMENT, MESH>::ScatteringProblem()
{
  ...
  outer_boundary_line_pt[0] = new TriangleMeshCurviLine(
    outer_circle_pt, s_start, s_end, n_segments, boundary_id);

  // The intrinsic coordinates for the beginning and end of the curve
  s_start = MathematicalConstants::Pi;
  s_end = 2.0 * MathematicalConstants::Pi;
  boundary_id = 3;
  outer_boundary_line_pt[1] = new TriangleMeshCurviLine(
    outer_circle_pt, s_start, s_end, n_segments, boundary_id);
...
void ScatteringProblem<ELEMENT, MESH>::set_prescribed_incoming_flux_pt()
{
  ...
  for (unsigned e = 0; e < n_element; e++)
  {
    ...
    el_pt->flux_fct_pt() = &GlobalParameters::prescribed_incoming_flux;
  }
...
//============================================================================
template<class ELEMENT, class MESH>
void ScatteringProblem<ELEMENT, MESH>::create_flux_elements(
  const unsigned& b,
  Mesh* const& bulk_mesh_pt,
  Mesh* const& helmholtz_inner_boundary_mesh_pt)
{
...
//===========================================================================
template<class ELEMENT, class MESH>
void ScatteringProblem<ELEMENT, MESH>::create_outer_bc_elements(
  const unsigned& b,
  Mesh* const& bulk_mesh_pt,
  Mesh* const& helmholtz_outer_boundary_mesh_pt)
{
...
//==========================================================
template<class ELEMENT, class MESH>
void ScatteringProblem<ELEMENT, MESH>::delete_face_elements(
