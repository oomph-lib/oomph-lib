...
UnstructuredPoissonProblem<ELEMENT>::UnstructuredPoissonProblem()
{
  ...
  curvilinear_boundary_pt[0] = new TriangleMeshCurviLine(
    ellipse_pt, zeta_start, zeta_end, nsegment, boundary_id);
  ...
  const std::string& comment)
{
  ...
  My_mesh_pt->compute_error(
    some_file, TanhSolnForPoisson::zero, dummy_error, zero_norm);
  some_file.close();
...
int main(int argc, char** argv)
{
  ...
  // Create problem
  UnstructuredPoissonProblem<TPoissonElement<2, 3>> problem;
