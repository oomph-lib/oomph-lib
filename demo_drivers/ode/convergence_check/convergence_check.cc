...
int main(int argc, char* argv[])
{
  ...
  /// Compute current error value
  double err1 = abs(my_ode_problem.compute_current_error());

  double accuracy_estimate =
    (log10(err0) - log10(err1)) / (log10(t_step * 2.0) - log10(t_step));

  printf("Error_0 = %g, Error_1 = %g, Approximate accuracy: %g\n",
         err0,
         err1,
         accuracy_estimate);
