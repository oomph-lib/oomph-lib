...
int main(int argc, char* argv[])
{
  ...
  specify_command_line_flag(
    "-max-step", &max_step, "Maximum number of steps to run");

  double tol = 0.0;
  specify_command_line_flag(
    "-tol", &tol, "Adaptive time step tolerance (default 0 = fixed step)");

  double dt_initial = 1e-5;
  specify_command_line_flag(
    "-dt-initial", &dt_initial, "Initial dt for adaptive time step selection.");
  ...
  specify_command_line_flag(
    "-outdir", &outdir, "Directory to write output to.");
  ...
  {
    mesh_pt[0]->add_element_pt(
      new ODEElement(time_stepper_pt, problem.Exact_solution_pt));
  }
  else if (element_type == "imr-element")
  {
    mesh_pt[0]->add_element_pt(
      new IMRODEElement(time_stepper_pt, problem.Exact_solution_pt));
  }
  else
  {
  ...
  while ((problem.time() < tmax) && (problem.N_steps_taken < max_step))
  {
    // Output some basic info
    oomph_info << std::endl
               << std::endl
               << "Time step " << problem.N_steps_taken
               << ", time = " << problem.time() << ", dt = " << dt << std::endl
               << "=============================================" << std::endl
               << std::endl;
