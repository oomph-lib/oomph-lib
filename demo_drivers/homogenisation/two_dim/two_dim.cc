...
HomogenisationProblem<ELEMENT>::HomogenisationProblem() : M(0), P(0)
{
  ...
  for (unsigned h = 0; h < 1; h++)
  {
    ...
    curvilinear_boundary_pt[0] = new TriangleMeshCurviLine(
      Internal_circle_pt[h], zeta_start, zeta_end, nsegment, boundary_id);
...
void HomogenisationProblem<ELEMENT>::complete_problem_setup()
{
  ...
  std::sort(boundary_nodes_pt[1].begin(),
            boundary_nodes_pt[1].end(),
            CompareNodeCoordinatesY());
  std::sort(boundary_nodes_pt[2].begin(),
            boundary_nodes_pt[2].end(),
            CompareNodeCoordinatesX());
  std::sort(boundary_nodes_pt[3].begin(),
            boundary_nodes_pt[3].end(),
            CompareNodeCoordinatesY());
  ...
  for (unsigned n = 1; n < n_node - 1; n++)
  {
                 ...
                 boundary_nodes_pt[2][n]->x(0)) > tol)
    {
      ...
      error_stream
        ...
        << "but the nodes have x-coordinates " << boundary_nodes_pt[0][n]->x(0)
        << " and " << boundary_nodes_pt[2][n]->x(0);
  ...
  for (unsigned n = 1; n < n_node - 1; n++)
  {
    ...
    if (std::abs(boundary_nodes_pt[1][n]->x(1) -
                 boundary_nodes_pt[3][n]->x(1)) > tol)
    {
      std::ostringstream error_stream;
      error_stream
        << "Trying to make periodic nodes across the left/right boundary,\n"
        << "but the nodes have y-coordinates " << boundary_nodes_pt[1][n]->x(1)
        << " and " << boundary_nodes_pt[3][n]->x(1);
