...
namespace TanhSolnForPoisson
{
  ...
  void get_source(const Vector<double>& x, double& source)
  {
    source = 2.0 * tanh(-1.0 + Alpha * (Beta * x[0] - x[1])) *
             ...
             2.0 * tanh(-1.0 + Alpha * (Beta * x[0] - x[1])) *
               (1.0 - pow(tanh(-1.0 + Alpha * (Beta * x[0] - x[1])), 2.0)) *
               Alpha * Alpha;
  }
  ...
  Domain_configuration(domain_configuration)
{
  ...
  else
  {
    ...
    error_message << "The choosen domain configuration is not implemented\n"
...
void PoissonProblem<ELEMENT>::half_circle_domain()
{
  ...
  for (unsigned i = 0; i < num_vertices_b0; i++) vertices_b0[i].resize(2);
...
void PoissonProblem<ELEMENT>::half_circle_domain_with_internal_boundaries()
{
  ...
  for (unsigned i = 0; i < num_vertices_b2; i++) verticesb2[i].resize(2);
  ...
  for (unsigned i = 0; i < num_vertices_b3; i++) verticesb3[i].resize(2);
  ...
  for (unsigned i = 0; i < num_vertices_b4; i++) verticesb4[i].resize(2);
  ...
  for (unsigned i = 0; i < num_vertices_b5; i++) verticesb5[i].resize(2);
...
void PoissonProblem<ELEMENT>::complex_domain_with_holes()
{
  ...
  for (unsigned i = 0; i < num_vertices_b2; i++) vertices_b2[i].resize(2);
  ...
  for (unsigned i = 0; i < num_vertices_b4; i++) vertices_b4[i].resize(2);
  ...
  for (unsigned i = 0; i < num_vertices_b5; i++) vertices_b5[i].resize(2);
  ...
  for (unsigned i = 0; i < num_vertices_b6; i++) vertices_b6[i].resize(2);
  ...
  for (unsigned i = 0; i < num_vertices_b7; i++) vertices_b7[i].resize(2);
  ...
  Vector<unsigned>& output_distribution)
{
  ...
  if (file_pt == 0)
  {
    ...
    error_stream << "Input file could not be opened: " << file_name
                 << std::endl;
    throw OomphLibError(
  ...
  for (unsigned e = 0; e < n_data; e++)
  {
    unsigned counter = 0;
    iret = fscanf(file_pt,
                  "ELEMENT(%i):PROCESSOR(%i)\n",
                  &counter,
                  &output_distribution[e]);
  }

  // Close the file
  fclose(file_pt);

  oomph_info << "Read custom distribution from file: " << file_name << " [DONE]"
             << std::endl;
}
  ...
  Vector<unsigned>& input_distribution)
{
  ...
  else
  {
    // Error, the file could not be opened
    std::ostringstream error_stream;
    error_stream << "Output file could not be opened: " << file_name
                 << std::endl;
    throw OomphLibError(
      error_stream.str(), OOMPH_CURRENT_FUNCTION, OOMPH_EXCEPTION_LOCATION);
  }
  ...
  oomph_info << "Save custom distribution to file: " << file_name << " [DONE]"
             << std::endl;
}
...
int main(int argc, char* argv[])
{
  ...
  CommandLineArgs::specify_command_line_flag(
    "--domain_configuration", &TestArguments::Domain_configuration);
  ...
  // Max. permitted error
  CommandLineArgs::specify_command_line_flag(
    "--max_permitted_error", &TestArguments::Max_permitted_error);

  // Min. permitted error
  CommandLineArgs::specify_command_line_flag(
    "--min_permitted_error", &TestArguments::Min_permitted_error);
  ...
  // Max adaptations
  CommandLineArgs::specify_command_line_flag(
    "--folder_distribution_file", &TestArguments::Folder_distribution_file);
  ...
  // Set up the problem
  PoissonProblem<ProjectablePoissonElement<TPoissonElement<2, 3>>> problem(
