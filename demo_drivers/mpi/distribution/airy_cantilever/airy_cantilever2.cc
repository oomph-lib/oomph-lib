        ...
        case 2:
          ...
          for (unsigned l2 = 0; l2 < n_plot; l2++)
          {
            ...
            for (unsigned l1 = 0; l1 < n_plot; l1++)
            {
              ...
              // Output stress
              outfile << sigma(0, 0) << " " << sigma(1, 0) << " " << sigma(1, 1)
                      << " " << std::endl;
            }
          }
                                              ...
                                              const bool &use_fd)
{
  ...
  // Now create the mesh
  Problem::mesh_pt() =
    new ElasticRectangularQuadMesh<ELEMENT>(n_x, n_y, l_x, l_y, origin);
...
void CantileverProblem<ELEMENT>::doc_solution()
{
  ...
  double b =
    -3.0 / 8.0 * Global_Physical_Variables::P / Global_Physical_Variables::H;
  double c = 1.0 / 8.0 * Global_Physical_Variables::P /
             pow(Global_Physical_Variables::H, 3);
  double d =
    1.0 / 20.0 * Global_Physical_Variables::P / Global_Physical_Variables::H;
  ...
  for (unsigned e = 0; e < nel; e++)
  {
    ...
    for (unsigned l2 = 0; l2 < n_plot; l2++)
    {
      ...
      for (unsigned l1 = 0; l1 < n_plot; l1++)
      {
        ...
        sigma(0, 0) =
          c * (6.0 * xx * xx * yy - 4.0 * yy * yy * yy) + 6.0 * d * yy;
        sigma(1, 1) = 2.0 * (a + b * yy + c * yy * yy * yy);
        sigma(1, 0) = 2.0 * (b * xx + 3.0 * c * xx * yy * yy);
        sigma(0, 1) = sigma(1, 0);

        // Output stress
        some_file << sigma(0, 0) << " " << sigma(1, 0) << " " << sigma(1, 1)
                  << " " << std::endl;
      }
    }
  }
  ...
  Trace_file << Global_Physical_Variables::P << " " << Trace_node_pt->x(0)
             << " " << Trace_node_pt->x(1) << " " << std::endl;
...
void CantileverProblem<ELEMENT>::run_it(const unsigned &i_case)
{
  ...
  std::cout << "Total fill_in... : "
            << PVDEquationsBase<2>::Solid_timer.cumulative_time(0) << std::endl;

  std::cout << "Total d_stress_dG: "
            << PVDEquationsBase<2>::Solid_timer.cumulative_time(1) << std::endl;

  std::cout << "Total Jac        : "
            << PVDEquationsBase<2>::Solid_timer.cumulative_time(2) << std::endl;
  ...
  for (unsigned i = 0; i < 2; i++)
  {
    ...
    //-------------------------------------------
    {
      Global_Physical_Variables::Constitutive_law_pt = new GeneralisedHookean(
        &Global_Physical_Variables::Nu, &Global_Physical_Variables::E);
      ...
      {
        // Set up the problem with pure displacement based elements
        CantileverProblem<MySolidElement<QPVDElement<2, 3>>> problem(incompress,
                                                                     use_fd);
...
#ifdef REFINE
      {
        ...
        CantileverProblem<
          MySolidElement<RefineableQPVDElementWithContinuousPressure<2>>>
      ...
      {
        // Set up the problem with continous pressure/displacement
        CantileverProblem<MySolidElement<QPVDElementWithContinuousPressure<2>>>
          problem(incompress, use_fd);
...
#ifdef REFINE
      {
        ...
        CantileverProblem<MySolidElement<RefineableQPVDElementWithPressure<2>>>
          problem(incompress, use_fd);
      ...
      {
        // Set up the problem with discontinous pressure/displacement
        CantileverProblem<MySolidElement<QPVDElementWithPressure<2>>> problem(
          incompress, use_fd);
    ...
    //-----------------------------
    {
      Global_Physical_Variables::Strain_energy_function_pt = new MooneyRivlin(
