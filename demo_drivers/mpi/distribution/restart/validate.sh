...
for first in $(echo $first_list); do
    ...
    if test "$3" = "no_fpdiff"; then
        ...
        $OOMPH_ROOT_DIR/scripts/fpdiff.py ../validata/result_$(echo $first_flag)_first_for_restart.dat.gz result_$(echo $first_flag)_first_for_restart.dat >>validation.log
    fi
    ...
    for restart_step in $(echo $restart_list); do
        ...
        if test "$3" = "no_fpdiff"; then
            ...
            $OOMPH_ROOT_DIR/scripts/fpdiff.py ../validata/result_$(echo $first_flag)_first_for_restart.dat.gz result_$(echo $first_flag)_first_restarted_from_step_restart$(echo $restart_step).dat >>validation.log
        fi
