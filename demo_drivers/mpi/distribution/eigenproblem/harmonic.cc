...
void HarmonicProblem<ELEMENT, EIGEN_SOLVER>::solve(const unsigned& label)
{
  ...
  // Normalise the eigenvector
  {
    ...
    if (eigenvectors[second_smallest_index].distributed() &&
        eigenvectors[second_smallest_index]
            .distribution_pt()
            ->communicator_pt()
            ->nproc() > 1)
    {
    ...
    {
      if (eigenvectors[second_smallest_index][0] < 0)
      {
        sign = -1;
      }
    }
...
int main(int argc, char** argv)
{
  ...
  oomph_info << "ANASAZI TIME (non-distributed): "
             << (double)(t_end1 - t_start1) / CLOCKS_PER_SEC << std::endl;

  oomph_info << "ANASAZI TIME (distributed): "
             << (double)(t_end2 - t_start2) / CLOCKS_PER_SEC << std::endl;
#endif
