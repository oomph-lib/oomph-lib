  ...
  GeomObject* const& undeformed_midplane_pt)
{
  ...
  for (unsigned n = 0; n < n_node; n++)
  {
    ...
    for (unsigned i = 0; i < 3; i++)
    {
      ...
      node_pt(n)->x_gen(1, i) =
        0.5 * a(0, i) * ((this->Xmax - this->Xmin) / this->Nx);
      node_pt(n)->x_gen(2, i) =
...
public:
  ...
  void actions_after_distribute()
  {
    ...
    {
      first_el_pt = dynamic_cast<ELEMENT*>(solid_mesh_pt()->element_pt(0));
    }
...
void ShellProblem<ELEMENT>::solve()
{
  ...
  for (unsigned i = 1; i < 13; i++)
  {
    ...
    // Output the pressure
    trace << Global_Physical_Variables::external_pressure() /
               (pow(0.05, 3) / 12.0)
          << " "
  ...
  sprintf(
    filename, "final_shape_on_proc%i.dat", this->communicator_pt()->my_rank());
  ...
  sprintf(
    filename, "trace_disp_on_proc%i.dat", this->communicator_pt()->my_rank());
  trace.open(filename);
  // Take fifteen continuation steps
  for (unsigned i = 0; i < 15; i++)
  {
    ds = arc_length_step_solve(Global_Physical_Variables::Pext_data_pt, 0, ds);
...
int main(int argc, char* argv[])
{
...
#endif
  {
    ...
    // Set up the problem
    ShellProblem<StorableShapeSolidElement<DiagHermiteShellElement>> problem(
      5, 5, L, L_phi);
    ...
    for (unsigned e = 0; e < n_element / 2; e++)
    {
      element_partition[e] = 0;
    }
    for (unsigned e = n_element / 2; e < n_element; e++)
    {
