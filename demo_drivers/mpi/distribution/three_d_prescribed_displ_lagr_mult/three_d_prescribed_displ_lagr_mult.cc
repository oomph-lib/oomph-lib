...
public:
  ...
  void position(const Vector<double>& zeta, Vector<double>& r) const
  {
    ...
    r[1] = zeta[1] + 5.0 * Ampl * zeta[1] * (zeta[1] - 1.0) * (zeta[1] - 0.7) *
                       zeta[0] * (zeta[0] - 1.0);
    r[2] = 1.0 + Ampl * 0.25 *
                   (1.0 - cos(2.0 * MathematicalConstants::Pi * zeta[0])) *
...
template<class ELEMENT>
PrescribedBoundaryDisplacementProblem<
  ELEMENT>::PrescribedBoundaryDisplacementProblem()
{
  // Create the mesh
  ...
  // Now create the mesh
  solid_mesh_pt() =
    new RefineableElasticCubicMesh<ELEMENT>(n_x, n_y, n_z, l_x, l_y, l_z);
  ...
  for (unsigned i = 0; i < n_element; i++)
  {
    ...
    for (unsigned j = 0; j < nnod; j++)
    {
      ...
      if ((nod_pt->is_on_boundary(1)) || (nod_pt->is_on_boundary(2)) ||
          (nod_pt->is_on_boundary(3)) || (nod_pt->is_on_boundary(4)))
      {
