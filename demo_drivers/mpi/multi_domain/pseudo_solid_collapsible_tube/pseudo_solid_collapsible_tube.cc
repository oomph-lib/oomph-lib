...
namespace oomph
{
  ...
  class PseudoElasticBulkElement : public virtual ELEMENT
  {
  public:
    /// default constructor
...
template<class ELEMENT>
class WallMesh :
  public virtual RefineableSimpleCubicMesh<ELEMENT>,
...
public:
  ...
  void doc_parameters()
  {
    ...
    std::cout << "T                     : " << Global_Parameters::T
              << std::endl;
    std::cout << "t                     : " << time_pt()->time() << std::endl;
    std::cout << "=================================================\n\n";
  }
  ...
  PseudoElasticCollapsibleChannelProblem()
{
  ...
  Fluid_mesh_pt = new FluidMesh<FLUID_ELEMENT>(Wall_pt,
                                               xi_lo,
                                               frac_mid,
                                               xi_hi,
                                               Global_Parameters::N_slice,
  ...
  else
  {
    ...
    Fluid_mesh_pt->spatial_error_estimator_pt() =
      new DummyErrorEstimator(Fluid_mesh_pt,
                              elements_to_refine,
                              central_node_number,
  ...
  Solid_mesh_pt = new WallMesh<SOLID_ELEMENT>(
    r_min, r_max, lz, nr, nphi, nz, Wall_time_stepper_pt);
  ...
  else
  {
    ...
    Solid_mesh_pt->spatial_error_estimator_pt() =
      new DummyErrorEstimator(Solid_mesh_pt,
                              elements_to_refine,
                              central_node_number,
                              use_lagrangian_coordinates);
  }
  ...
  RefineableNavierStokesEquations<3>::pin_redundant_nodal_pressures(
  ...
  {
    Fluid_mesh_pt->boundary_node_pt(b, i)->set_auxiliary_node_update_fct_pt(
  ...
  actions_after_adapt()
{
  ...
  RefineableNavierStokesEquations<3>::unpin_all_pressure_dofs(
    Fluid_mesh_pt->element_pt());
  ...
  actions_before_implicit_timestep()
{
  ...
  double fact =
    1.0 + 0.5 * sin(MathematicalConstants::Pi * t / Global_Parameters::T);
  ...
  doc_solution(DocInfo& doc_info)
{
  ...
  //------------------------
  filename << doc_info.directory() << "/solid_boundaries" << doc_info.number()
           << ".dat";
  ...
  //-----------------------
  filename.str("");
  filename << doc_info.directory() << "/solid_soln" << doc_info.number()
           << ".dat";
  some_file.open(filename.str().c_str());
  ...
  //------------------------
  filename.str("");
  filename << doc_info.directory() << "/fluid_boundaries" << doc_info.number()
           << ".dat";
  some_file.open(filename.str().c_str());
  ...
  //-----------------------
  filename.str("");
  filename << doc_info.directory() << "/fluid_soln" << doc_info.number()
           << ".dat";
  some_file.open(filename.str().c_str());
  ...
  //--------------------
  filename.str("");
  filename << doc_info.directory() << "/fsi_traction" << doc_info.number()
           << ".dat";
  some_file.open(filename.str().c_str());
  ...
  //--------------------
  filename.str("");
  filename << doc_info.directory() << "/solid_traction" << doc_info.number()
           << ".dat";
  some_file.open(filename.str().c_str());
  ...
  //----------------------------
  filename.str("");
  filename << doc_info.directory() << "/lagrange" << doc_info.number()
           << ".dat";
  some_file.open(filename.str().c_str());
  ...
  unsteady_run(DocInfo& doc_info)
{
  ...
  unsigned nstep =
    Global_Parameters::Nstep_per_period * Global_Parameters::Nperiod;
  if (CommandLineArgs::command_line_flag_has_been_set("--validate"))
  {
...
int main(int argc, char* argv[])
{
  ...
  oomph_info << "Before manual refine: doc_info.number()=" << doc_info.number()
             << std::endl;
  doc_info.number()++;
  ...
  // Doc afterwards
  problem.doc_solution(doc_info);
  oomph_info << "After manual refine: doc_info.number()=" << doc_info.number()
             << std::endl;
  doc_info.number()++;

  // Setup timestepping
  double dt =
    Global_Parameters::T / double(Global_Parameters::Nstep_per_period);
