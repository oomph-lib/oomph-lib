  ...
  double flux(const double& t)
  {
    return Min_flux +
           (Max_flux - Min_flux) * 0.5 *
             (1.0 - cos(2.0 * MathematicalConstants::Pi * t / Period));
  }
                                                         ...
                                                         ofstream& trace)
{
  ...
  {
    trace << time << " " << Global_Physical_Variables::flux(time) << " "
          << tip_node_pt->x(0) << " " << tip_node_pt->x(1) << " "
          << tip_node_pt->dposition_dt(0) << " " << tip_node_pt->dposition_dt(1)
  ...
  unsigned bound = 4;
  sprintf(filename,
          "%s/fluid_boundary_elements_front_%i_on_proc%i.dat",
          doc_info.directory().c_str(),
          doc_info.number(),
          this->communicator_pt()->my_rank());
  some_file.open(filename);
  ...
  bound = 5;
  sprintf(filename,
          "%s/fluid_boundary_elements_back_%i_on_proc%i.dat",
          doc_info.directory().c_str(),
          doc_info.number(),
          this->communicator_pt()->my_rank());
  some_file.open(filename);
  ...
  sprintf(filename,
          "%s/wall_normal_%i_on_proc%i.dat",
          doc_info.directory().c_str(),
          doc_info.number(),
          this->communicator_pt()->my_rank());
  some_file.open(filename);
...
int main(int argc, char* argv[])
{
  ...
  FSIChannelWithLeafletProblem<
    ...
    problem(lleft, lright, hleaflet, htot, nleft, nright, ny1, ny2, x_0);
