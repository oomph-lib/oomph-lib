...
namespace Global_Physical_Variables
{
  ...
  void set_params()
  {
    cout << "\n\n======================================================"
         << std::endl;
    cout << "\nSetting parameters. \n\n";
    cout << "Prescribed: Square of Womersley number: Alpha_sq = " << Alpha_sq
         << std::endl;
    cout << "            Density ratio:         Density_ratio = "
         << Density_ratio << std::endl;
    cout << "            Wall thickness:                    H = " << H
         << std::endl;
    cout << "            Poisson ratio:                    Nu = " << Nu
         << std::endl;
    cout << "            Pressure perturbation:          Pcos = " << Pcos
         << std::endl;

    Q = 1.0 / 12.0 * pow(H, 3) / Alpha_sq;
    cout << "\nDependent:  Stress ratio:                      Q = " << Q
         << std::endl;

    Lambda_sq = 1.0 / 12.0 * pow(H, 3) * Density_ratio;
    cout << "            Timescale ratio:           Lambda_sq = " << Lambda_sq
         << std::endl;

    Re = Alpha_sq;
    cout << "            Reynolds number:                  Re = " << Re
         << std::endl;

    ReSt = Re;
    cout << "           Womersley number:                ReSt = " << ReSt
         << std::endl;
    cout << "\n======================================================\n\n"
         << std::endl;
  }
...
public:
  ...
  /// \short Update the problem specs after distribution:
                                  ...
                                  ofstream& trace_file)
{
  ...
  {
    doc_info.enable_doc();
    cout << "Full doc step " << doc_info.number() << " for time "
  ...
  else
  {
    ...
    cout << "Only trace for time " << time_stepper_pt()->time_pt()->time()
         << std::endl;
  }
  ...
  if (doc_info.is_doc_enabled())
  {
    ...
    sprintf(filename,
            "%s/new_wall_soln%i_on_proc%i.dat",
            doc_info.directory().c_str(),
            doc_info.number(),
            this->communicator_pt()->my_rank());
    // Open the output file
  ...
  trace_file << time_pt()->time()
             ...
             << Veloc_trace_node_pt->x(0) << " " << Veloc_trace_node_pt->x(1)
             << " " << Veloc_trace_node_pt->value(0) << " "
             << Veloc_trace_node_pt->value(1) << " "
             << Fluid_mesh_pt->nelement() << " " << ndof() << " "
             << Fluid_mesh_pt->nrefinement_overruled() << " "
             << Fluid_mesh_pt->max_error() << " " << Fluid_mesh_pt->min_error()
  ...
  Eps_ampl(eps_ampl), Pcos_initial(pcos_initial), Pcos_duration(pcos_duration)
{
  ...
  Wall_mesh_pt = new OneDLagrangianMesh<SOLID_ELEMENT>(
