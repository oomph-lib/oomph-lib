...
namespace ProblemParameters
{
  ...
  void get_exact_u(const Vector<double>& x, Vector<double>& u)
  {
    ...
    CRBond_Bessel::bessjyv(
      order_max_in, kr, order_max_out, &jv[0], &yv[0], &djv[0], &dyv[0]);
    ...
    for (unsigned i = N_fourier; i < N_terms; i++)
    {
      ...
      double p = Legendre_functions_helper::plgndr2(i, N_fourier, cos(theta));
      // Set exact solution
      u_ex += Coeff[i] * sqrt(MathematicalConstants::Pi / (2.0 * kr)) *
              (jv[i] + I * yv[i]) * p;
    }
      ...
      for (unsigned l = 0; l < n_node; l++)
      {
        ...
        if (local_eqn_real >= 0)
        {
          residuals[local_eqn_real] -=
            2.0 * Point_source_magnitude.real() * psi(l);
        }
        ...
        if (local_eqn_imag >= 0)
        {
          ...
          residuals[local_eqn_imag] -=
            2.0 * Point_source_magnitude.imag() * psi(l);
        }
      }
    }
...
void PMLFourierDecomposedHelmholtzProblem<ELEMENT>::complete_problem_setup()
{
  ...
  {
    cout << "Zero Dirichlet boundary condition has been applied on symmetry "
            "line\n";
...
//==================================================================
template<class ELEMENT>
void PMLFourierDecomposedHelmholtzProblem<
...
template<class ELEMENT>
PMLFourierDecomposedHelmholtzProblem<
  ELEMENT>::PMLFourierDecomposedHelmholtzProblem()
{
  ...
  boundary_id = 5;
  outer_boundary_line_pt[5] = new TriangleMeshCurviLine(
  ...
  DocInfo& doc_info)
{
  ...
  Bulk_mesh_pt->compute_error(
    some_file, ProblemParameters::get_exact_u, error, norm);
  some_file.close();
...
template<class ELEMENT>
void PMLFourierDecomposedHelmholtzProblem<
  ELEMENT>::create_flux_elements_on_inner_boundary()
{
...
int main(int argc, char** argv)
{
  ...
  PMLFourierDecomposedHelmholtzProblem<
    ProjectablePMLFourierDecomposedHelmholtzElement<
      PMLHelmholtzPointSourceElement<TPMLFourierDecomposedHelmholtzElement<3>>>>
    problem;
  ...
  // TPMLFourierDecomposedHelmholtzElement family.
  PMLFourierDecomposedHelmholtzProblem<TPMLFourierDecomposedHelmholtzElement<3>>
    problem;
