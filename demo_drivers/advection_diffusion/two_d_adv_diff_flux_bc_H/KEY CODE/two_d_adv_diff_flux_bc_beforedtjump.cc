...
namespace TanhSolnForAdvectionDiffusion
{
 ...
 {
  u[0]=1.0;//tanh(1.0-Alpha*(TanPhi*x[0]-x[1]));
 }

 /// Exact solution as a scalar
 void get_exact_u(const Vector<double>& x, double& u)
 {
  u=1.0;//tanh(1.0-Alpha*(TanPhi*x[0]-x[1]));
 }
...
//25/06/24: Commenting out flux-related parts...
...
//  Use this one for the funky unforced solution
 ...
 {
  wind[0]=0.0;//*sin(0.0*x[1]);
  wind[1]=4*(0.25-(x[0]-0.5)*(x[0]-0.5));//*cos(0.0*x[0]);
  //wind[1]=4*(1-(x[0])*(x[0]));
  //wind[1]=10.0;
 }
 
 
 // Swimming function, taken from Bearon et al.
 
 void swimming(const Vector<double> &x, Vector<double> &swim)

{
  //local shear	
   double sigma=x[0]*2.0*Peclet/(Beta*Beta);	
...
public:
 ...
 TwoMeshFluxAdvectionDiffusionProblem(
  GeneralisedAdvectionDiffusionEquations<2>::GeneralisedAdvectionDiffusionSourceFctPt source_fct_pt,
  GeneralisedAdvectionDiffusionEquations<2>::GeneralisedAdvectionDiffusionWindFctPt wind_fct_pt,
  GeneralisedAdvectionDiffusionEquations<2>::GeneralisedAdvectionDiffusionWindFctPt conserved_wind_fct_pt);
...
private:
 ...
 //void create_flux_elements(const int b, Mesh* const &bulk_mesh_pt,
 ...
 AdvectionDiffusionEquations<2>::AdvectionDiffusionWindFctPt Conserved_wind_fct_pt;
...
TwoMeshFluxAdvectionDiffusionProblem(
 ...
 GeneralisedAdvectionDiffusionEquations<2>::GeneralisedAdvectionDiffusionWindFctPt wind_fct_pt, 
 GeneralisedAdvectionDiffusionEquations<2>::GeneralisedAdvectionDiffusionWindFctPt conserved_wind_fct_pt)
 :  Source_fct_pt(source_fct_pt), Wind_fct_pt(wind_fct_pt), Conserved_wind_fct_pt(conserved_wind_fct_pt)
{ 
 // Setup "bulk" mesh
...
/* 25/06/24: Commenting out flux-related parts.
 ...
 //create_flux_elements(2,Bulk_mesh_pt,Surface_mesh_pt);
 //create_flux_elements(0,Bulk_mesh_pt,Surface_mesh_pt);
 //create_flux_elements(1,Bulk_mesh_pt,Surface_mesh_pt);
 //create_flux_elements(3,Bulk_mesh_pt,Surface_mesh_pt); */
 ...
 // 25/06/24: No Dirichlet boundaries, commenting out...
 /* unsigned n_bound = Bulk_mesh_pt->nboundary();
 for(unsigned b=0;b<n_bound;b++)
  {
   //Leave nodes on all boundaries free - Hakan
   // Original only had boundary 1
    //if (b!=2 && b!=0)
	//if (b==10)
    //{
 ...
 for(unsigned e=0;e<n_element;e++)
  {
   ...
   // Swimming component
   el_pt->conserved_wind_fct_pt() =  Conserved_wind_fct_pt;

   // Set the Peclet number
   el_pt->pe_pt() = &TanhSolnForAdvectionDiffusion::Peclet;
   
  //Set the Peclet Strouhal number
   el_pt->pe_st_pt() = &TanhSolnForAdvectionDiffusion::Peclet_St;
 ...
 // 25/06/24: Flux elements are incompatible with generalised advection diffusion
// elements. Related parts are commented out.
...
void TwoMeshFluxAdvectionDiffusionProblem<ELEMENT>::actions_before_newton_solve()
{
 ...
 // 25/06/24: No Dirichlet boundary conditions, commenting out...
...
/* template<class ELEMENT>
void TwoMeshFluxAdvectionDiffusionProblem<ELEMENT>::
create_flux_elements(const int b, Mesh* const &bulk_mesh_pt,
                     Mesh* const &surface_mesh_pt)
{
...
{
 // Kill the flux elements and wipe surface mes
 /////////////////////////////////////////////////////////////////////////
 // 25/06/24: Flux elements incompatible with generalised advection diffusion
 // elements. Related parts are commented out.
...
{
/*  // Create prescribed-flux elements from all elements that are 
 // adjacent to boundary 1 and add them to surface mesh
 //create_flux_elements(2,Bulk_mesh_pt,Surface_mesh_pt);
 //create_flux_elements(0,Bulk_mesh_pt,Surface_mesh_pt);
 //create_flux_elements(1,Bulk_mesh_pt,Surface_mesh_pt);
 //create_flux_elements(3,Bulk_mesh_pt,Surface_mesh_pt); */
...
int main()
{
 ...
 TwoMeshFluxAdvectionDiffusionProblem<RefineableQGeneralisedAdvectionDiffusionElement<2,
  3> > problem(&TanhSolnForAdvectionDiffusion::source_function,
               &TanhSolnForAdvectionDiffusion::wind_function,
			   &TanhSolnForAdvectionDiffusion::swimming);
 ...
 TanhSolnForAdvectionDiffusion::TanPhi=0.0;
 
 // Initial value for the steepness of the "step"
 TanhSolnForAdvectionDiffusion::Alpha=0.0; 
 ...
 for (unsigned istep=0;istep<nstep;istep++)
  {
   ...
   TanhSolnForAdvectionDiffusion::Alpha=double(0.0)*5.0;
