/* 24/06/24: This version of the code contains:
 - Gyrotactic motion (based on radial formulation)
 - Periodic boundaries (top and bottom)
 - Poiseuille-like flow
 - No pinned boundaries (no constant value on any boundary)
 
 This seems to be somewhat working (ie accumulation at the center/walls depending
 on flow direction).
 
 What this misses are:
 - Flux conditions. These are removed because they are not compatible with generalised
 elements. I moved the code to generalised advection diffusion elements because they have
 conserved wind functions (used for swimming)
 - Proper boundary conditions: The code does not pin any boundary, meaning no fixed value on
 any boundary. I don't know if this automatically translates to a no-flux condition because
 I don't define a no-flux either.
 - Non-negative concentration: Concentration values tend to negative at upwelling case. Also,
 the upwelling distribution appears when the flow appears to be downwelling. This may be related
 to the improper transition from cylindrical coordinates to two-dimensional
 - Correct gyrotactic expressions for two-dimensional geometry: Currently the expressions assume
 a cylindrical geometry. This is somewhat consistent with my geometry extending from 0 to 1 in x 
 (or r). The flow profile however is not consistent with this (Poiseuille with maximum at the center,
 x=0.5).
 - Custom diffusivity tensor: Currently it is isotropic.

Despite all this, it seems to be workable. Keeping you here as v.0 - Hakan
