...
namespace ParameterSpace
{
 ...
 void wind_function(const Vector<double>& x, Vector<double>& wind)
 {
  ...
  wind[1]=-2.0*(1-(x[0]-1)*(x[0]-1));//*cos(0.0*x[0]);
  //wind[1]=2.0*(1.0-x[0]*x[0]);
  //wind[1]=10.0;
 }
...
{
  //local shear	
   double sigma=-(x[0]-1.0)*2.0*Peclet/(Beta*Beta);	
...
int main()
{
 ...
 BiasedActiveMatterDispersionProblem<RefineableQGeneralisedAdvectionDiffusionElement<2,	
