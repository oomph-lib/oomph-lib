...
namespace TanhSolnForSteadyAxisymAdvectionDiffusion
{
                                           ...
                                           double &beta)
  {
    ...
    // The beta is (for the problem with bound1 = F(z) = R + epsilon*sin(z) with
    // 0<z<H=2*Pi)

    beta = -1.0 * (1.0 / (sqrt(1.0 + pow(Epsilon * cos(z), 2.0)))) *
           (1.0 / (1.0 - K_Alpha * tanh(f_rz))) * (1.0 - pow(tanh(f_rz), 2.0)) *
...
/// \short Constructor for SteadyAxisymAdvectionDiffusion problem: Pass
  ...
  Source_fct_pt(source_fct_pt), Wind_fct_pt(wind_fct_pt)
{
  ...
  for (unsigned i = 0; i < n_node; i++)
  {
    ...
    Bulk_mesh_pt->node_pt(i)->x(0) =
      r_value - (r_value / l_r) * epsilon * sin(z_value);
  ...
  for (unsigned e = 0; e < n_element; e++)
  {
    ...
    el_pt->beta_fct_pt() = &TanhSolnForSteadyAxisymAdvectionDiffusion::
                             prescribed_beta_on_fixed_r_boundary;
    // Set the pointer to the prescribed alpha function
    el_pt->alpha_fct_pt() = &TanhSolnForSteadyAxisymAdvectionDiffusion::
  ...
  DocInfo &doc_info)
{
  ...
  Bulk_mesh_pt->output_fct(
    some_file, npts, TanhSolnForSteadyAxisymAdvectionDiffusion::get_exact_u);
  some_file.close();
