...
namespace oomph
{
  ...
  namespace Problem_Parameter
  {
                     ...
                     Vector<double>& force)
    {
      ...
      {
        force[1] = -1.0 + Oscillating_body_force_multiplier * F_max *
                            sin(2.0 * MathematicalConstants::Pi *
                                (time - Initial_settling_time));
      }
      else
      {
        force[1] = -1.0;
...
#include "overloaded_element_body.h"
  ...
  //=======================================================================
  template<>
  class FaceGeometry<MyTaylorHoodElement> : public virtual SolidTElement<1, 3>
  {
  public:
...
public:
  ...
  double calculate_error_of_fixed_point_iteration()
  {
    ...
    for (unsigned e = 0; e < nel; e++)
    {
      ...
      el_pt->square_of_norm_of_fixed_point(
        el_norm_squared, el_latest_guess_norm_squared, el_error_norm_squared);
    ...
    oomph_info << "Norm of latest fixed point iteration guess for "
               << "strain rate invariant: " << sqrt(latest_guess_norm_squared)
               << std::endl;
    oomph_info << "Norm of error in fixed point iteration "
               << "strain rate invariant: " << sqrt(error_norm_squared)
               << " equivalent to ";
  ...
  double strainrate_norm()
  {
    ...
    for (unsigned e = 0; e < nel; e++)
    {
      ...
      test_size = el_pt->square_of_norm_of_strain_invariant(
        el_norm_squared, el_extrapolated_norm_squared, el_error_norm_squared);
  ...
  void restart()
  {
    ...
    restart_file_pt =
      new ifstream(Problem_Parameter::Restart_file.c_str(), ios_base::in);
    if (restart_file_pt != 0)
    {
      oomph_info << "Have opened " << Problem_Parameter::Restart_file.c_str()
    ...
    else
    {
      ...
      error_stream << "ERROR while trying to open "
                   << Problem_Parameter::Restart_file.c_str() << " for restart."
                   << std::endl;

      throw OomphLibError(
        error_stream.str(), "restart()", OOMPH_EXCEPTION_LOCATION);
    }
    ...
    if (restart_file_pt != 0)
    {
      ...
      Problem_Parameter::Doc_info_trace.number() =
        unsigned(atoi(input_string.c_str()));
...
AxisymmetricVibratingShellProblem<ELEMENT>::AxisymmetricVibratingShellProblem()
{
  ...
  boundary_curve_section_pt[1] =
    new TriangleMeshPolyLine(vertex_coord, Symmetry_line_id);
  ...
  TriangleMeshParameters triangle_mesh_parameters(Outer_boundary_polyline_pt);
  ...
  unsigned n_boundary_node =
    Fluid_mesh_pt->nboundary_node(Shell_wall_boundary_id);
...
void AxisymmetricVibratingShellProblem<ELEMENT>::complete_problem_setup()
{
  ...
  for (unsigned ibound = 0; ibound < nbound; ++ibound)
  {
    if ((ibound == Shell_wall_boundary_id) || (ibound == Symmetry_line_id))
    {
  ...
  else
  {
    ...
    oomph_info << "Central node now at: "
               ...
               << Central_node_on_free_surface_pt->x(1) << " " << std::endl;
  }
...
//========================================================================
template<class ELEMENT>
void AxisymmetricVibratingShellProblem<ELEMENT>::doc_solution(
  const std::string& comment)
{
  ...
  // hierher if(this->time_pt()->time() >= double(Count_doc)*Dt_doc)
  {
    ...
    sprintf(filename,
            ...
            Problem_Parameter::Doc_info_soln.number());
    some_file.open(filename);
    some_file << "-0.5 0.0 0.0 " << body_force[0] << " " << body_force[1] << " "
              << body_force[2] << " " << std::endl;
    some_file.close();
  ...
  for (unsigned e = 0; e < nel; e++)
  {
    ...
    {
      oomph_info << "Trouble: " << test_size << " " << size << std::endl;
    }
    ...
    el_pt->square_of_norm_of_viscosity(el_visc_norm_squared,
    ...
    el_pt->max_and_min_invariant_and_viscosity(
      el_min_invariant, el_max_invariant, el_min_viscosity, el_max_viscosity);
  ...
  oomph_info << "Norm of strain rate invariant: " << sqrt(norm_squared)
             << std::endl;
  ...
  oomph_info << "min_invariant = " << min_invariant << "\n"
             ...
             << "max_viscosity = " << max_viscosity << std::endl;
...
//========================================================================
template<class ELEMENT>
void AxisymmetricVibratingShellProblem<ELEMENT>::compute_error_estimate(
...
int main(int argc, char** argv)
{
  ...
  // BDF TYPE (1,2 or 4)
  CommandLineArgs::specify_command_line_flag("--bdf_type",
  ...
  CommandLineArgs::specify_command_line_flag("--min_ntstep_per_period",
                                             &min_ntstep_per_period);

  // Initial number of steps period
  unsigned initial_ntstep_per_period = 40;
  CommandLineArgs::specify_command_line_flag("--initial_ntstep_per_period",
                                             &initial_ntstep_per_period);

  // Manual adaptation?
  unsigned nadapt_interval = 1;
  CommandLineArgs::specify_command_line_flag("--manual_adapt",
                                             &nadapt_interval);

  // Uniform element area
  CommandLineArgs::specify_command_line_flag(
    "--uniform_element_area", &Problem_Parameter::Uniform_element_area);

  // Use Newtonian constitutive equation (and specify its
  // viscosity ratio)
  double newtonian_viscosity_ratio = 1.0;
  CommandLineArgs::specify_command_line_flag(
    "--use_newtonian_constitutive_equation", &newtonian_viscosity_ratio);
             ...
             "--use_casson_constitutive_equation"))
  {
    oomph_info << "Using Casson constitutive equation"
               ...
               << Problem_Parameter::Critical_strain_rate << std::endl;
    Problem_Parameter::Const_eqn_pt =
  ...
  // Open trace file
  Problem_Parameter::Trace_file.open(
    (Problem_Parameter::Directory + "/trace.dat").c_str());
  ...
  // Open norm file
  Problem_Parameter::Norm_file.open((Problem_Parameter::Directory + "/norm.dat")
                                      .c_str()); // hierher needed? Maybe keep
  ...
  constitutive_file.open(
    (Problem_Parameter::Directory + "/constitutive.dat").c_str());
  ...
  oomph_info << "Tolerance for adaptive timestepping: epsilon_t = " << epsilon_t
             << std::endl;
  ...
  else
  {
    ...
    oomph_info << "Doing " << nstep << " steps with constant timestep and mesh "
               << std::endl;
    ...
    while (problem.time_pt()->time() < t_max)
    {
      ...
      dt_new =
        problem.adaptive_unsteady_newton_solve(problem.next_dt(), epsilon_t);
      ...
      if (dt_new < Problem_Parameter::Dt_min)
      {
        ...
        cout << "dt limited (from below) to: " << Problem_Parameter::Dt_min
             << endl;
      }
      problem.next_dt() = dt_new;
          ...
          CommandLineArgs::command_line_flag_has_been_set("--validation"))
      {
        ...
        while (error_fixed_point_iteration > fixed_point_tol)
        {
          ...
          oomph_info << "FIXED POINT RE-SOLVE " << i_fp
                     << " FOR t = " << problem.time_pt()->time() << "\n";
  ...
  // Now do temporally adaptive solves, explicitly adaptiving every specified
  // number
  //--------------------------------------------------------------------------------
  // of steps
  //---------
  ...
  else
  {
    ...
    bool first = false;
    dt_new = problem.doubly_adaptive_unsteady_newton_solve(
      problem.next_dt(), epsilon_t, max_adapt, first);
