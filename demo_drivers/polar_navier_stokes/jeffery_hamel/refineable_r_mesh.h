...
namespace oomph
{
  ...
  private:
    void stretch_mesh()
    {
      ...
      double cut =
        R_min + (R_max - R_min) * ((static_cast<double>(Nx - end)) / Nx);

      // Now spread those final 7% of elements out over the final 10% of the
      // domain
      double a = cut;
      ...
      if (Global_Physical_Variables::new_outlet_region)
      {
        ...
        std::cout << std::endl
                  << "Number of end elements in r: " << end << std::endl;
        std::cout << "Start of outlet region: " << cut << std::endl
                  << std::endl;
      }
                          ...
                          double R_max)
    {
      if ((R_max - a) < 1.e-14)
      {
        std::cout << "Warning - no elements in outlet region.  Attempting to "
                     "divide by zero"
                  << std::endl;
      }
