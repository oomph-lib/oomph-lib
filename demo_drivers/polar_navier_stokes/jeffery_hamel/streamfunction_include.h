...
namespace oomph
{
  ...
  class streamfunction_mesh :
    public virtual Refineable_r_mesh<RefineablePolarStreamfunctionElement>
  {
  protected:
  ...
  public:
    ...
    void make_traction_elements(const bool& outlet)
    {
      ...
      for (unsigned ielt = 0; ielt < num_elt; ielt++)
      {
        ...
        if (outlet)
        {
          this->Outlet_traction_elt_pt.push_back(surface_element_pt);
        }
        else
        {
          this->Inlet_traction_elt_pt.push_back(surface_element_pt);
        }
      ...
      std::cout
        << std::endl
        << "Streamfunction traction elements attached to streamfunction_mesh"
        << std::endl
        << std::endl;
    ...
    void remove_traction_elements()
    {
      ...
      std::cout
        << std::endl
        << "Streamfunction traction elements removed from streamfunction_mesh"
        << std::endl
        << std::endl;
    ...
    void assign_fluid_element_vector()
    {
      ...
      if (check != 0)
      {
        ...
        std::cout
          << "whilst streamfunction traction elements are attached to the mesh"
          << std::endl;
      }
      else
      {
  ...
  public:
    ...
    /// After adaptation: Unpin all pressures and then pin redudant pressure
    /// dofs.
  ...
  StreamfunctionProblem::StreamfunctionProblem(const bool& eigen)
  {
    ...
    // Problem::mesh_pt() = new
    // streamfunction_mesh<RefineablePolarStreamfunctionElement>(n_x,n_y);
                                       ...
                                       double& stream)
  {
    ...
    for (unsigned long e = 0; e < n_element; e++)
    {
      ...
      if ((x_lower_left[0] <= x_to_get[0]) && (x_lower_left[1] <= x_to_get[1]))
      {
            ...
            (x_upper_right[1] > x_to_get[1]))
        {
          ...
          {
            if (abs(a + c - 2. * b) < 1.e-8)
              std::cout << "Element almost linear: " << abs(a + c - 2. * b)
                        << std::endl;
            std::cout << "error in interpolation: " << (x[0] - x_to_get[0])
                      << std::endl;
            std::cout << "r:" << x[0] << "r to get: " << x_to_get[0]
                      << std::endl;
          }
  ...
  //========================================================================
  void StreamfunctionProblem::my_output(const int radial,
                                        const int azimuthal,
                                        bool log_output,
                                        string file_name)
  {
    ...
    out << "# Streamfunction output at " << radial << " radial and "
        << azimuthal << " azimuthal points" << std::endl;
    out << "# Re = " << Global_Physical_Variables::Re
        << " Alpha = " << Global_Physical_Variables::Alpha
        << " R_l = " << Global_Physical_Variables::R_l << std::endl;
    out << "# Initial xmesh: " << Global_Physical_Variables::xmesh
        << " Initial ymesh " << Global_Physical_Variables::ymesh << std::endl;
    out << "# Uniform refinements: " << Global_Physical_Variables::uniform
        << " Adaptive refinements: " << Global_Physical_Variables::adaptive
        << std::endl;
    out << "# log_mesh = " << Global_Physical_Variables::log_mesh
        << " new_outlet_region = "
        << Global_Physical_Variables::new_outlet_region << std::endl;
    out << "# Inlet traction: " << Global_Physical_Variables::inlet_traction
        << " Outlet traction: " << Global_Physical_Variables::outlet_traction
        << std::endl;
    out << "# eta_inlet: " << Global_Physical_Variables::eta_inlet
    ...
    std::cout << endl;
    std::cout << "Outputting streamfunction to file " << file_name << " at "
              << radial << " radial plot points and " << azimuthal
    ...
    if (log_output)
    {
      for (int i = 0; i < radial; i++)
      {
        ...
        for (int j = 0; j < azimuthal; j++)
        {
          ...
          // Output to file
          out << position[0] << " " << position[1] << " " << stream
              << std::endl;
  ...
  void StreamfunctionProblem::header(ofstream& some_file)
  {
    some_file << "# Refineable streamfunction mesh"
              << "\n";
    some_file << "# Re = " << Global_Physical_Variables::Re
              << " Alpha = " << Global_Physical_Variables::Alpha
    ...
    some_file << "# Uniform refinements: " << Global_Physical_Variables::uniform
              ...
              << Global_Physical_Variables::adaptive << "\n";
    some_file << "# inlet_traction = "
              << Global_Physical_Variables::inlet_traction
              << " eta_inlet = " << Global_Physical_Variables::eta_inlet
              << "\n";
    some_file << "# outlet_traction = "
              << Global_Physical_Variables::outlet_traction
              << " eta_outlet = " << Global_Physical_Variables::eta_outlet
              << "\n";
    some_file << "# log_mesh = " << Global_Physical_Variables::log_mesh
              << " new_outlet_region = "
              << Global_Physical_Variables::new_outlet_region << "\n";
