...
namespace oomph
{
  ...
  public:
    ...
    virtual inline unsigned u_index_streamfunction() const
    {
      return 0;
    }
  ...
  public:
    ...
    PolarStreamfunctionElement() :
      QElement<2, 3>(), PolarStreamfunctionEquations()
    {
    }
    ...
    Vector<double>& residuals, DenseMatrix<double>& jacobian, unsigned flag)
  {
    ...
    {
      u_nodal_index[i] = u_index_velocity(i);
    }
    ...
    for (unsigned ipt = 0; ipt < n_intpt; ipt++)
    {
      ...
      for (unsigned l = 0; l < n_node; l++)
      {
        ...
        if (local_eqn >= 0)
        {
          ...
          residuals[local_eqn] += interpolated_dsdx[0] * dtestdx(l, 0) *
                                  interpolated_x[0] * Alpha * W;
          residuals[local_eqn] += (1. / (interpolated_x[0] * Alpha)) *
                                  interpolated_dsdx[1] *
                                  (1. / (interpolated_x[0] * Alpha)) *
                                  dtestdx(l, 1) * interpolated_x[0] * Alpha * W;

          // Should equal the vorticity
          residuals[local_eqn] -= (interpolated_dudx(1, 0) +
                                   (interpolated_u[1] / interpolated_x[0])) *
                                  test(l) * interpolated_x[0] * Alpha * W;
          residuals[local_eqn] += (1. / (interpolated_x[0] * Alpha)) *
                                  interpolated_dudx(0, 1) * test(l) *
                                  interpolated_x[0] * Alpha * W;
          ...
          if (flag)
          {
            ...
            for (unsigned l2 = 0; l2 < n_node; l2++)
            {
              ...
              if (local_unknown >= 0)
              {
                ...
                jacobian(local_eqn, local_unknown) +=
                  (1. / (interpolated_x[0] * Alpha)) * dpsidx(l2, 1) *
                                            ...
                                            const unsigned& nplot)
  {
    ...
    for (unsigned iplot = 0; iplot < num_plot_points; iplot++)
    {
      ...
      outfile << interpolated_streamfunction(s) << " "
              << interpolated_vorticity(s) << " " << interpolated_velocity(s, 0)
