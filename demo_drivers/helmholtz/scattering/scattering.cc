...
namespace GlobalParameters
{
  ...
  void get_exact_u(const Vector<double> &x, Vector<double> &u)
  {
    ...
    {
      u_ex -=
        pow(I, i) * h[i] * ((jnp_a[i]) / hp_a[i]) * pow(exp(I * theta), i);
    }
...
ScatteringProblem<ELEMENT>::ScatteringProblem()
{
  ...
  // Build "bulk" mesh
  Bulk_mesh_pt = new RefineableTwoDAnnularMesh<ELEMENT>(
    periodic, azimuthal_fraction, n_theta, n_r, a, h);
  ...
  // Build "bulk" mesh
  Bulk_mesh_pt = new TwoDAnnularMesh<ELEMENT>(
    periodic, azimuthal_fraction, n_theta, n_r, a, h);
...
int main(int argc, char **argv)
{
  ...
  ScatteringProblem<RefineableQHelmholtzElement<2, 3>> problem;

#else

  // Set up the problem with 2D nine-node elements from the
  // QHelmholtzElement family.
  ScatteringProblem<QHelmholtzElement<2, 3>> problem;
