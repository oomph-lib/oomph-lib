      ...
      for (unsigned edge = 0; edge < 3; edge++)
      {
        ...
        if (nod_pt->is_on_boundary(ibound))
        {
          ...
          for (unsigned g = 0; g < n_flux_interpolation_points; g++)
          {
            ...
            Vector<double> s(1);
            el_pt->face_local_coordinate_of_flux_interpolation_point(
              edge, g, s);
                ...
                1.0e-3)
            {
              ...
              error << "Discrepancy in coordinate of flux interpolation point\n"
                    ...
                    << " and flux int pt " << g << "\n"
                    << "Face thinks node is at: " << x_face[0] << " "
                    << x_face[1] << "\n"
                    << "Bulk thinks node is at: " << x[0] << " " << x[1]
                    << "\n";
              throw OomphLibError(
                error.str(), OOMPH_CURRENT_FUNCTION, OOMPH_EXCEPTION_LOCATION);
            }
#endif
...
void DarcyProblem<ELEMENT>::doc_shape_functions()
{
  ...
  for (unsigned j = 0; j < n_q_basis_edge; j++)
  {
    ...
    sprintf(filename,
            "%s/outer_unit_normal%i.dat",
            TestProblem::Directory.c_str(),
            j);
    some_file.open(filename);
    some_file << x[0] << " " << x[1] << " " << unit_normal[0] << " "
              << unit_normal[1] << " " << std::endl;
    some_file.close();

    sprintf(filename,
            "%s/flux_interpolation_point%i.dat",
            TestProblem::Directory.c_str(),
            j);
    some_file.open(filename);
    ...
    some_file << flux_interpolation_point[0] << " "
              << flux_interpolation_point[1] << " " << 1.0 << " " << std::endl;
    some_file.close();

    sprintf(filename,
            "%s/q_shape_fct_with_project%i.dat",
            TestProblem::Directory.c_str(),
            j);
    some_file.open(filename);
    for (unsigned e = 0; e < nel; e++)
...
void DarcyProblem<ELEMENT>::doc_solution(const unsigned &label)
{
  ...
  unsigned npts_coarse = 2;
  sprintf(
    filename, "%s/coarse_soln%i.dat", TestProblem::Directory.c_str(), label);
  some_file.open(filename);
  Bulk_mesh_pt->output(some_file, npts_coarse);
  some_file.close();
  ...
  //----------------------
  sprintf(
    filename, "%s/exact_soln%i.dat", TestProblem::Directory.c_str(), label);
  some_file.open(filename);
  Bulk_mesh_pt->output_fct(some_file, npts, TestProblem::exact_soln);
  some_file.close();

  // Output boundary condition elements
  //-----------------------------------
  sprintf(
    filename, "%s/bc_elements%i.dat", TestProblem::Directory.c_str(), label);
  some_file.open(filename);
  ...
  some_file.open(filename);
  Bulk_mesh_pt->compute_error(some_file, TestProblem::exact_soln, error, norm);
  some_file.close();
  ...
  Trace_file << ndof() << " " << sqrt(error[0]) << " " << sqrt(error[1]) << " "
             << TestProblem::Element_area << " " << sqrt(norm[0]) << " "
...
int main(int argc, char **argv)
{
  ...
  // Output directory
  CommandLineArgs::specify_command_line_flag("--dir", &TestProblem::Directory);
