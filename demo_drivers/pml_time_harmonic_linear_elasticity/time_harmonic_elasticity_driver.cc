...
namespace Global_Parameters
{
  ...
  void exact_u(const Vector<double>& x, Vector<double>& u)
  {
    ...
    u[0] =
      sol1_switch * u_r_real * x[0] / r - sol2_switch * u_phi_real * x[1] / r;

    // Real part of y displacement
    u[1] =
      sol1_switch * u_r_real * x[1] / r + sol2_switch * u_phi_real * x[0] / r;

    // Imag part of x displacement
    u[2] =
      sol1_switch * u_r_imag * x[0] / r - sol2_switch * u_phi_imag * x[1] / r;

    // Imag part of y displacement
    u[3] =
      sol1_switch * u_r_imag * x[1] / r + sol2_switch * u_phi_imag * x[0] / r;
  }
...
ElasticAnnulusProblem<ELASTICITY_ELEMENT>::ElasticAnnulusProblem()
{
  ...
  outer_boundary_line_pt[0] =
    new TriangleMeshPolyLine(vertex_coord, boundary_id);
  ...
  inner_boundary_line_pt[0] = new TriangleMeshCurviLine(
    inner_circle_pt, s_start, s_end, n_segments, boundary_id);
  ...
  // Build the mesh
  Solid_mesh_pt =
    new RefineableTriangleMesh<ELASTICITY_ELEMENT>(triangle_mesh_parameters);
  ...
  // Build the mesh
  Solid_mesh_pt =
    new TriangleMesh<ELASTICITY_ELEMENT>(triangle_mesh_parameters);
...
void ElasticAnnulusProblem<ELASTICITY_ELEMENT>::doc_solution(DocInfo& doc_info)
{
  ...
  double error, norm;
  sprintf(filename,
          "%s/error_and_norm%i.dat",
          Doc_info.directory().c_str(),
          Doc_info.number());
  Solid_mesh_pt->compute_error(
    some_file, Global_Parameters::exact_u, error, norm);
  some_file.open(filename);
  ...
  sprintf(filename,
          "%s/wall_clock_time%i.dat",
          Doc_info.directory().c_str(),
          Doc_info.number());
  some_file.open(filename);
  some_file << Global_Parameters::T_end - Global_Parameters::T_start
            << std::endl;
  some_file.close();

  // Output number of degrees of freedom in a file
  sprintf(
    filename, "%s/ndof%i.dat", Doc_info.directory().c_str(), Doc_info.number());
  some_file.open(filename);
...
void ElasticAnnulusProblem<ELASTICITY_ELEMENT>::create_pml_meshes()
{
  ...
  PML_right_mesh_pt = TwoDimensionalPMLHelper::create_right_pml_mesh<
    PMLLayerElement<ELASTICITY_ELEMENT>>(Solid_mesh_pt,
                                         right_boundary_id,
  ...
  PML_left_mesh_pt = TwoDimensionalPMLHelper::create_left_pml_mesh<
    PMLLayerElement<ELASTICITY_ELEMENT>>(Solid_mesh_pt,
                                         left_boundary_id,
  ...
  PML_top_right_mesh_pt = TwoDimensionalPMLHelper::create_top_right_pml_mesh<
    PMLLayerElement<ELASTICITY_ELEMENT>>(
    PML_right_mesh_pt, PML_top_mesh_pt, Solid_mesh_pt, right_boundary_id);

  PML_bottom_right_mesh_pt =
    TwoDimensionalPMLHelper::create_bottom_right_pml_mesh<
      PMLLayerElement<ELASTICITY_ELEMENT>>(
      PML_right_mesh_pt, PML_bottom_mesh_pt, Solid_mesh_pt, right_boundary_id);

  PML_top_left_mesh_pt = TwoDimensionalPMLHelper::create_top_left_pml_mesh<
    PMLLayerElement<ELASTICITY_ELEMENT>>(
    PML_left_mesh_pt, PML_top_mesh_pt, Solid_mesh_pt, left_boundary_id);

  PML_bottom_left_mesh_pt =
    TwoDimensionalPMLHelper::create_bottom_left_pml_mesh<
      PMLLayerElement<ELASTICITY_ELEMENT>>(
      PML_left_mesh_pt, PML_bottom_mesh_pt, Solid_mesh_pt, left_boundary_id);
...
int main(int argc, char** argv)
{
  ...
  CommandLineArgs::specify_command_line_flag(
    "--n_pml", &Global_Parameters::N_pml_multiplier);

  // Over-write PML layers physical length in each dimension
  CommandLineArgs::specify_command_line_flag(
    "--l_pml", &Global_Parameters::L_pml_multiplier);
  ...
  Global_Parameters::E_pt =
    new PMLTimeHarmonicIsotropicElasticityTensor(Global_Parameters::Nu);

#ifdef ADAPTIVE

  // Set up the problem
  ElasticAnnulusProblem<ProjectablePMLTimeHarmonicLinearElasticityElement<
  ...
  // Set up the problem
  ElasticAnnulusProblem<TPMLTimeHarmonicLinearElasticityElement<2, 3>> problem;
