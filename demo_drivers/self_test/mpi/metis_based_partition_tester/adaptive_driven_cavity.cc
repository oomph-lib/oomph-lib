...
int main(int argc, char **argv)
{
 ...
 // Number of elements before distribution
 unsigned nelem_total=problem.mesh_pt()->nelement();
 ...
 // check
 {
  ...
  // Number of non-halo elements after distribution (as int for MPI sum)
  int nelem_nonhalo=int(problem.mesh_pt()->nnon_halo_element());
  
  // Get my rank
  int my_rank=MPI_Helpers::communicator_pt()->my_rank();
  
  // Each MPI process sends its number of non-halo elemements,
  // root MPI process collects the result
  int ntotal_non_halo = 0;
  int root_rank=0;
  unsigned nitem=1;
  MPI_Reduce(&nelem_nonhalo, &ntotal_non_halo, nitem, 
             MPI_INT,MPI_SUM,root_rank,
             MPI_Helpers::communicator_pt()->mpi_comm());
  
  // Check that we haven't lost any elements (though we don't care where
  // they are. Metis is non-deterministic.
  if (my_rank==root_rank)
   {
    oomph_info << "ntotal_non_halo, nelem_total: " << ntotal_non_halo
               << " " << nelem_total << std::endl;
    ofstream some_file;
    some_file.open("RESLT/passed_check.dat");
    some_file << "ntotal_non_halo, nelem_total: " << ntotal_non_halo
              << " " << nelem_total << " ";
    if (ntotal_non_halo!=nelem_total)
     {
      some_file << "Something's gone wrong in the distribution!" << std::endl;
     }
    else
     {
      some_file << "Distribution worked; yay!" << std::endl;
     }
    some_file.close();
   }
 }
 ...
 mesh_doc_info.set_directory("RESLT_load_balance");
 ...
 // check
 {
  ...
  // Total number of elements after uniform refinement
  nelem_total*=4;
  ...
  if (my_rank==root_rank)
   {
    oomph_info << "ntotal_non_halo, nelem_total: " << ntotal_non_halo
               << " " << nelem_total << std::endl;
    ofstream some_file;
    some_file.open("RESLT_load_balance/passed_check.dat");
    some_file << "ntotal_non_halo, nelem_total: " << ntotal_non_halo
              << " " << nelem_total << " ";
    if (ntotal_non_halo!=nelem_total)
     {
      some_file << "Something's gone wrong in the load balancing!" << std::endl;
     }
    else
     {
      some_file << "Load balancing worked; yay!" << std::endl;
     }
    some_file.close();
   }
 }
