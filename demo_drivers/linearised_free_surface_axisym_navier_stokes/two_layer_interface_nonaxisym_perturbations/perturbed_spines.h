...
namespace oomph
{
  ...
  public:
      ...
      Base_spine_pt(base_spine_pt)
    {
      ...
      {
        Geom_data_pt[0]->set_value(i, height);
      }
    }
  ...
  public:
    ...
    {
      return PerturbedSpine_mesh_pt;
    }

    /// Access function to i-th component of spine "height"
    double& h(const unsigned& i)
    {
      return PerturbedSpine_pt->height(i);
    }
    ...
    unsigned ngeom_data() const
    {
      ...
      {
        return PerturbedSpine_pt->ngeom_data();
      }
      else
      {
        return 0;
    ...
    unsigned ngeom_object() const
    {
      if (PerturbedSpine_pt)
      {
        return PerturbedSpine_pt->ngeom_object();
      }
      else
      {
        return 0;
    ...
    {
      return &(PerturbedSpine_pt->geom_data_pt(0));
    }
    ...
    inline int spine_local_eqn(const unsigned& n, const unsigned& i)
    {
      ...
      if (i >= 2)
      {
        ...
        error_message << "Perturbed spines only have two height components:"
                      << "\nComponent i=" << i << " is not in the range (0,2)";
        throw OomphLibError(error_message.str(),
                            "PerturbedSpineElement::spine_local_eqn()",
                            OOMPH_EXCEPTION_LOCATION);
      }
      ...
      if (PerturbedSpine_geometric_index[n] == this->ngeom_data())
      {
        ...
        error_stream << "PerturbedSpineNode " << n
                     << " does not have a PerturbedSpine\n"
                     << "attached, so you can't get its local equation number. "
                        "Check that\n"
                     << "the Mesh is correctly associating PerturbedSpines "
                        "with is Nodes\n";
...
#endif
      return this->geometric_data_local_eqn(PerturbedSpine_geometric_index[n],
                                            i);
    }
  ...
  public:
    ...
    virtual ~PerturbedSpineMesh()
    {
      ...
      {
        delete PerturbedSpine_pt[i - 1];
        PerturbedSpine_pt[i - 1] = 0;
      }
    }
    ...
    virtual void perturbed_spine_node_update(
      PerturbedSpineNode* spine_node_pt) = 0;
