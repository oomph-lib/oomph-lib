      ...
      Vector<double>& residuals, DenseMatrix<double>& jacobian, unsigned flag)
  {
    ...
    for (unsigned ipt = 0; ipt < n_intpt; ipt++)
    {
      ...
      for (unsigned l = 0; l < n_node; l++)
      {
        ...
        interpolated_dRCds += this->perturbation(l, 0) * dpsifds_;
        interpolated_dRSds += this->perturbation(l, 1) * dpsifds_;
        interpolated_dZCds += this->perturbation(l, 2) * dpsifds_;
        interpolated_dZCds += this->perturbation(l, 3) * dpsifds_;
      }

      interpolated_RC = interpolated_x[0];
      interpolated_RS = interpolated_x[0];
      interpolated_ZC = interpolated_x[1];
      interpolated_ZS = interpolated_x[1];

      interpolated_dRCdt = interpolated_dx_dt[0];
      interpolated_dRSdt = interpolated_dx_dt[0];
      interpolated_dZCdt = interpolated_dx_dt[1];
      interpolated_dZSdt = interpolated_dx_dt[1];
      ...
      for (unsigned l = 0; l < n_node; l++)
      {
        ...
        if (local_eqn >= 0)
        {
          residuals[local_eqn] -= (Sigma / Ca) *
                                  (interpolated_t1[0] * interpolated_dRCds +
                                   interpolated_t1[1] * interpolated_dZCds) *
                                  testf_ * w;
          residuals[local_eqn] -=
            (Sigma / Ca) *
            (interpolated_t1[0] * interpolated_RC +
             r / J * interpolated_t1[1] *
               (interpolated_t1[1] * interpolated_dRCds -
                interpolated_t1[0] * interpolated_dZCds)) *
            dtestfds_ * w;
          residuals[local_eqn] -= (Sigma / Ca) * k * k * J / r *
                                  interpolated_t1[1] *
                                  (interpolated_t1[1] * interpolated_RC -
                                   interpolated_t1[0] * interpolated_ZC) *
                                  testf_ * w;
          ...
          if (flag)
          {
            ...
            for (unsigned l2 = 0; l2 < n_node; l2++)
            {
              ...
              if (local_unknown >= 0)
              {
                ...
                jacobian(local_eqn, local_unknown) +=
                  (Sigma / Ca) * k * k * J * interpolated_t1[1] *
                  interpolated_t1[0] * psif_ * testf_ * w / r;
              }
        ...
        {
          residuals[local_eqn] -= (Sigma / Ca) *
                                  (interpolated_t1[0] * interpolated_dRSds +
                                   interpolated_t1[1] * interpolated_dZSds) *
                                  testf_ * w;
          residuals[local_eqn] -=
            (Sigma / Ca) *
            (interpolated_t1[0] * interpolated_RS +
             r / J * interpolated_t1[1] *
               (interpolated_t1[1] * interpolated_dRSds -
                interpolated_t1[0] * interpolated_dZSds)) *
            dtestfds_ * w;
          residuals[local_eqn] -= (Sigma / Ca) * k * k * J / r *
                                  interpolated_t1[1] *
                                  (interpolated_t1[1] * interpolated_RS -
                                   interpolated_t1[0] * interpolated_ZS) *
                                  testf_ * w;
            ...
            for (unsigned l2 = 0; l2 < n_node; l2++)
            {
              ...
              if (local_unknown >= 0)
              {
                jacobian(local_eqn, local_unknown) -=
                  ...
                  dpsifds_ * dtestfds_ * w / J;
                jacobian(local_eqn, local_unknown) -=
                  (Sigma / Ca) * k * k * J * interpolated_t1[0] *
                  interpolated_t1[0] * psif_ * testf_ * w / r;
              }
        ...
        if (local_eqn >= 0)
        {
          residuals[local_eqn] -= (Sigma / Ca) * 2.0 * k * J / (r * r) *
                                  interpolated_t1[1] *
                                  (interpolated_t1[0] * interpolated_ZS -
                                   interpolated_t1[1] * interpolated_RS) *
                                  testf_ * w;
          residuals[local_eqn] += (Sigma / Ca) * k / r *
                                  (interpolated_t1[0] * interpolated_RS +
                                   interpolated_t1[1] * interpolated_ZS) *
                                  dtestfds_ * w;
          residuals[local_eqn] += (Sigma / Ca) * k / r *
                                  (interpolated_t1[0] * interpolated_dRSds +
                                   interpolated_t1[1] * interpolated_dZSds -
          ...
          if (flag)
          {
            ...
            for (unsigned l2 = 0; l2 < n_node; l2++)
            {
              ...
              {
                jacobian(local_eqn, local_unknown) -=
                  (Sigma / Ca) * 2.0 * k * J * interpolated_t1[1] *
                  interpolated_t1[0] * psif_ * testf_ * w / (r * r);
        ...
        if (local_eqn >= 0)
        {
          ...
          residuals[local_eqn] -= (Sigma / Ca) * k / r *
                                  (interpolated_t1[0] * interpolated_RC +
                                   interpolated_t1[1] * interpolated_ZC) *
                                  dtestfds_ * w;
          residuals[local_eqn] -= (Sigma / Ca) * k / r *
                                  (interpolated_t1[0] * interpolated_dRCds +
                                   interpolated_t1[1] * interpolated_dZCds -
    ...
    for (unsigned ipt = 0; ipt < n_intpt; ipt++)
    {
      ...
      for (unsigned l = 0; l < n_node; l++)
      {
        ...
        if (local_eqn >= 0)
        {
          ...
          residuals[local_eqn] += (Sigma / Ca) * r * interpolated_t1[1] *
                                  interpolated_t1[0] * interpolated_dHCds *
                                  dtestfds_ * w / J;
          residuals[local_eqn] += (Sigma / Ca) * k * k * J *
                                  interpolated_t1[1] * interpolated_t1[0] *
        ...
        if (local_eqn >= 0)
        {
          ...
          residuals[local_eqn] += (Sigma / Ca) * r * interpolated_t1[1] *
                                  interpolated_t1[0] * interpolated_dHSds *
                                  dtestfds_ * w / J;
          residuals[local_eqn] += (Sigma / Ca) * k * k * J *
                                  interpolated_t1[1] * interpolated_t1[0] *
        ...
        if (local_eqn >= 0)
        {
          residuals[local_eqn] -= (Sigma / Ca) * r * interpolated_t1[0] *
                                  interpolated_t1[0] * interpolated_dHCds *
                                  dtestfds_ * w / J;
          residuals[local_eqn] -= (Sigma / Ca) * k * k * J *
        ...
        if (local_eqn >= 0)
        {
          ...
          residuals[local_eqn] += (Sigma / Ca) * k * interpolated_t1[1] *
                                  interpolated_HS * dtestfds_ * w / r;
          residuals[local_eqn] += (Sigma / Ca) * k * interpolated_t1[1] *
                                  interpolated_dHSds * testf_ * w / r;
        ...
        if (local_eqn >= 0)
        {
          ...
          residuals[local_eqn] -= (Sigma / Ca) * k * interpolated_t1[1] *
                                  interpolated_HC * dtestfds_ * w / r;
          residuals[local_eqn] -= (Sigma / Ca) * k * interpolated_t1[1] *
                                  interpolated_dHCds * testf_ * w / r;
        ...
        if (local_eqn >= 0)
        {
          ...
          residuals[local_eqn] -= k * interpolated_utheta * interpolated_t1[0] *
                                  interpolated_HS * testf_ * w * J;
