...
#ifndef CA_HEIGHT_CONTINUATION_AXISYM_DYNAMIC_CAP_PROBLEM_HEADER
#define CA_HEIGHT_CONTINUATION_AXISYM_DYNAMIC_CAP_PROBLEM_HEADER
...
namespace oomph
{
  ...
  class CaHeightContinuationAxisymDynamicCapProblem : public MyEigenproblem
  {
  public:
    ...
    // Constructor
    CaHeightContinuationAxisymDynamicCapProblem(const double& contact_angle,
                                                const bool& has_restart)
      : Ca(1.0), // Initialise parameters
        Bo(1.0),
        Re(0.0),
        ReSt(0.0),
        Viscosity_ratio(1.0),
  ...
  private:
    ...
    void create_bulk_mesh()
    {
      ...
      // 0.08.
      // If `d` is the argument, the radius of curvature is = 0.5/d * sqrt(d^2 +
      // 0.5^2)
      Outer_boundary_polyline_pt->set_polyline_refinement_tolerance(
        Mesh_Control_Parameters::Polyline_refinement_tolerence);

      // Set a measure of the minimum local curvature before unrefining. Default
      // = 0.04. If `d` is the argument, the radius of curvature is = 0.5/d *
      // sqrt(d^2 + 0.5^2)
    ...
    void create_circular_domain()
    {
      ...
      // The rest of the polylines only have two vertices -- provide storage for
      // their coordinates
      Vector<Vector<double>> vertex_coord(2);
  ...
  public:
    ...
    // void actions_after_change_in_global_parameter(double* const&
    // parameter_pt)
    // {
    //   oomph_info << "actions_after_change_in_global_parameter" << std::endl;
    //   oomph_info << "ReInvFr : " << this->ReInvFr << std::endl;
    //   oomph_info << "&ReInvFr : " << &this->ReInvFr << std::endl;
    //   oomph_info << "parameter_pt : " << parameter_pt << std::endl;
    //   oomph_info << "*parameter_pt : " << *parameter_pt << std::endl;
    // }

    // void set_continuation_parameter_pt()
    //{
    //   Parameter_data_pt = new Data(1);
    //   Parameter_data_pt->set_value(0, *parameter_pt);
    //   parameter_pt = &(Parameter_data_pt->value(0);
    //   add_global_data(Parameter_data_pt);

    //  oomph_info << "Number of unknowns: " << assign_eqn_numbers() <<
    //  std::endl;
    //}
    ...
    int steady_newton_solve_adapt_if_needed(const unsigned& max_adapt)
    {
      ...
      {
        OomphLibError(
          "Problem is not steady. Call make_steady before steady newton solve.",
    ...
    void pin_kinematic_lagrange_multiplier(const double& value)
    {
    ...
    // Set the maximum number of adapts when timestepping or solving the static
    // problem
    ...
    // than at each time step. Closed in the destructor. This is public so that
    // they can be opened after adjusting problem parameters.
    // If this is the first time, then add the variable headers.
    ...
    void doc_solution()
    {
      ...
      if (local_rank == 0)
      {
        ...
        if (Doc_Parameters::Doc_free_surface)
        {
          // Output free surface
        ...
        if (Doc_Parameters::Doc_contact_angle)
        {
...
#ifdef OOMPH_HAS_MPI
      // Ensure each rank doesn't start further computations, until documenting
      // has completed.
      MPI_Barrier(comm_pt->mpi_comm());
#endif
  ...
  private:
    ...
    void set_boundary_conditions()
    {
      ...
      // pin_velocity_on_boundary(v_index, Upper_boundary_id);
