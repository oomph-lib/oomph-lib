  ...
  *continuation_param_pt -= step;
  ...
  while (iterations < max_iterations)
  {
    ...
    *continuation_param_pt += step;
    ...
    // double custom_Bo = 0.84 * exp(1.6616 * alpha) + 0.8594 * alpha;

    double custom_Bo = 0.0;
    if (alpha <= MathematicalConstants::Pi / 2)
    {
      custom_Bo = 0.84 * exp(1.6616 * alpha) + 0.916732472209317 * alpha;
    }
    else
    {
      alpha = MathematicalConstants::Pi - alpha;
      custom_Bo = 0.84 * exp(1.6616 * alpha) + 0.916732472209317 * alpha;
    }
    cout << "alpha: " << alpha << ", ";
    cout << "Bo: " << custom_Bo << endl;

    problem.set_contact_angle(
      Global_Physical_Parameters::Equilibrium_contact_angle);
    problem.set_bond_number(custom_Bo);
    problem.set_capillary_number(Global_Physical_Parameters::Ca);
    problem.set_reynolds_number(Global_Physical_Parameters::Re);
