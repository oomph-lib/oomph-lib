...
#ifndef AXISYM_DYNAMIC_CAP_PROBLEM_HEADER
#define AXISYM_DYNAMIC_CAP_PROBLEM_HEADER
...
namespace oomph
{
  ...
  class AxisymDynamicCapProblem : public MyEigenproblem
  {
  public:
    ...
    // Constructor
    AxisymDynamicCapProblem(const double& contact_angle,
  ...
  private:
    ...
    void create_contact_angle_element()
    {
      ...
      FREE_SURFACE_ELEMENT* bulk_el_pt = 0;
      int face_index = 0;
      find_corner_free_surface_element_and_face_index(
        Outer_boundary_with_slip_id, bulk_el_pt, face_index);

      // Make bounding element on the free surface's face
      el_pt = bulk_el_pt->make_bounding_element(face_index);
      ...
      // ---
      // Find the other end, the inner boundary side.
      find_corner_free_surface_element_and_face_index(
        Inner_boundary_id, bulk_el_pt, face_index);
    ...
    void find_corner_free_surface_element_and_face_index(
      const unsigned& boundary_id,
      FREE_SURFACE_ELEMENT*& bulk_el_pt,
      int& face_index)
    {
      ...
      for (unsigned e = 0; e < n_free_surface; e++)
      {
        ...
        bulk_el_pt = dynamic_cast<FREE_SURFACE_ELEMENT*>(
        ...
        if (bulk_el_pt->node_pt(0)->is_on_boundary(boundary_id))
        {
        ...
        if (bulk_el_pt->node_pt(n_node - 1)->is_on_boundary(boundary_id))
        {
    ...
    void find_corner_slip_surface_element_and_face_index(
      const unsigned& boundary_id, SLIP_ELEMENT*& bulk_el_pt, int& face_index)
    {
      // Here we have no guarantee of order so we need to loop over all
      // surface elements to find the one that is next to the outer boundary
      unsigned n_slip_surface = Slip_boundary_mesh_pt->nelement();
      for (unsigned e = 0; e < n_slip_surface; e++)
      {
        // Locally cache the element pointer
        bulk_el_pt =
          dynamic_cast<SLIP_ELEMENT*>(Slip_boundary_mesh_pt->element_pt(e));
