...
$OOMPH_ROOT_DIR/scripts/fpdiff.py ../validata/result_two_d.dat.gz \
    result_two_d.dat  >> validation.log
fi
if test "$2" = "no_fpdiff"; then
  echo "dummy [OK] -- Can't run fpdiff.py because we don't have python or validata" >> validation.log
else
$OOMPH_ROOT_DIR/scripts/fpdiff.py ../validata/2d_pres_disp.dat.gz \
    2d_pres_disp.dat  >> validation.log
fi
if test "$2" = "no_fpdiff"; then
  echo "dummy [OK] -- Can't run fpdiff.py because we don't have python or validata" >> validation.log
else
$OOMPH_ROOT_DIR/scripts/fpdiff.py ../validata/2d_pres_disp_inc.dat.gz \
