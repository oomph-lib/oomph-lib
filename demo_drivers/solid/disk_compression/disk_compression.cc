  ...
  void make_traction_element_mesh(SolidMesh *&traction_mesh_pt)
  {
    ...
    for (unsigned e = 0; e < n_element; e++)
    {
      ...
      traction_mesh_pt->add_element_pt(
        new SolidTractionElement<ELEMENT>(fe_pt, face_index));
    }
  }
};
...
void StaticDiskCompressionProblem<ELEMENT>::doc_solution(DocInfo &doc_info)
{
  ...
  double exact_r =
    sqrt(Global_Physical_Variables::Uniform_gamma) *
  ...
  Trace_file << Global_Physical_Variables::P << " "
             << Global_Physical_Variables::Uniform_gamma << " " << volume << " "
             << exact_r << " ";
...
int main(int argc, char *argv[])
{
  ...
  {
    // Set up the problem
    StaticDiskCompressionProblem<RefineableQPVDElementWithContinuousPressure<2>>
      problem;
