...
namespace oomph
{
  ...
  public:
    ...
    /// Twice the number of spatial dimensions (for the constrained and
...
{
  /// \short Create a matrix-based diagonal preconditioner for
...
public:
  ...
  void position(const Vector<double>& zeta, Vector<double>& r) const
  {
    ...
    r[1] =
      1.0 + Ampl * 0.5 * (1.0 - cos(2.0 * MathematicalConstants::Pi * zeta[0]));
  }
...
PrescribedBoundaryDisplacementProblem<
  ELEMENT>::PrescribedBoundaryDisplacementProblem(const unsigned& nel_1d)
{
  // Create the mesh
  ...
  // Now create the mesh
  solid_mesh_pt() =
    new ElasticRefineableRectangularQuadMesh<ELEMENT>(n_x, n_y, l_x, l_y);
...
//=======================================================================
template<class ELEMENT>
void PrescribedBoundaryDisplacementProblem<
...
void PrescribedBoundaryDisplacementProblem<ELEMENT>::doc_solution()
{
  ...
  //----------------------------
  sprintf(
    filename, "%s/lagr%i.dat", Doc_info.directory().c_str(), Doc_info.number());
  some_file.open(filename);
...
int main(int argc, char* argv[])
{
  ...
  PrescribedBoundaryDisplacementProblem<
    PseudoElasticBulkElement<RefineableQPVDElement<2, 3>>>
    problem(nel_1d);
