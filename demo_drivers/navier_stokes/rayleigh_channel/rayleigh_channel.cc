...
namespace ExactSoln
{
  ...
  void get_exact_u(const double& t, const Vector<double>& x, Vector<double>& u)
  {
    ...
    // Solution
    complex<double> sol(exp(complex<double>(0.0, omega * t)) *
...
void RayleighProblem<ELEMENT, TIMESTEPPER>::doc_solution(DocInfo& doc_info)
{
  ...
  some_file.open(filename);
  mesh_pt()->output_fct(
    some_file, npts, time_pt()->time(), ExactSoln::get_exact_u);
  some_file.close();
  ...
  mesh_pt()->compute_error(
    some_file, ExactSoln::get_exact_u, time_pt()->time(), error, norm);
  some_file.close();
  ...
  // Write trace file
  Trace_file << time << " " << x[0] << " " << x[1] << " " << node_pt->value(0)
             << " " << node_pt->value(1) << " " << u[0] << " " << u[1] << " "
             << abs(u[0] - node_pt->value(0)) << " "
             << abs(u[1] - node_pt->value(1)) << " " << error << " " << norm
             << " " << std::endl;
...
int main(int argc, char* argv[])
{
  ...
  cout << "Long run flag: " << Global_Parameters::Long_run_flag << std::endl;
  cout << "Impulsive start flag: " << Global_Parameters::Impulsive_start_flag
       << std::endl;
