...
namespace Jeffery_Solution
{
  ...
  double velocity(const double &t)
  {
    ...
    return (a * a * sin(chi) * sin(chi) + b * b * cos(chi) * cos(chi)) /
           (a * a + b * b);
  }
  ...
  double acceleration(const double &t)
  {
    ...
    return 2.0 * (a * a - b * b) * (sin(chi) * cos(chi)) * chi_dot /
           (a * a + b * b);
  }
} // namespace Jeffery_Solution
...
template<class ELEMENT>
UnstructuredImmersedEllipseProblem<
  ELEMENT>::UnstructuredImmersedEllipseProblem()
{
  // Output directory
    ...
    else
    {
      ...
      for (unsigned inod = 0; inod < num_nod; inod++)
      {
        ...
        for (unsigned t = 0; t <= n_prev; t++)
        {
          ...
          {
            ramp = a1 * time_ * time_ * time_ + b1 * time_ * time_ + c1 * time_;
          }
...
//===================================================================
template<class ELEMENT>
void UnstructuredImmersedEllipseProblem<
  ELEMENT>::solve_for_consistent_nodal_positions()
{
...
int main(int argc, char **argv)
{
  ...
  UnstructuredImmersedEllipseProblem<
    ProjectableTaylorHoodElement<MyTaylorHoodElement>>
    problem;
