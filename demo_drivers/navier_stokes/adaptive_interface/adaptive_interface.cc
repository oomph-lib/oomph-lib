...
public:
  ...
  CylinderAndInterfaceDomain(const double &Length, const double &Height)
  {
    ...
    Cylinder_pt =
      new GeneralEllipse(centre_x, centre_y, 0.2 * Height, 0.2 * Height);
...
public:
    ...
    Height(height)
  {
    ...
    for (unsigned n = 0; n < Np; n++)
    {
      ...
      finite_element_pt(4)->node_pt(n * Np) = finite_element_pt(1)->node_pt(n);
...
class RefineableCylinderAndInterfaceMesh :
  public CylinderAndInterfaceMesh<ELEMENT>,
  ...
  Angle(1.57)
{
  ...
  // Build mesh
  Bulk_mesh_pt = new RefineableCylinderAndInterfaceMesh<ELEMENT>(
...
int main()
{
  RefineableRotatingCylinderProblem<RefineablePseudoSolidNodeUpdateElement<
