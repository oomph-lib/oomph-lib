...
namespace oomph
{
  ...
  public:
    ...
    void debug_elemental_jacobian()
    {
      oomph_info << "debug_elemental_jacobian" << std::endl;
      {
        unsigned n_element = Bulk_mesh_pt->nelement();
        for (unsigned e = 0; e < n_element; e++)
        {
    ...
    void pin_no_penetration_lagrange_multiplier_on_upper_boundary()
    {
      if (No_penetration_boundary_mesh_pt)
      {
        const unsigned n_el = No_penetration_boundary_mesh_pt->nelement();
    ...
    void debug_residuals()
    {
      ...
      std::ofstream output_stream(this->doc_info().directory() + "/residuals" +
    ...
    void add_header_to_trace_file()
    {
      ...
      Trace_file << "H ";
      Trace_file << "ndof ";
      Trace_file << "desired_corner_element_length ";
      Trace_file << "actual_corner_element_length ";
      Trace_file << "velocity_norm ";
      Trace_file << "deformation_norm ";
      Trace_file << "max_err ";
      Trace_file << "min_err ";
      Trace_file << "max_surf_err ";
    ...
    void doc_trace(const double& max_err, const double& min_err)
    {
      ...
      find_corner_bulk_element_and_face_index(Outer_boundary_with_slip_id,
                                              ...
                                              element_pt,
                                              face_index);
      // the actual corner element length
      Trace_file << element_pt->size() << " ";
      ...
      Trace_file << min_err << " ";
      Trace_file << this->max_free_surface_error() << " ";
    ...
    {
      Vector<double> pressure_value;

      // Find free surface element at the corner and the node numbering
      Vector<FiniteElement*> free_surface_el_pt;
      Vector<int> face_index;
      find_two_nearest_corner_surface_element_and_face_index(
      ...
      for (unsigned n = 0; n < 2; n++)
      {
        if (face_index[n] == 1)
        {
          pressure_value.push_back(
            free_surface_el_pt[1 - n]->node_pt(0)->value(3));
          pressure_value.push_back(
            free_surface_el_pt[1 - n]->node_pt(2)->value(3));
        }
        else
        {
          pressure_value.push_back(
      ...
      Vector<int> slip_face_index;
      Vector<FiniteElement*> slip_surface_el_pt;
      find_two_nearest_corner_surface_element_and_face_index(
        Slip_boundary_mesh_pt,
        Free_surface_boundary_id,
        slip_surface_el_pt,
        slip_face_index);
      for (unsigned n = 0; n < 2; n++)
      {
        if (slip_face_index[n] == 1)
        {
          pressure_value.push_back(slip_surface_el_pt[n]->node_pt(2)->value(3));
          pressure_value.push_back(slip_surface_el_pt[n]->node_pt(0)->value(3));
        }
        else
        {
          pressure_value.push_back(slip_surface_el_pt[n]->node_pt(0)->value(3));
  ...
  private:
    void find_two_nearest_corner_surface_element_and_face_index(
      ...
      Vector<FiniteElement*>& bulk_el_pt,
      Vector<int>& face_index)
    {
      // Here we have no guarantee of order so we need to loop over all
      // surface elements to find the one that is next to the outer boundary
      FiniteElement* el_pt = 0;
      unsigned n_surface = surface_mesh_pt->nelement();
      for (unsigned e = 0; e < n_surface; e++)
      {
        // Locally cache the element pointer
        el_pt = dynamic_cast<FiniteElement*>(surface_mesh_pt->element_pt(e));

        // Read out number of nodes in the element
        unsigned n_node = el_pt->nnode();

        // Is the "left" hand node on the boundary
        if (el_pt->node_pt(0)->is_on_boundary(boundary_id))
        {
          // Create bounding element on "left" hand face, with the normal
          // pointing into the fluid
          bulk_el_pt.push_back(el_pt);
        ...
        // Is the "right" hand node on the boundary
        if (el_pt->node_pt(n_node - 1)->is_on_boundary(boundary_id))
        {
          // Create bounding element on "right" hand face, with the normal
          // pointing out of the fluid
          bulk_el_pt.push_back(el_pt);
      ...
      // Find node at the other end of the first element.
      unsigned n_node2 = el_pt->nnode();
      Node* node_pt = 0;
      if (face_index[0] == 1)
      {
        node_pt = el_pt->node_pt(0);
      }
      else
      {
        node_pt = el_pt->node_pt(n_node2 - 1);
      }

      n_surface = surface_mesh_pt->nelement();
      for (unsigned e = 0; e < n_surface; e++)
      {
        // Locally cache the element pointer
        el_pt = dynamic_cast<FiniteElement*>(surface_mesh_pt->element_pt(e));
        if (el_pt != bulk_el_pt[0])
        {
          // Read out number of nodes in the element
          unsigned n_node = el_pt->nnode();

          // Is the "left" hand node on the boundary
          if (el_pt->node_pt(0) == node_pt)
          {
            // Create bounding element on "left" hand face, with the normal
            // pointing into the fluid
            bulk_el_pt.push_back(el_pt);
          ...
          // Is the "right" hand node on the boundary
          if (el_pt->node_pt(n_node - 1) == node_pt)
          {
            // Create bounding element on "right" hand face, with the normal
            // pointing out of the fluid
            bulk_el_pt.push_back(el_pt);
  ...
  private:
    ...
    void set_boundary_conditions()
    {
      ...
      pin_no_penetration_lagrange_multiplier_on_upper_boundary();
        ...
        for (unsigned i = 0; i < n_node; i++)
        {
          ...
          if (node_pt->is_on_boundary(b))
          {
            ...
            else
            {
              ...
              // Its imposed value is 0.0 which is already the value by
              // default in the vector Imposed_velocity_values_at_node. So
    ...
    double max_free_surface_error()
    {
      ...
      for (unsigned e = 0; e < n_free_surface; e++)
      {
        ...
        FREE_SURFACE_ELEMENT* el_pt = dynamic_cast<FREE_SURFACE_ELEMENT*>(
          Free_surface_mesh_pt->element_pt(e));
        el_pt->compute_error();
        max_error = std::max(el_pt->get_error(), max_error);
      }
      return max_error;
    }
