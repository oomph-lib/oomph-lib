...
namespace oomph
{
  ...
  class SingularNavierStokesElement : public virtual BASIC_NAVIER_STOKES_ELEMENT
  {
  private:
    double Error;
  ...
  public:
    ...
    SingularNavierStokesElement()
      : BASIC_NAVIER_STOKES_ELEMENT(), Error(0.0), IsAugmented(false)
    {
    ...
    {
      return BASIC_NAVIER_STOKES_ELEMENT::required_nvalue(n);
      //      // Basic element
      //      // dim * [u] + 1 * [p]
      //
      //      // This element
    ...
    void setup_new_data()
    {
      ...
      for (unsigned n = 0; n < n_node; n++)
      {
        ...
        {
          this->node_pt(n)->set_value(u_index_nst(n, i),
  ...
  public:
    void pin()
    {
      ...
      for (unsigned i = 0; i < n_node; i++)
      {
        ...
        {
          this->node_pt(i)->pin(this->momentum_index_nst(i, j));
        }
        if (this->IsAugmented)
        {
          for (unsigned j = 0; j < 2; j++)
      ...
      {
        this->node_pt(i)->pin(this->p_index_nst());
      }
    }
    ...
    virtual double p_nst(const unsigned& n) const
    {
      return this->nodal_value(n, this->p_nodal_index_nst());
    }
    ...
    double dpdx_fe_only(Vector<double> s, const unsigned* direction_pt)
    {
      ...
      {
        interpolated_dpdx_fe_only += p_nst(j) * dpsipdx(j, *direction_pt);
      }
    ...
    {
      BASIC_NAVIER_STOKES_ELEMENT::fill_in_contribution_to_residuals(residuals);
                                          ...
                                          DenseMatrix<double>& jacobian)
    {
      ...
      if (!this->IsAugmented)
      {
        ...
        BASIC_NAVIER_STOKES_ELEMENT::fill_in_contribution_to_jacobian(residuals,
                                                                      jacobian);
      ...
      else
      {
        ...
        // of the fd version.
        /*
        BASIC_NAVIER_STOKES_ELEMENT::fill_in_contribution_to_jacobian(
          residuals, jacobian);
        fill_in_generic_residual_contribution_additional_terms(
    ...
    void interpolated_u_nst(const Vector<double>& s, Vector<double>& veloc)
    {
      if (IsAugmented)
      {
                                 ...
                                 const unsigned& j)
    {
      ...
      for (unsigned l = 0; l < n_node; l++)
      {
        ...
        {
          u_value = this->nodal_value(l, u_index_nst(l, i));
        }
