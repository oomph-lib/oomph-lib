...
namespace oomph
{
  ...
  public:
    ...
    MyCrouzeixRaviartElement() :
      PseudoSolidNodeUpdateElement<TCrouzeixRaviartElement<2>,
                                   TPVDBubbleEnrichedElement<2, 3>>()
    {
      Error = 0.0;
    }
...
DropInChannelProblem<ELEMENT>::DropInChannelProblem()
{
  ...
  drop_polyline_pt[0] =
    new TriangleMeshPolyLine(drop_vertex, First_drop_boundary_id);
  ...
  drop_polyline_pt[1] =
    new TriangleMeshPolyLine(drop_vertex, Second_drop_boundary_id);

  // Create closed polygon from two polylines
  Drop_polygon_pt[0] = new TriangleMeshPolygon(drop_polyline_pt);
  ...
  triangle_mesh_parameters.internal_closed_curve_pt() = drop_closed_curve_pt;
...
void DropInChannelProblem<ELEMENT>::create_volume_constraint_elements()
{
  ...
  Drop_pressure_data_pt =
    Hijacked_element_pt->hijack_internal_value(0, index_of_traded_pressure);
...
void DropInChannelProblem<ELEMENT>::complete_problem_setup()
{
  ...
  for (unsigned idrop = 0; idrop < ndrop; idrop++)
  {
    ...
    Vector<unsigned> drop_bound_id =
      this->Drop_polygon_pt[idrop]->polygon_boundary_id();
  ...
  for (unsigned ibound = 0; ibound < nbound; ++ibound)
  {
    if ((ibound == Upper_wall_boundary_id) ||
        ...
        (ibound == Outflow_boundary_id) || (ibound == Inflow_boundary_id))
    {
  ...
  for (unsigned inod = 0; inod < num_nod; inod++)
  {
    ...
    double y = nod_pt->x(1);
    nod_pt->set_value(0,
                      Problem_Parameter::Inflow_veloc_magnitude * y * (1 - y));
  }
}
...
void DropInChannelProblem<ELEMENT>::doc_solution(const std::string& comment)
{
  ...
  Problem_Parameter::Trace_file
    << this->time_pt()->time() << " " << Fluid_mesh_pt->nelement() << " "
    << max_area << " " << min_area << " " << max_err << " " << min_err << " "
    << sqrt(square_of_l2_norm) << " " << std::endl;
...
int main(int argc, char** argv)
{
  ...
  DropInChannelProblem<
    Hijacked<ProjectableCrouzeixRaviartElement<MyCrouzeixRaviartElement>>>
    problem;
