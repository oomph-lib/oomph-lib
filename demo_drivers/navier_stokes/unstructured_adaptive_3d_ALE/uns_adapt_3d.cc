...
public:
  ...
  void snap_onto_sphere()
  {
    ...
    for (unsigned n = 0; n < n_bound; ++n)
    {
      ...
      // Do the snapping
      double R_new =
        sqrt(1.0 + 0.5 * 0.5 * (1.0 + sqrt(5.0)) * (1.0 + sqrt(5.0)));
  ...
  void actions_before_newton_solve()
  {
    ...
    // Now set a Poiseuille like inflow
    {
      ...
      for (unsigned inod = 0; inod < num_nod; inod++)
      {
        ...
        double y = nod_pt->x(1);
        double u =
          (Box_width - x) * (Box_width + x) * (Box_width - y) * (Box_width + y);
...
FallingBlockProblem<ELEMENT>::FallingBlockProblem()
{
  ...
  Problem::mesh_pt() = new RefineableTetgenMesh<ELEMENT>(
...
int main(int argc, char* argv[])
{
  ...
  {
    FallingBlockProblem<
      ProjectableCrouzeixRaviartElement<TCrouzeixRaviartElement<3>>>
      problem;
