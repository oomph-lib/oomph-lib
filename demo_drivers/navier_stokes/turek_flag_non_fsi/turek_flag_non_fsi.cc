...
namespace Flag_definition
{
  ...
  public:
                  ...
                  Vector<double>& r) const
    {
      ...
      point_on_circle[0] =
        Centre_x + Radius * sqrt(1.0 - H * H / (4.0 * Radius * Radius));
      point_on_circle[1] = Centre_y + H / 2.0;

      r[0] = point_on_circle[0] +
             xi[0] / L * (upper_tip(Time_pt->time(t))[0] - point_on_circle[0]);

      r[1] = point_on_circle[1] +
             xi[0] / L * (upper_tip(Time_pt->time(t))[1] - point_on_circle[1]) +
             1.0 / 3.0 *
               sin((r[0] - point_on_circle[0]) /
                   (upper_tip(Time_pt->time(t))[0] - point_on_circle[0]) *
  ...
  public:
                  ...
                  Vector<double>& r) const
    {
      ...
      r[0] = point_on_circle[0] +
             xi[0] / L * (lower_tip(Time_pt->time(t))[0] - point_on_circle[0]);

      r[1] = point_on_circle[1] +
             xi[0] / L * (lower_tip(Time_pt->time(t))[1] - point_on_circle[1]) +
             1.0 / 3.0 *
               sin((r[0] - point_on_circle[0]) /
                   (lower_tip(Time_pt->time(t))[0] - point_on_circle[0]) *
                   MathematicalConstants::Pi) *
               sin(2.0 * MathematicalConstants::Pi * Time_pt->time(t) / Period);
    }
  ...
  public:
                  ...
                  Vector<double>& r) const
    {
      ...
      r[1] = point_bottom[1] +
             (xi[0] + H / 2.0) / H * (point_top[1] - point_bottom[1]);
      r[0] = point_bottom[0] +
             (xi[0] + H / 2.0) / H * (point_top[0] - point_bottom[0]);
    }
  ...
  Height(height)
{
  ...
  Problem::mesh_pt() =
    new RefineableCylinderWithFlagMesh<ELEMENT>(Flag_definition::Cylinder_pt,
...
int main(int argc, char* argv[])
{
  ...
  TurekNonFSIProblem<RefineableQCrouzeixRaviartElement<2>> problem(length,
                                                                   height);

#else

  // Create Problem with AlgebraicMesh-based node update
  TurekNonFSIProblem<AlgebraicElement<RefineableQCrouzeixRaviartElement<2>>>
