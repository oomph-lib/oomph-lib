...
public:
                ...
                Vector<double>& r) const
  {
    ...
    if (Remain_steady_at_maximum_amplitude)
    {
      // Position vector
      r[0] =
        zeta[0] + X_left + B * A * sin(2.0 * 3.14159 * zeta[0] / Length) * ramp;

      r[1] =
        H - A * ((Length - zeta[0]) * zeta[0]) / pow(0.5 * Length, 2) * ramp;
    }
    else
    {
      // Position vector
...
private:
  ...
  /// Number of elements in the x direction in the downstream part of the
  /// channel
  unsigned Ndown;
  ...
  const unsigned& outflow)
{
  ...
  Wall_pt =
    new OscillatingWall(height, x_left, length, amplitude, period, time_pt());

  // Build mesh
  Bulk_mesh_pt =
    new RefineableCollapsibleChannelMesh<ELEMENT>(nup,
                                                  ncollapsible,
                                                  ndown,
                                                  ny,
                                                  lup,
                                                      ...
                                                      ofstream& trace_file)
{
  ...
  // Write trace file
  trace_file << time_pt()->time() << " " << wall_point[1] << " "
             << Left_node_pt->value(0) << " " << Right_node_pt->value(0) << " "
             << std::endl;
  ...
  for (unsigned i = 0; i < nplot; i++)
  {
    ...
    Wall_pt->position(zeta, wall_point);
    some_file << wall_point[0] << " " << wall_point[1] << std::endl;
  }
  some_file.close();
...
void CollapsibleChannelProblem<ELEMENT>::setup_outflow_flux_control_elements()
{
  ...
  Outflow_flux_control_master_mesh_pt->add_element_pt(flux_control_el_pt);
...
void CollapsibleChannelProblem<ELEMENT>::setup_outflow_impedance_elements()
{
  ...
  if (time_stepper_pt()->time() > 0.0)
  {
    std::ostringstream error_stream;
    error_stream
      << "Impedance is set up assuming problem has been at rest for all "
         "previous "
  ...
  if (Outflow_type == 1)
  {
    ...
    Womersley_impedance_tube_pt =
      new WomersleyOutflowImpedanceTube<QWomersleyElement<1, 3>, 1>(
        L_impedance, Outflow_impedance_mesh_pt, fixed_coordinate, v_index);
  }
  else
  {
    // Loop over all elements on this boundary
...
void CollapsibleChannelProblem<ELEMENT>::set_poiseuille_outflow()
{
  ...
  for (unsigned e = 0; e < n_bound_el; e++)
  {
    ...
    bulk_mesh_pt()->node_pt(e)->set_value(
      0, 6.0 * (x[1] / Ly) * (1.0 - (x[1] / Ly)));
  ...
  string directory_for_data, double nstep, const bool& validation_run)
{
  ...
  {
    double q_initial =
      Womersley_impedance_tube_pt->total_volume_flux_into_impedance_tube();
    oomph_info << "q_initial=" << q_initial << "\n";
    Womersley_impedance_tube_pt->setup(
      &Global_Physical_Variables::ReSt, dt, q_initial, new BDF<2>);
  }
  ...
  */
  ...
  if (print_help)
  {
    // if (MPI_Helpers::My_rank==0)
    {
      ...
      oomph_info << "-validation_run           Generate validation data for 1D "
                    "Womersley\n";
    }
  ...
  // Output settings
  oomph_info << "Storing data in /" << directory_for_data << "\n";
  oomph_info << "Outflow type: " << outflow << "\n";
  oomph_info << "Re=" << Global_Physical_Variables::Re << "\n";
  if (solver == 0)
  {
...
#else
      oomph_info << "Warning: Hypre not available. Using SuperLU on P matrix\n";
#endif
    }
    else if (p_solver == 1)
    {
...
#else
      oomph_info << "Warning: Hypre not available. Using SuperLU on F matrix\n";
#endif
    }
    else if (f_solver == 1)
    {
  ...
  // LSC preconditioner!
  CollapsibleChannelProblem<RefineableQTaylorHoodElement<2>> problem(
    nup,
    ncollapsible,
    ndown,
    ny,
  ...
  // delete stuff - remembering P and F matrix preconditioners are deleted in
  ...
  std::cout << "dummy output to suppress compiler warnings: "
            << f_bamg_smoother + f_bamg_damping + f_bamg_strength +
                 f_ml_settings
            << " " << p_preconditioner_pt << " " << f_preconditioner_pt
            << std::endl;
