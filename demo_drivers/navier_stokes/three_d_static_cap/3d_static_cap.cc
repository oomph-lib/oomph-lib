...
//=start of mesh class===============================================
template<class ELEMENT,class INTERFACE_ELEMENT>
class AxialElasticQuarterTubeMesh : public RefineableQuarterTubeMesh<ELEMENT>,
public SolidMesh
{
 /// Vector of pointers to element in the fluid layer
 Vector <GeneralisedElement *> Bulk_element_pt;
...
public:
 ...
 ///Constructor
 AxialElasticQuarterTubeMesh(GeomObject* wall_pt,
                           const Vector<double>& xi_lo,
                           const double& fract_mid,
                           const Vector<double>& xi_hi,
                           const unsigned& nlayer,
  ...
  SolidMesh()
  {
   ...
   //Loop over the remaining layers
   {
    ...
    for(unsigned e=0;e<n_element;e++)
     {
      //If the element is on the last layer
      if(std::abs(finite_element_pt(e)->node_pt(0)->x(2)-boundary) < 1.0e-15)
       {
        //If on the last layer
        if(l==n_layer-1)
...
//====================================================================
template<class ELEMENT>
class ElasticFreeSurfaceRotationProblem : public Problem
{
 /// The volume of the fluid
 double Volume;
...
public:
 ...
 /// Constructor: Pass DocInfo object and target errors
 ElasticFreeSurfaceRotationProblem(DocInfo& doc_info, 
                                   const double& min_error_target,
                                   const double& max_error_target, 
                                   const unsigned &hijack_flag);

 /// Destructor to clean up memory (empty)
 ~ElasticFreeSurfaceRotationProblem() {}
 ...
 /// \short Overload generic access function by one that returns
 /// a pointer to the specific  mesh
 AxialElasticQuarterTubeMesh<ELEMENT,
FixedVolumeElasticSurfaceFluidInterfaceElement<ELEMENT> >* mesh_pt() 
  {
   return 
    dynamic_cast<AxialElasticQuarterTubeMesh<ELEMENT,
    FixedVolumeElasticSurfaceFluidInterfaceElement<ELEMENT> >*>
    (Problem::mesh_pt());
  }
...
//========================================================================
template<class ELEMENT>
ElasticFreeSurfaceRotationProblem<ELEMENT>::
ElasticFreeSurfaceRotationProblem(DocInfo& doc_info,
                                  const double& min_error_target,
                                  const double& max_error_target,
                                  const unsigned &hijack_flag) 
 : Volume(atan(1.0)), Doc_info(doc_info)
{
 ...
 //linear_solver_pt() = new DenseLU; //new FD_LU;
 ...
 // Build and assign mesh
 Problem::mesh_pt()=
  new AxialElasticQuarterTubeMesh<ELEMENT,
  FixedVolumeElasticSurfaceFluidInterfaceElement<ELEMENT> >
  (Wall_pt,xi_lo,frac_mid,xi_hi,nlayer);
  ...
  for(unsigned n=0;n<n_node;n++)
   {
    ...
    static_cast<SolidNode*>(mesh_pt()->boundary_node_pt(b,n))
     ->pin_position(2);
   }
 }
 ...
 /*{
  ...
  for(unsigned n=0;n<n_node;n++)
   {
    SolidNode* solid_nod_pt = static_cast<SolidNode*>(mesh_pt()->node_pt(n));
    for(unsigned i=0;i<2;i++)
     {
 ...
 //Now only allow nodes on the top boundary to move vertically
 /*{
  unsigned n_node = mesh_pt()->nboundary_node(4);
  for(unsigned n=0;n<n_node;n++)
   {
    static_cast<SolidNode*>(mesh_pt()->boundary_node_pt(4,n))->unpin_position(2);
   }
   }*/
 ...
 for(unsigned e=0;e<n_interface;e++)
  {
   FixedVolumeElasticSurfaceFluidInterfaceElement<ELEMENT>* el_pt
    = dynamic_cast<FixedVolumeElasticSurfaceFluidInterfaceElement<ELEMENT>*>(
     mesh_pt()->interface_element_pt(e));
 ...
 for(unsigned e=0;e<n_interface_edge;e++)
  {
   ElasticVolumeConstraintLineElement<ELEMENT>* el_pt
    = dynamic_cast<ElasticVolumeConstraintLineElement<ELEMENT>*>(
     mesh_pt()->interface_edge_element_pt(e));
...
//========================================================================
template<class ELEMENT>
void ElasticFreeSurfaceRotationProblem<ELEMENT>::doc_solution()
{ 
