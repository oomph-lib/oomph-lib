...
class AxialSpineQuarterTubeMesh :
    ...
    SpineMesh()
  {
    ...
    for (unsigned l = 1; l < n_layer; l++)
    {
      ...
      for (unsigned e = 0; e < n_element; e++)
      {
        ...
        if (std::abs(finite_element_pt(e)->node_pt(0)->x(2) - boundary) <
            1.0e-15)
        {
          ...
          if (l == n_layer - 1)
          {
            ...
            for (unsigned n = 0; n < n_p; n++)
            {
              ...
              // Right
              adjacent[1] &= interface_element_pt->node_pt(n * n_p + n_p - 1)
                               ->is_on_boundary(3);
              // Top
              adjacent[2] &= interface_element_pt->node_pt(n_p * (n_p - 1) + n)
                               ->is_on_boundary(3);
              // Left
              adjacent[3] &=
  ...
  Volume(atan(1.0)), Doc_info(doc_info)
{
  ...
  Bulk_mesh_pt =
    new AxialSpineQuarterTubeMesh<ELEMENT,
  ...
  for (unsigned b = 0; b < 5; b++)
  {
    ...
    for (unsigned e = 0; e < n_element; e++)
    {
      ...
      SpineSurfaceVolumeConstraintBoundingElement<ELEMENT> *el_pt =
        new SpineSurfaceVolumeConstraintBoundingElement<ELEMENT>(bulk_elem_pt,
                                                                 face_index);
