    ...
    SolidMesh()
  {
    ...
    for (unsigned e = 0; e < n_element; e++)
    {
      ...
      for (unsigned b = 0; b < 5; b += 4)
      {
        ...
        corner[2] =
          interface_element_pt->node_pt(np * (np - 1))->is_on_boundary(b);
        corner[3] =
          interface_element_pt->node_pt(np * np - 1)->is_on_boundary(b);
        ...
        if (bottom)
        {
          Interface_edge_element_pt.push_back(
            interface_element_pt->make_bounding_element(-2));
        }
...
int main(int argc, char* argv[])
{
  ...
  //-----------------------------
  {
    ...
    // Build problem
    EntryFlowProblem<RefineablePseudoSolidNodeUpdateElement<
