  ...
  Angle(0.5 * MathematicalConstants::Pi) // Initialise the contact angle
{
  ...
  // Construct our mesh
  Bulk_mesh_pt =
    new TwoLayerSpineMesh<SpineElement<ELEMENT>>(Nx, Nh1, Nh2, 0.5, 1.0, 1.0);
...
//========================================================================
template<class ELEMENT>
class ElasticTwoLayerMesh : public ElasticRectangularQuadMesh<ELEMENT>
{
public:
...
void PseudoSolidCapProblem<ELEMENT>::doc_solution(DocInfo& doc_info)
{
  ...
  unsigned np =
    Free_surface_mesh_pt->finite_element_pt(ninterface - 1)->nnode();
  ...
  Trace_file << Angle * 180.0 / MathematicalConstants::Pi;
  Trace_file << " "
             << Free_surface_mesh_pt->finite_element_pt(0)->node_pt(0)->x(1)
             << " "
