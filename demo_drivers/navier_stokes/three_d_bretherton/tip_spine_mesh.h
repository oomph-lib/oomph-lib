...
// The cilinder may be the part of an ellipe if (xmax - xmi)n noneq ro (zmax
// -zmin)
/// coincides with the numeration of the spines
//======================================================================
template<class ELEMENT, class INTERFACE_ELEMENT>
class MyTipMesh : public SimpleCubicMesh<ELEMENT>, public SpineMesh
{
public:
  ...
  double init_spine_height(SpineNode* spine_node_pt)
  {
    ...
    double daxis = sqrt(
      (this->Xax - spine_node_pt->x(0)) * (this->Xax - spine_node_pt->x(0)) +
      (this->Yax - spine_node_pt->x(1)) * (this->Yax - spine_node_pt->x(1)) +
...
protected:
  ...
  // Flag that indicates wheterher we must do a rotation before setting the
  // spines
  ...
  TimeStepper* time_stepper_pt)
{
  ...
  // NOW WE GO THROUH ALL THE ELEMENTS OF THE LOWER LAYERS AND ATTACHING THE
  // SPINES
  ...
  for (unsigned l1 = 0; l1 < n_p; l1++) // y loop over the nodes
  {
    ...
    {
      // Node j + i*np
      // Vector to the origin node, it will be passed as argument for the a
      // later update of the spine
      ...
      {
        // Add bottom spine to element (equal to the one defined before the for
        // structure (for k=0) or the top of the last one)
  ...
  // The procedure is the same but we have to identify the before defined spines
  // for not defining them two times

  for (unsigned j = 1; j < n_x; j++) // loop over the elements in the first row
  {
    for (unsigned l1 = 0; l1 < n_p; l1++) // y loop over the nodes
    {
      // First we copy the last row of nodes into the first row of the new
      // element (and extend to the third dimension)
      /*for(unsigned k=0;k<n_z;k++)
       {
            //Recover the Spine of the before defined element
            Spine* old_spine =
       dynamic_cast<ELEMENT*>(Element_pt[k*n_x*n_y+j-1])->spine_pt(l1*n_p+n_p-1);
      ...
      for (unsigned l2 = 1; l2 < n_p; l2++) // x loop over the nodes
      {
        ...
        for (unsigned long k = 0; k < n_z; k++)
        {
          // Add bottom spine to element (equal to the one defined before the
          // for structure (for k=0) or the top of the last one)
  ...
  // Now we loop over the rest of the elements. We will separe the first of each
  // row being al the rest equal
    ...
    /*  for(unsigned l2=0;l2<n_p;l2++)
      {
        ...
        for(unsigned long k=0;k<n_z;k++)
        {
             //Recover the Spine of the before defined element
             Spine* old_spine =
      dynamic_cast<ELEMENT*>(Element_pt[k*n_x*n_y+(i-1)*n_x])->spine_pt(n_p*(n_p-1)+l2);
             //Add before defined  spine to element
      ...
      /*    for(unsigned l2=0;l2<n_p;l2++)
          {
            ...
            for(unsigned long k=0;k<n_z;k++)
            {
                 //Recover the Spine of the before defined element
                 Spine* old_spine =
         dynamic_cast<ELEMENT*>(Element_pt[k*n_x*n_y+(i-1)*n_x+j])->spine_pt(n_p*(n_p-1)+l2);
                 //Add before defined  spine to element
        ...
        /*    for(unsigned long k=0;k<n_z;k++)
         {
            //Recover the Spine of the before defined element
            Spine* old_spine =
         dynamic_cast<ELEMENT*>(Element_pt[k*n_x*n_y+(j-1)+i*n_x])->spine_pt(l1*n_p+n_p-1);
            //Add before defined  spine to element
