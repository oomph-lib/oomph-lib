...
///
// *****We do not introduce periodic meshes  at this point beacause we should
// change the complete bulk mesh*******
///
...
///
/// This mesh has been carefully desihned so that the numeration of the nodes on
/// the boundaries 0 and 5 (bottom and top) coincides with the numeration of the
/// spines
//======================================================================
...
public:
  ...
  virtual void spine_node_update(SpineNode *spine_node_pt)
  {
    ...
    // SpineNode* origin_pt =
    // dynamic_cast<SpineNode*>(spine_node_pt->spine_pt()->geom_data_pt(0));
    ...
    double norm = sqrt((Xax - origin_pt[0]) * (Xax - origin_pt[0]) +
                       (Yax - origin_pt[1]) * (Yax - origin_pt[1]) +
...
protected:
  ...
  void rotate_90_zeq1_xeq0(SpineMesh *rot_mesh_pt)
  {
    ...
    // We rotate as well the geometric parameters which were originally set as
    // the coordinate of the node at the origin of the spine
  ...
  TimeStepper *time_stepper_pt)
{
  ...
  // cout<<"After adding 5 in  the tip mesh the boundary 1
  // has"<<this->nboundary_node(1)<<"nodes"<<endl;
  ...
  std::cout << "The tip mesh consists of  " << nbulk() << " bulk elements and "
            << ninterface_element() << " interface elements." << std::endl;

  // At the end we destroy the block meshes in two steps

  // 1. We disconect the meshesh from the nodes and elements (if not we delete
  // them at the time we delete the meshes)
  ...
  // 2. Simply delete (this function can give a segmentation fault error when
  // there are spines still attached to the old update functions written in the
  // constructor)
  delete mesh1_pt;
  delete mesh2_pt;
  delete mesh3_pt;
  ...
  // introduce the aspect ratio and different lenght (there is a factor 2
  // becasue 1 = halve height= b/2)
  int ncount;
  ncount = this->nnode();
  ...
  for (int i = 0; i < ncount; i++)
  {
    ...
    double dorg =
      sqrt((spine_node_pt->x(0) - x_spine) * (spine_node_pt->x(0) - x_spine) +
...
// The function neeeds the pointer to the new added mes;, the sahred boundary of
// the problem mesh; a pointer which is a map between the boundaries in the
// addeed mesh and their new values when the mesh is attached (we will set as
// convection boundary = -1 for the shared boundary), the new total value of
// boundaries and a flag
// which will be used by the function update_node ( so that all the sub meshes
// with the same flag will be updated using the same function)

// This function can not be extended to adding every spine_mesh because the
// class SpineMesh lacks the bulk and interface element pointers
  ...
  unsigned spine_flag)
{
  ...
  MeshHelper::merge_spine_meshes(this,
                                 bound1,
                                 addmesh_pt,
                                 addmesh_map_boundary,
                                 total_boundaries,
                                 spine_flag);
...
// Merges this boundaries of the problem mesh. Both boundaries must contain
// nodes exactly in the same positions and equivalent spines. The nodes of one
// of the boundaries will be completly deleted and the ones of the other will
// keep on the mesh but not any more in the former boundary (they will remain in
// other
// boundaries different from bound1 and bound2) The function neeeds the boundary
// numbers to be merged and the end total number of boundaries (the merged
// boundaries can still exists but they
  ...
  unsigned int total_boundaries)
{
  ...
  {
    if (!(pt_mesh->boundary_node_pt(oldbound, i)
            ->is_on_boundary(newbound))) // Do not add the nodes which were
                                         // before on the boundary
      pt_mesh->add_boundary_node(newbound,
                                 pt_mesh->boundary_node_pt(oldbound, i));
  }
