...
public:
  ...
  /// \short Return the spatial (Eulerian) dimension of the element
  unsigned dim() const
  {
    return 0;
  }
  ...
  void set_traded_pressure_data(Data* traded_pressure_data_pt)
  {
    ...
    if (traded_pressure_data_pt->nvalue() != 1)
    {
      ...
      error_stream << "Traded pressure Data must only contain a single value!\n"
  ...
  Vector<double>& residuals)
{
  ...
  if (Height_pt == 0)
  {
    throw OomphLibError("Please set the pointer to the prescribed height",
  ...
  else
  {
    ...
    if (external_data_pt(0)->nvalue() != 1)
    {
      std::cout << "The external Data item that stores the traded pressure in "
                << std::endl;
      std::cout << "SpineVolumeConstraintPointElement should only have a single"
                << std::endl;
      std::cout << "value but is has " << external_data_pt(0)->nvalue()
                << std::endl;
      exit(1); // assert(false);
    }
#endif
...
protected:
  ...
  ///\short Array to hold the local eqn number information for the
  /// external data (other nodes in the bulk element)
  DenseMatrix<int> External_u_local_eqn;

  ///\short Vector to keep track of the external data associated
  /// with each bulk node
  Vector<unsigned> External_node;

  // BEGIN, bit fo rconsidering Ca as an unknown

  /// \short The Data that contains the capillary number is stored
...
public:
  ...
  /// \short Return the number of external velocity data
  ...
  double flow()
  {
    ...
    for (unsigned ipt = 0; ipt < n_intpt; ipt++)
    {
      ...
      {
        normal_flux += interpolated_u[i] * normal[i];
      }
                                         ...
                                         unsigned flag)
  {
    ...
    for (unsigned ipt = 0; ipt < n_intpt; ipt++)
    {
      ...
      (void)bulk_el_pt->dshape_eulerian(s_parent, psif_parent, dpsifdx_parent);
      ...
      for (unsigned l = 0; l < n_node; l++)
      {
        // Do the first (axial) velocity component -- Poisson equation
        {
          ...
          if (local_eqn >= 0)
          {
            ...
            // Add in the equation term (equation constant traction in the axial
            // direction d(Tyy+p)/dy =0)
            residuals[local_eqn] -=
              Viscosity_Ratio * W *
              ...
              for (unsigned l2 = 0; l2 < n_node; l2++)
              {
                ...
                // Poisson equation term
                {
                  ...
                  if (local_unknown >= 0)
                  {
                    jacobian(local_eqn, local_unknown) -=
                      Viscosity_Ratio *
                      (dpsifdx_parent(bulk_node_number(l2), 0) *
                         dtestfdx_parent(bulk_node_number(l), 0) +
              ...
              for (unsigned l2 = 0; l2 < n_external; l2++)
              {
                ...
                // Poisson equation term
                {
                  ...
                  if (local_unknown >= 0)
                  {
                    jacobian(local_eqn, local_unknown) -=
                      Viscosity_Ratio *
                      (dpsifdx_parent(External_node[l2], 0) *
                         dtestfdx_parent(bulk_node_number(l), 0) +
