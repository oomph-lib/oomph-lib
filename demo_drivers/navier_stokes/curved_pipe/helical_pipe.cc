...
public:
  ...
  void position(const Vector<double> &xi, Vector<double> &r) const
  {
    ...
    r[1] =
      (1.0 / Delta) * sin(xi[0]) + xi[2] * Radius * sin(xi[0]) * cos(xi[1]);
  ...
  Doc_info(doc_info)
{
  ...
  // Build geometric object that forms the domain boundary: a tapered curved
  // tube
  ...
  Problem::mesh_pt() = new RefineableTubeMesh<ELEMENT>(
...
int main(int argc, char *argv[])
{
  ...
  //------------------------
  {
    ...
    // Build problem
    SteadyHelicalProblem<RefineableQTaylorHoodElement<3>> problem(
      doc_info, min_error_target, max_error_target);
  ...
  //------------------------
  {
    ...
    // Build problem
    SteadyHelicalProblem<RefineableQCrouzeixRaviartElement<3>> problem(
      doc_info, min_error_target, max_error_target);
