...
public:
  ...
  SpikedLine(const Vector<Data*>& geom_data_pt) : GeomObject(1, 2)
  {
    ...
    if (geom_data_pt.size() != 1)
    {
      ...
      error_stream << "Wrong size for geom_data_pt " << geom_data_pt.size()
                   << std::endl;
                ...
                Vector<double>& r) const
  {
    ...
    {
      std::ostringstream error_stream;
      error_stream << "t > nprev_values() in SpikedLine:  " << t << " "
...
SpikedChannelSpineFlowProblem<ELEMENT>::SpikedChannelSpineFlowProblem()
{
  ...
  Problem::mesh_pt() = new ChannelSpineMesh<ELEMENT>(
...
int main()
{
  ...
  {
    // Build problem
    SpikedChannelSpineFlowProblem<SpineElement<QTaylorHoodElement<2>>> problem;
