  ...
  void deform_interface(const double& epsilon, const unsigned& n_periods)
  {
    ...
    for (unsigned n = 0; n < n_node; n++)
    {
      ...
      const double new_y_pos =
        current_y_pos + (1.0 - fabs(1.0 - current_y_pos)) * epsilon *
                          (cos(2.0 * n_periods * MathematicalConstants::Pi *
                               current_x_pos / Problem_Parameter::Length));
...
TwoLayerInterfaceProblem<ELEMENT>::TwoLayerInterfaceProblem()
{
  ...
  TriangleMeshPolyLine* interface_polyline_pt =
    new TriangleMeshPolyLine(vertex_coord, Interface_boundary_id);
  ...
  Vector<TriangleMeshClosedCurve*> inner_boundaries_pt(
    n_internal_closed_boundaries);
...
int main(int argc, char** argv)
{
  ...
  char filename[100];
  sprintf(
    filename, "%s/trace.dat", Problem_Parameter::Doc_info.directory().c_str());
