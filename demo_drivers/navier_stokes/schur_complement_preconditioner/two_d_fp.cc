                             ...
                             const int& problem_id)
{
  ...
  else
  {
    ...
    if (Problem_id == Global_Variables::Step)
    {
      ...
      l_y = 2.0;
      n_x =
        6 * n_el; // These are appropriate for n_el = number of elements across
      ...
      {
        mesh_pt() =
          new RefineableBackwardStepQuadMesh<RefineableQTaylorHoodElement<2>>(
    ...
    else
    {
      ...
      {
        mesh_pt() =
          new RefineableRectangularQuadMesh<RefineableQTaylorHoodElement<2>>(
            n_x, n_y, l_x, l_y);
      }
      else
      {
