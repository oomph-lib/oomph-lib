...
#include <math.h>
  double Diss_sarah(double rho, double zeta, double t)
  {
    ...
    double t8;
    {
      ...
      t39 = cos(-t19 + t22);
      t43 = cos(t22);
      return (t1 * t2 * t6 * t8 * Omega * t11 * t21 * t26 +
              4.0 * alpha * t2 * t6 * t14 * Omega * t10 * t20 * t25 *
...
#include <math.h>
  double Kin_energy_sarah(double t)
  {
    ...
    double t7;
    {
      ...
      t31 = sin(2.0 * t5 + t18);
      t33 = sqrt(2.0);
      return (t1 * (0.3141592653589793E1 * t2 / N * t7 / 2.0 +
                    t11 * 0.3141592653589793E1 * t13 * Omega * t16 * t6 * t20) +
              0.3141592653589793E1 * t1 * t2 * t11 *
                (t27 * (t28 * t31 / 4.0 + t33 * t28 / 4.0) -
                 2.0 * t28 * t16 * t6 * t20) /
                2.0);
    }
  }
...
//     t7 = cos(Omega*t);
//     t8 = t7*t7;
//     return(t1*0.3141592653589793E1*t3/N*t8/2.0);
//   }
// }
...
#include <math.h>
  double P_sarah(double rho, double zeta, double t)
  {
    ...
    double t9;
    {
      ...
      return (epsilon * (t1 / N * t4 * t6 * t9 -
                         t12 * Omega * (1.0 + A) * t4 * t6 * t19 / alpha));
    }
  }
...
#include <math.h>
  double Total_Diss_lead_sarah(double t)
  {
    ...
    double t6;
    {
      ...
      t13 = cos(t11);
      t16 = t13 * t13;
      return (-alpha * t1 * 0.3141592653589793E1 * t4 * t6 * t5 * t10 *
              (-1.0 + 2.0 * t12 * t13 - 2.0 * t16) / 8.0);
    }
  }
...
#include <math.h>
  double Total_Diss_sarah(double t)
  {
    ...
    double t8;
    {
      ...
      t19 = t18 * t18;
      t20 = sin(t6);
      return (4.0 * t2 * (N - 1.0) * t4 * t8 -
              alpha * t4 * 0.3141592653589793E1 * t14 * t15 * t1 * t19 *
                (-1.0 + 2.0 * t20 * t7 - 2.0 * t8) / 8.0 +
              t4 * t18 * t2 *
                (-10.0 * A * t8 - A + 8.0 * A * N * t8 + 22.0 * t8 - 1.0 -
                 24.0 * N * t8) /
                8.0);
    }
  }
...
#include <math.h>
  double U_sarah(double rho, double zeta, double t)
  {
    ...
    double t9;
    {
      ...
      return (
        epsilon *
          (t1 * Omega * t4 * t6 * t9 + t12 * Omega * t4 * t14 * t9 +
           (t1 * N * t19 * t22 * t6 * t25 + t12 * N * t19 * t22 * t14 * t25) *
             t35) -
        epsilon * t12 *
          (t14 * (Omega * t21 * t47 * t49 - t52) +
           t14 * (t55 * t21 * t59 - t62 * t64 * t9) * t35) +
        epsilon * t1 *
          (t52 * t6 + t6 * (-t55 * t21 * t59 - t62 * t43 * t3 * t9) * t35) -
        (-t12 * (-Omega * t14 * t87 - t89 * t19 * t21 * t14 * t25) +
         t1 * (Omega * t6 * t87 + t89 * t6 * t19 * t21 * t25)) *
          epsilon);
    }
  }
  ...
  IC_Fct_pt(IC_fct_pt)
{
  ...
  Wall_pt = new PseudoBucklingRingElement(
  ...
  // Make space for two submeshes
  // Sub_mesh_pt.resize(2);
  ...
  // Build mesh containing the single geometric wall element
  ...
  Veloc_trace_node_pt =
    fluid_mesh_pt()->finite_element_pt(0)->node_pt(nnode - 1);

  //  Extract pointer to node in symmetry plane (this node exists
  // at all refinement levels and can be used to doc continuous timetrace
  // of velocities)
  unsigned nnode_1d =
    dynamic_cast<ELEMENT*>(fluid_mesh_pt()->finite_element_pt(0))->nnode_1d();
  Sarah_veloc_trace_node_pt =
    fluid_mesh_pt()->finite_element_pt(0)->node_pt(nnode_1d - 1);

  // The "pseudo-elsatic" wall element is "loaded" by a pressure.
  // Use the "constant" pressure component in Crouzeix Raviart
  // fluid element as that pressure.
  dynamic_cast<PseudoBucklingRingElement*>(Wall_pt)->set_reference_pressure_pt(
    fluid_mesh_pt()->element_pt(0)->internal_data_pt(0));
  ...
  SarahBL::epsilon =
    static_cast<PseudoBucklingRingElement*>(Wall_pt)->eps_buckl();
  SarahBL::alpha = sqrt(Global_Physical_Variables::Re);
  SarahBL::A = static_cast<PseudoBucklingRingElement*>(Wall_pt)->ampl_ratio();
  SarahBL::N =
    static_cast<PseudoBucklingRingElement*>(Wall_pt)->n_buckl_float();
...
void OscRingNStProblem<ELEMENT, TIMESTEPPER>::doc_solution(DocInfo& doc_info)
{
  ...
  //---------------
  sprintf(
    filename, "%s/Wall%i.dat", doc_info.directory().c_str(), doc_info.number());
  some_file.open(filename);
  ...
  {
    xi_wall[0] =
      0.5 * MathematicalConstants::Pi * double(iplot) / double(nplot - 1);
  ...
  sprintf(filename,
          ...
          doc_info.number());
  some_file.open(filename);
  fluid_mesh_pt()->output_fct(some_file,
                              npts,
  ...
  for (unsigned ielem = 0; ielem < nelem; ielem++)
  {
    ...
    press_int += dynamic_cast<ELEMENT*>(fluid_mesh_pt()->element_pt(ielem))
                   ->pressure_integral();
    diss +=
      dynamic_cast<ELEMENT*>(fluid_mesh_pt()->element_pt(ielem))->dissipation();
    kin_en +=
      dynamic_cast<ELEMENT*>(fluid_mesh_pt()->element_pt(ielem))->kin_energy();
  }
  ...
  Trace_file << time_pt()->time() << " " << r[1] << " " << global_kin << " "
             ...
             << area << " " << press_int / area << " " << diss << " "
             << diss_asympt << " " << Veloc_trace_node_pt->x(0) << " "
             << Veloc_trace_node_pt->x(1) << " "
             << Veloc_trace_node_pt->value(0) << " "
             << Veloc_trace_node_pt->value(1) << " "
             << fluid_mesh_pt()->nelement() << " " << ndof() << " " << min_level
             << " " << max_level << " "
             ...
             << doc_info.number() << " " << Sarah_veloc_trace_node_pt->x(0)
  ...
  fluid_mesh_pt()->forest_pt()->stick_all_tree_nodes_into_vector(
    all_element_pt);
  ...
  DocInfo& doc_info)
{
  ...
  Trace_file << time_pt()->time() << " " << r[1] << " " << sarah_global_kin
             << " " << global_kin << " " << global_kin << " " << global_length
             << " ";
  ...
  // Get veloc in Sarah's soln
  {
    double rho = 0.5;
    double zeta = 0.0;
    Trace_file << SarahBL::X_sarah(
                    rho, zeta, time_stepper_pt()->time_pt()->time())
               << " ";
    Trace_file << SarahBL::Y_sarah(
                    rho, zeta, time_stepper_pt()->time_pt()->time())
               << " ";
    Trace_file << SarahBL::U_sarah(
                    rho, zeta, time_stepper_pt()->time_pt()->time())
               << " ";
    Trace_file << SarahBL::V_sarah(
                    rho, zeta, time_stepper_pt()->time_pt()->time())
               << " ";
  }
  ...
  const unsigned& ntsteps, const bool& restarted, DocInfo& doc_info)
{
  ...
  cout << "\n Initial mesh: min/max refinement levels: " << min_refinement_level
...
void OscRingNStProblem<ELEMENT, TIMESTEPPER>::write_trace_file_header()
{
  ...
  Trace_file << "# err_max " << fluid_mesh_pt()->max_permitted_error()
             << std::endl;
  Trace_file << "# err_min " << fluid_mesh_pt()->min_permitted_error()
             << std::endl;
...
//========================================================================
/// Write header for (historic) trace file -- maintained for
/// backwards compatibility
//========================================================================
template<class ELEMENT, class TIMESTEPPER>
void OscRingNStProblem<ELEMENT, TIMESTEPPER>::write_trace_file_header_historic()
{
  ...
  Trace_file << "# max_refinement_level "
             << fluid_mesh_pt()->max_refinement_level() << std::endl;
  Trace_file << "# max_adapt " << 1 << std::endl;
...
int main(int argc, char* argv[])
{
  ...
  else if (CommandLineArgs::Argc == 2)
  {
    ...
    {
      cout << "Have opened " << CommandLineArgs::Argv[1] << " for restart. "
           << std::endl;
    }
    else
    {
      std::ostringstream error_stream;
      error_stream << "ERROR while trying to open " << CommandLineArgs::Argv[1]
