...
namespace GlobalParameters
{
  ...
  void doc_navier_stokes_parameters()
  {
    ...
    oomph_info << ANSIEscapeCode::Red
               << "Solving for (Re,ReSt): " << ANSIEscapeCode::Reset << "("
  ...
  double Annular_region_radius = std::min(
    Radius + 1.0, Radius + 0.5 * ((0.5 * Length_of_central_box) - Radius));
  ...
  void doc_maximum_central_box_deformation()
  {
    ...
    else
    {
      ...
      oomph_info << "\033[1;31m"
                 ...
                 << "\033[0m" << deformation_ratio * 100.0 << "%\n"
                 << std::endl;
    }
...
FlowAroundCylinderProblem<ELEMENT>::FlowAroundCylinderProblem() :
  Bulk_mesh_pt(0), Solver_pt(0), Prec_pt(0), F_matrix_preconditioner_pt(0)
{
  // Use BDF2
...
void FlowAroundCylinderProblem<ELEMENT>::apply_boundary_conditions()
{
  ...
  for (unsigned b = 0; b < n_boundary; b++)
  {
    ...
    if ((b == Lower_wall_boundary_id) || (b == Upper_wall_boundary_id) ||
        (b == Inflow_boundary_id) || (b == Cylinder_surface_boundary_id))
    {
      // Loop over the nodes
...
void FlowAroundCylinderProblem<ELEMENT>::unsteady_simulation()
{
  ...
  oomph_info << "\nTimestepping with "
             ...
             << " steps per period, corresponding to a timestep of " << dt
             << std::endl;
  ...
  for (unsigned t = 1; t <= n_timestep; t++)
  {
    ...
    // Tell the user
    oomph_info << "Obtained solution at timestep " << t << " out of "
               << n_timestep << "." << std::endl;
  }
