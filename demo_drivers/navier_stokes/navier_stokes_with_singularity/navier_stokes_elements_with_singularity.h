...
public:
  ...
  /// \short Set pointer to associated wrapped Navier-Stokes element which
  ...
  /// \short Evaluate gradient of velocity singular function
...
private:
  ...
  /// \short Pointer to gradient of velocity singular function;
  /// grad[i][j] = du_i/dx_j
  NavierStokesGradVelocitySingularFctPt Grad_velocity_singular_fct_pt;

  /// \short Pointer to pressure singular function
  NavierStokesPressureSingularFctPt Pressure_singular_fct_pt;

  /// \short Pointer to gradient of pressure singular function
  NavierStokesGradPressureSingularFctPt Grad_pressure_singular_fct_pt;
...
public:
  ...
  /// \short Add pointer to associated SingularNavierStokesSolutionElement that
  /// determines the value of the amplitude of the singular functions (and gives
  /// access to the singular functions). The unknown amplitude becomes external
  /// Data for this element so assign_eqn_numbers() must be called after this
    ...
    const unsigned& flag)
  {
    ...
    for (unsigned j = 0; j < n_node; j++)
    {
      if (!(this->node_pt(j)->time_stepper_pt()->is_steady()))
      {
        ...
        error_stream
          ...
          << "   "
             "NavierStokesElementWithSingularity::fill_in_generic_residual_"
             "contribution_wrapped_nst()\n\n"
          << std::endl;
        throw OomphLibError(
          error_stream.str(), OOMPH_CURRENT_FUNCTION, OOMPH_EXCEPTION_LOCATION);
      }
    ...
    if ((!all_singular_functions_satisfy_stokes_equation) && (flag == 1))
    {
      ...
      error_stream
        ...
        << "  "
           "SingularNavierStokesSolutionElement::singular_function_satisfies_"
           "stokes_equation()"
        << "\n\n\n"
        << "for the SingularNavierStokesSolutionElement that specifies the "
           "singular functions.\n"
        ...
        << "Alternatively, use FD-based setup of Jacobian (still there, "
           "commented out, in code).\n"
        << "The reason this never got implemented is because this case "
           "typically only arises\n"
        << "when trying to \"blend\" solutions (to give them finite support) "
           "and this didn't\n"
        << "seem to work too well and was sort of abandoned. Next person to "
           "look at this\n"
        << "should consider only adding the non-integrated by parts "
           "(source-fct-like) terms\n"
