...
namespace Global
{
                      ...
                      Vector<double>& u)
  {
    ...
    // The density
    u[0] = pow((1.0 - ((Gamma - 1.0) / (16 * Gamma * pi * pi)) * Beta * Beta *
                        exp_term * exp_term),
               (1.0 / (Gamma - 1.0)));
             ...
             TimeStepper* time_stepper_pt = &Mesh::Default_TimeStepper)
  {
    ...
    for (unsigned ex = 0; ex < Nx; ex++)
    {
      ...
      for (unsigned ey = 0; ey < Ny; ey++)
      {
        ...
        double ll_corner[2] = {llx + ex * el_length[0],
                               lly + ey * el_length[1]};
    ...
    for (unsigned ex = 0; ex < Nx; ex++)
    {
      for (unsigned ey = 0; ey < Ny; ey++)
      {
        ...
        if (ey > 0)
        {
          index[2] = element_index - 1;
        }
        else
        {
          index[2] = element_index + Ny - 1;
        }

        // West neighbour
        if (ex > 0)
        {
          index[3] = element_index - Ny;
        }
        else
        {
          index[3] = element_index + (Nx - 1) * Ny;
        }
...
public:
  ...
  TwoDDGProblem(const unsigned& Nx, const unsigned& Ny)
  {
    ...
    for (unsigned b = 0; b < 4; b++)
    {
      ...
      for (unsigned n = 0; n < n_node; n++)
      {
        ...
        for (unsigned i = 0; i < n_value; i++)
        {
          nod_pt->set_value(i, u_initial[i]);
        }
      }
    }
  ...
  void compute_error(const double& t, Vector<double>& error)
  {
    ...
    for (unsigned e = 0; e < n_element; e++)
    {
      dynamic_cast<ELEMENT*>(Problem::mesh_pt()->element_pt(e))
        ->compute_error(
          std::cout, Global::exact_solution, t, local_error, local_norm);
      for (unsigned i = 0; i < 4; i++)
      {
  ...
  void parameter_study(std::ostream& trace, const bool& disc)
  {
    ...
    for (unsigned t = 0; t < 50; t++)
    {
      ...
      if (count == 50)
      {
        ...
        {
          sprintf(
            filename, "disc_%li_time%g.dat", mesh_pt()->nelement(), time());
        }
        else
        {
          sprintf(
            filename, "cont_%li_time%g.dat", mesh_pt()->nelement(), time());
        }
      ...
      trace << mesh_pt()->nelement() << " " << this->time() << " "
            << sqrt(error[0]) << " " << sqrt(error[1]) << " " << sqrt(error[2])
...
int main()
{
  ...
  {
    TwoDDGProblem<DGSpectralEulerElement<2, 2>> problem(n_element, n_element);
