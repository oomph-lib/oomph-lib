  ...
  const bool &fluid_jacobian_ignores_geometric_data)
{
  ...
  if (Control_Flags::Steady_run)
  {
    ...
    oomph_info << "Displacement control for element: " << nel_ctrl << std::endl;
    oomph_info << "Displacement control applied at xi = (" << xi[0] << ")"
               << std::endl;
    oomph_info << "Corresponding to                x  = (" << x[0] << ", "
               << x[1] << ")" << std::endl;
...
int main(int argc, char *argv[])
{
  ...
  // with the Jacobian approximations otherwise the Newton method converges,
  // this is not the experience found with the 3D equivalent problem.
