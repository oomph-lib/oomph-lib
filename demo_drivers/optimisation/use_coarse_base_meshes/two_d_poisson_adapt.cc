...
public:
  ...
  RefineablePoissonProblem(
    PoissonEquations<2>::PoissonSourceFctPt source_fct_pt, const int& n_power);
  ...
  {
    Problem::Problem_is_nonlinear = true;
  }
...
void RefineablePoissonProblem<ELEMENT>::doc_solution(DocInfo& doc_info)
{
  ...
  //----------------------------
  {
    ...
    cout << "------------------------------------------------------- "
         << std::endl;
    cout << "Total time for C++ style output [sec]: " << total_time
         << std::endl;
    cout << "------------------------------------------------------- "
         << std::endl;
  }
...
int main(int argc, char* argv[])
{
  ...
  for (unsigned i = 0; i < 2; i++)
  {
    ...
    RefineablePoissonProblem<RefineableQPoissonElement<2, 3>> problem(
      &TanhSolnForPoisson::get_source, n_power);
