...
class UnsteadyHeatProblem : public virtual Problem
{
  ...
  typedef SpaceTimeUnsteadyHeatEquations<2>::SpaceTimeUnsteadyHeatSourceFctPt
    SpaceTimeFunctionPt;
...
public:
  ...
  // /// \short Assign the dof type to each dof according to the temporal
  // position
  // /// of their associated Node. This allows the block preconditioner to group
  // /// dofs in the same time-slice together.
  // /// NOTE: The boundary conditions need to be applied before this function
  // is
  // /// invoked to ensure that the correct number of time slide IDs are
  // assigned. void assign_time_slice_id();
