...
public:
  ...
  void position(const Vector<double>& xi, Vector<double>& r) const
  {
    ...
    // Position vector
    r[0] = (A + A_hat * sin(2.0 * MathematicalConstants::Pi * time / T)) *
           cos(xi[0]);
    r[1] = (B + B_hat * sin(2.0 * MathematicalConstants::Pi * time / T)) *
           sin(xi[0]);
                   ...
                   Vector<double>& u)
  {
    ...
    u[0] = tanh(
      ...
      Alpha *
        (TanPhi *
           (X - Beta *
                  tanh(Gamma * cos(0.2E1 * MathematicalConstants::Pi * time))) -
         Y));
  }
    ...
    source =
      -2.0 *
        tanh(0.1E1 -
             Alpha * (TanPhi *
                        (X - Beta * tanh(Gamma *
                                         cos(0.2E1 * MathematicalConstants::Pi *
                                             time))) -
                      Y)) *
        (1.0 -
         pow(tanh(0.1E1 -
      ...
      0.2E1 *
        (1.0 -
         pow(tanh(0.1E1 -
             ...
             2.0)) *
        Alpha * TanPhi * Beta *
        (1.0 - pow(tanh(Gamma * cos(0.2E1 * MathematicalConstants::Pi * time)),
                   2.0)) *
        Gamma * sin(0.2E1 * MathematicalConstants::Pi * time) *
      ...
      (1.0 -
       pow(tanh(0.1E1 -
                Alpha *
                  (TanPhi *
                     (X - Beta *
                            tanh(Gamma * cos(0.2E1 * MathematicalConstants::Pi *
                                             time))) -
                   Y)),
           2.0)) *
        Alpha * NY;
...
void RefineableUnsteadyHeatProblem<ELEMENT>::doc_solution()
{
  ...
  Bulk_mesh_pt->output_fct(
    some_file, npts, time_pt()->time(), TanhSolnForUnsteadyHeat::get_exact_u);
  ...
  Trace_file << time_pt()->time() << " " << Doc_node_pt->value(0) << " "
             << u_exact << " " << r_wall[0] << " "
             << TanhSolnForUnsteadyHeat::step_position(time_pt()->time()) << " "
             << Bulk_mesh_pt->nelement() << " " << Bulk_mesh_pt->nrefined()
             << " " << Bulk_mesh_pt->nunrefined() << " " << error << " " << norm
             << std::endl;
...
int main(int argc, char* argv[])
{
  ...
  // Build problem
  RefineableUnsteadyHeatProblem<RefineableQUnsteadyHeatElement<2, 3>> problem(
    &TanhSolnForUnsteadyHeat::get_source);
