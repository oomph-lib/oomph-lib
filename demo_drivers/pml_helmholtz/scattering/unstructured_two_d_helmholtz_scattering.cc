...
namespace GlobalParameters
{
  ...
  void get_exact_u(const Vector<double> &x, Vector<double> &u)
  {
    ...
    Vector<double> jn(N_fourier + 1), yn(N_fourier + 1), jnp(N_fourier + 1),
      ynp(N_fourier + 1);
    ...
    int n_actual = 0;
    CRBond_Bessel::bessjyna(N_fourier,
                            Wavenumber,
                            n_actual,
                            &jn_a[0],
                            &yn_a[0],
    ...
    if (n_actual != int(N_fourier))
    {
      ...
      error_stream << "CRBond_Bessel::bessjyna() only computed " << n_actual
                   << " rather than " << N_fourier << " Bessel functions.\n";
      throw OomphLibError(
    ...
    {
      u_ex -= pow(I, i) * h[i] * jnp_a[i] *
              pow(exp(I * theta), static_cast<double>(i)) / hp_a[i];
    }
  ...
  void prescribed_incoming_flux(const Vector<double> &x, complex<double> &flux)
  {
    ...
    CRBond_Bessel::bessjyna(
      N_fourier, rr, n_actual, &jn[0], &yn[0], &jnp[0], &ynp[0]);
  ...
  public:
                               ...
                               const double &alpha_shift = 0.0)
    {
      ...
      return 1.0 +
             (1.0 / std::complex<double>(sqrt(k_squared_local), 0)) *
               std::complex<double>(0.0, 2.0 / (std::fabs(pml_width_i - nu_i)));
    }
  };
...
void PMLProblem<ELEMENT>::doc_solution(DocInfo &doc_info)
{
  ...
  Bulk_mesh_pt->compute_error(
    some_file, GlobalParameters::get_exact_u, error, norm);
  some_file.close();
...
int main(int argc, char **argv)
{
  ...
  PMLProblem<ProjectablePMLHelmholtzElement<TPMLHelmholtzElement<2, 3>>>
    problem;
