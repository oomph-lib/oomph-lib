...
namespace oomph
{
  ...
  public:
    ...
    /// x is a Vector!
    typedef void (*ExactSolnPt)(const Vector<double>& x,
    ...
    virtual inline unsigned u_index() const
    {
    ...
    ExactSolnPt& exact_pt()
    {
      return Exact_soln_pt;
    }

    /// Access function: Pointer to source function. Const version
    ExactSolnPt exact_pt() const
    {
      return Exact_soln_pt;
    }
    ...
    inline Vector<double> interpolated_u(const Vector<double>& s)
    {
      ...
      DenseMatrix<double> interpolated_dudxi(1, 2, 0.0),
        interpolated_d2udxi(1, 3, 0.0);
      DenseMatrix<double> M(21, 36, 0.0);
      this->my_interpolated_u_normal1(
        s, u, interpolated_dudxi, interpolated_d2udxi, M);
  ...
  //======================================================================
  template<unsigned DIM, unsigned NNODE_1D>
  class BiharmonicCurvedElement :
    public virtual MyBiharmonicEquations<DIM, NNODE_1D>
  {
  private:
...
namespace oomph
{
      ...
      const unsigned& flag)
  {
    ...
    for (unsigned ipt = 0; ipt < n_intpt; ipt++)
    {
      ...
      double J = d2shape_and_d2test_eulerian_at_knot_biharmonic(
        ipt, psi, dpsidxi, d2psidxi, test, dtestdxi, d2testdxi);
      ...
      DenseMatrix<double> M(21, 36, 0.0);
      this->my_interpolated_u_normal1(
        s, interpolated_u, interpolated_dudxi, interpolated_d2udxi, M);
        ...
        for (unsigned l = 0; l < 3; l++)
        {
          for (unsigned k = 0; k < n_position_type; k++)
          {
            ...
            if (local_eqn >= 0)
            {
              ...
              residuals[local_eqn] +=
                (interpolated_d2udxi(0, 0) * d2psidxi(l, k, 0) * W +
                 interpolated_d2udxi(0, 1) * d2psidxi(l, k, 1) * W +
                 2.0 * interpolated_d2udxi(0, 2) * d2psidxi(l, k, 2) * W);
                  ...
                  for (unsigned k2 = 0; k2 < n_position_type; k2++)
                  {
                    ...
                    if (local_unknown >= 0)
                    {
                      ...
                      jacobian(local_eqn, local_unknown) +=
                        d2psidxi(l2, k2, 0) * d2psidxi(l, k, 0) * W +
                        d2psidxi(l2, k2, 1) * d2psidxi(l, k, 1) * W +
                        2.0 * d2psidxi(l2, k2, 2) * d2psidxi(l, k, 2) * W;
                    }
                  }
                }
              } // End of flag
          ...
          if (local_eqn >= 0)
          {
            for (unsigned j = 0; j < 36; j++)
            {
              if (M(row, j) != 0)
              {
                ...
                residuals[local_eqn] +=
                  (interpolated_d2udxi(0, 0) * d2psi_curve(j, 0) +
                   interpolated_d2udxi(0, 1) * d2psi_curve(j, 1) +
                   2.0 * interpolated_d2udxi(0, 2) * d2psi_curve(j, 2)) *
                  W * M(row, j);
                    ...
                    if (local_unknown >= 0)
                    {
                      for (unsigned j2 = 0; j2 < 36; j2++)
                      {
                        if (M(row2, j2) != 0)
                        {
                          // Add contribution to Elemental Matrix
                          jacobian(local_eqn, local_unknown) +=
                            (d2psi_curve(j2, 0) * d2psi_curve(j, 0) +
                             d2psi_curve(j2, 1) * d2psi_curve(j, 1) +
                             2.0 * d2psi_curve(j2, 2) * d2psi_curve(j, 2)) *
                            W * M(row2, j2) * M(row, j);
                        }
                      }
                    }
  ...
  Source_fct_pt(source_fct_pt), Exact_soln_pt(exact_pt)
{
  ...
  for (unsigned n = 0; n < n_element; n++)
  {
    ...
    // Loop over nodes in an element to check whether it is a (curved)boundary
    // element or not
    unsigned count = 0;
    ...
    for (unsigned i = 0; i < nnode; i++)
    {
      ...
      if (i > 2)
      {
        ...
        {
          // elem_pt->node_pt(i)->pin(0); // supposed to be free for the curved
          // boundary
  ...
  for (unsigned n = 0; n < n_element; n++)
  {
    ...
    if (bd_element == 1)
    {
      ...
      if (angle_1 > angle_2)
      {
        ...
        else if (position(1, 0) == bd_position(0, 0) &&
                 position(1, 1) == bd_position(0, 1))
        {
      ...
      else if (angle_1 < angle_2)
      {
        ...
        {
          if (position(2, 0) == bd_position(1, 0) &&
              position(2, 1) == bd_position(1, 1))
          {
...
//========================================================================
template<class ELEMENT, unsigned DIM, unsigned NNODE_1D>
void MyBiharmonicProblem<ELEMENT, DIM, NNODE_1D>::doc_solution(
  DocInfo& doc_info)
{
  ofstream some_file;
  char filename[100];
  ...
  mesh_pt()->compute_error(
    some_file, Physical_Variables::get_exact_u, error, norm);
  some_file.close();
...
int main(int argc, char* argv[])
{
  ...
  // Set up the problem:
  MyBiharmonicProblem<BiharmonicCurvedElement<2, 2>,
                      2,
                      2> // Element type as template parameter
    problem(Physical_Variables::source_function,
            Physical_Variables::get_exact_u,
            node_file_name,
            element_file_name,
            poly_file_name);
