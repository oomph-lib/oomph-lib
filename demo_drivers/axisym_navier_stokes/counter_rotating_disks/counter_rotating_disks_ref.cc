...
// Navier-Stokes equations in cylindrical polar coordinates which replicates
// the investigation of C. Nore, M. Tartar, O. Daube and L.S. Tuckerman's
...
public:
  ...
  /// \short Actions before the timestep (empty)
  ...
  void actions_after_adapt()
  {
    ...
    RefineableAxisymmetricNavierStokesEquations::unpin_all_pressure_dofs(
      mesh_pt()->element_pt());

    // Pin redudant pressure dofs
    RefineableAxisymmetricNavierStokesEquations::pin_redundant_nodal_pressures(
      mesh_pt()->element_pt());
                                                 ...
                                                 const double& domain_height)
{
  ...
  Problem::mesh_pt() = new RefineableRectangularQuadMesh<BASE_ELEMENT>(
    n_r, n_z, 0.0, 1.0, 0.0, domain_height);
...
public:
  ...
  void actions_after_adapt()
  {
    ...
    if (GlobalPhysicalVariables::k == 0)
    {
      fix_pressure(0, 0, 0.0);
    }
  ...
  /// \short Set the three velocity components to a smoothly varying,
  /// non-zero profile
  void set_initial_condition();
  ...
  Base_state_mesh_pt(external_mesh_pt), Domain_height(domain_height)
{
  ...
  // Build and assign mesh
  Problem::mesh_pt() = new RefineableRectangularQuadMesh<PERTURBED_ELEMENT>(
    n_r, n_z, 0.0, 1.0, 0.0, domain_height, time_stepper_pt());
...
//=======================================================================
template<class BASE_ELEMENT, class PERTURBED_ELEMENT>
void PerturbedStateProblem<BASE_ELEMENT,
                           PERTURBED_ELEMENT>::set_initial_condition()
{
  // Determine number of nodes in mesh
  const unsigned n_node = mesh_pt()->nnode();
...
template<class BASE_ELEMENT, class PERTURBED_ELEMENT>
void PerturbedStateProblem<BASE_ELEMENT,
                           PERTURBED_ELEMENT>::set_boundary_conditions()
{
  // Determine number of mesh boundaries
  const unsigned n_boundary = mesh_pt()->nboundary();
...
/// \short Container class for the Tuckerman counter-rotating lids
/// stability problem
//=======================================================================
template<class BASE_ELEMENT, class PERTURBED_ELEMENT>
...
public:
                   ...
                   const double& domain_height)
  {
    ...
    Base_state_problem_pt =
      new BaseStateProblem<BASE_ELEMENT>(base_n_r, base_n_z, domain_height);
  ...
  const double& dt, const unsigned& n_timesteps, DocInfo& doc_info)
{
  ...
  for (unsigned i = 0; i < n_timesteps; i++)
  {
    ...
    std::cout << "  Timestep " << i + 1 << " of " << n_timesteps << "; time is "
              << Perturbed_state_problem_pt->time_pt()->time() << std::endl;
                       ...
                       DoubleVector& input)
{
  ...
  {
    cout << "\nThis is power method loop number " << p + 1 << " of a maximum "
         << max_iter << "." << endl;
    ...
    for (unsigned i = 0; i < n_dof_perturbed; i++)
    {
      convergence_criterion += (output[i] - calc_eigenvalue * input[i]) *
  ...
  cout << "\nPower method has failed to converge to within a tolerance of "
       << tolerance << "\nin " << max_iter << " iterations of the power method"
       << endl;
...
int main(int argc, char* argv[])
{
  ...
  // --------------------------------------------------------------------
  {
    ...
    StabilityProblem<
      ...
      problem_TH(
        base_n_r, base_n_z, perturbed_n_r, perturbed_n_z, domain_height);
    ...
    trace.open(filename);
    trace << "Re, dominating eigenvalue, n_power_method_iterations"
          << std::endl;
    ...
    const unsigned temp_array[22] = {0,  1,  4,  5,  8,  9,  12, 13,
                                     15, 29, 31, 45, 47, 50, 51, 54,
                                     55, 58, 59, 61, 62, 63};
    Vector<unsigned> els_to_refine;
    for (unsigned i = 0; i < 22; i++)
    {
      els_to_refine.push_back(temp_array[i]);
    }

    // Refine selected elements
    problem_TH.base_state_problem_pt()->refine_selected_elements(els_to_refine);
    problem_TH.perturbed_state_problem_pt()->refine_selected_elements(
      els_to_refine);
    ...
    const unsigned n_power_method_iterations =
      problem_TH.perform_power_method(dt,
                                      nstep,
                                      doc_info,
    ...
    trace << GlobalPhysicalVariables::Re << " " << eigenvalue << " "
  ...
  // --------------------------------------------------------------------
  {
    ...
    problem_CR.perturbed_state_problem_pt()->refine_selected_elements(
      els_to_refine);
    ...
    // Perform power method and store number of iterations
    const unsigned n_power_method_iterations =
      problem_CR.perform_power_method(dt,
                                      nstep,
                                      doc_info,
                                      power_method_tolerance,
                                      max_iter,
