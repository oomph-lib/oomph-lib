...
namespace oomph
{
  ...
  //==============================================================
  class MyTaylorHoodElement : public virtual PseudoSolidNodeUpdateElement<
...
public:
  ...
  void position(const Vector<double>& xi, Vector<double>& r) const
  {
    ...
    {
      r_square[0] =
        1.0 / Global_Physical_Variables::Delta + Radius * tan(0.5 * pi - xi[0]);
...
public:
  ...
  void actions_after_adapt()
  {
    ...
    for (unsigned b = 0; b < n_boundary; ++b)
    {
      Backed_up_surface_mesh_pt[b]->project_onto_new_mesh(
        this->Lagrange_multiplier_mesh_pt[b]);
    }
    ...
    for (unsigned e = 0; e < n_element; e++)
    {
      ...
      // Set the body force
      el_pt->axi_nst_body_force_fct_pt() =
        &Global_Physical_Variables::axial_pressure_gradient;
    ...
    for (unsigned b = 0; b < n_boundary; b++)
    {
      ...
      if (boundary_geom_obj_pt != 0)
      {
        ...
        for (unsigned e = 0; e < n_element; e++)
        {
          ...
          el_pt->set_boundary_shape_geom_object_pt(boundary_geom_obj_pt, b);
  ...
  const double& min_error_target, const double& max_error_target)
{
  ...
  curvilinear_boundary_pt[0] = new TriangleMeshCurviLine(
    area_pt, zeta_start, zeta_end, nsegment, boundary_id);
  ...
  triangle_mesh_parameters.internal_closed_curve_pt() = Inner_hole_pt;
  ...
  const double& time)
{
  ...
  for (unsigned b = 0; b < n_boundary; ++b)
  {
    ...
    for (unsigned n = 0; n < n_boundary_node; n++)
    {
      ...
      Fluid_mesh_pt->boundary_node_pt(b, n)->set_value(
        2, r * (1.0 - exp(-20.0 * time)));
    }
  }
}
