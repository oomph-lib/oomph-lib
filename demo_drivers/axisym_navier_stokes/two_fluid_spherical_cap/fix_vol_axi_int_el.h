...
#ifndef FIXED_VOLUME_AXISYMMETRIC_INTERFACE_ELEMENTS_HEADER
#define FIXED_VOLUME_AXISYMMETRIC_INTERFACE_ELEMENTS_HEADER
...
#include "fluid_interface/axisym_interface_elements.h"
...
class SpineAxisymmetricVolumeConstraintPointElement : 
   ...
   if(local_eqn >= 0)
    {
     ...
     //The integral of x.n on the RHS of the boundary is just r*r**x(1)
     //of the top right-hand corner, we divide by three because we are working
     //in two dimensions and then subtract the desired volume
     residuals[local_eqn] = 
      (1.0/3.0)*nod_pt->x(0)*nod_pt->x(0)*nod_pt->x(1) 
      - *Volume_pt;
    }
  }
...
//======================================================================
template<class ELEMENT>
class FixVolSpineAxisymmetricFluidInterfaceElement : 
   ...
   if(local_eqn >=0)
    {
     ...
     residuals[local_eqn] += (1.0/3.0)*r*dot*W*J;
    }
  }
...
public:
 ...
 /// and the face index
 /// We simply call the underlying constructor
   ...
   if (traded_pressure_data_pt->nvalue()!=1)
    {
     ...
     std::string function_name = 
      "FixedVolumeSpineAxisymmetricFluidInterfaceElement::\n";
     function_name += "set_traded_pressure_data()";
 ...
 FluidInterfaceEdgeElement* make_edge_element(const int &face_index)
  {
   ...
   SpineAxisymmetricVolumeConstraintPointElement<ELEMENT> *Temp_pt =
    new SpineAxisymmetricVolumeConstraintPointElement<ELEMENT>;
