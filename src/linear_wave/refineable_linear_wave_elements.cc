...
namespace oomph
{
      ...
      Vector<double>& residuals, DenseMatrix<double>& jacobian, unsigned flag)
  {
    ...
    for (unsigned ipt = 0; ipt < n_intpt; ipt++)
    {
      ...
      for (unsigned l = 0; l < n_node; l++)
      {
        ...
        ddudt += this->d2u_dt2_lin_wave(l) * psi(l);
      ...
      for (unsigned l = 0; l < n_node; l++)
      {
        ...
        for (unsigned m = 0; m < n_master; m++)
        {
          ...
          if (local_eqn >= 0)
          {
            ...
            residuals[local_eqn] +=
              (ddudt + source) * test(l) * W * hang_weight;

            // Laplace operator itself
                ...
                for (unsigned m2 = 0; m2 < n_master2; m2++)
                {
                  ...
                  if (local_unknown >= 0)
                  {
                    ...
                    jacobian(local_eqn, local_unknown) +=
                      test(l) * psi(l2) *
                      this->node_pt(l2)->time_stepper_pt()->weight(2, 0) * W *
                    ...
                    {
                      jacobian(local_eqn, local_unknown) +=
                        dpsidx(l2, k) * dtestdx(l, k) * W * hang_weight *
                        hang_weight2;
                    }
                  }
  ...
  template class RefineableQLinearWaveElement<3, 2>;
  template class RefineableQLinearWaveElement<3, 3>;
  template class RefineableQLinearWaveElement<3, 4>;
