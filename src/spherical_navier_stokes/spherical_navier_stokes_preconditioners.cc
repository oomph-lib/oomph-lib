...
//===========================================================================
 void SphericalNavierStokesLSCPreconditioner::identify_required_blocks(
...
//===========================================================================
/// Setup the least-squares commutator SphericalNavier Stokes preconditioner. This
  ...
  if (P_matrix_using_scaling)
   {
    ...
    // function that assembles it from the SphericalNavier-Stokes elements
    // in the mesh.
  ...
  if (matrix.ncol() != diag_matrix.size() )
   {
    ...
    throw OomphLibError(error_message.str(),
                     	"SphericalNavierStokesLSCPreconditioner::mat_diag_multiply()",
                        OOMPH_EXCEPTION_LOCATION);
   }
#endif
...
//========================================================================
 ...
 assemble_velocity_mass_matrix_diagonal(Vector<double> &vmm_diagonal)
 {
  ...
  // pointers to the only  possible element types
  // (required for accessing the function get_velocity_mass_matrix_diagonal)
  SphericalNavierStokesEquations* el_pt=0;
  ...
  for(unsigned e=0; e<n_el; e++)
   {
    ...
    el_pt = dynamic_cast< SphericalNavierStokesEquations* >
     ( Mesh_pt[0]->element_pt(e) );
    el_pt->get_velocity_mass_matrix_diagonal(el_vmm_diagonal);
