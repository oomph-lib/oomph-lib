...
namespace oomph
{
  ...
  public:
    ...
    void get_Z2_flux(const Vector<double>& s, Vector<double>& flux)
    {
      ...
      if (flux.size() < num_entries)
      {
        ...
        error_message << "The flux vector is too small, size " << flux.size()
      ...
      const Vector<GeneralisedElement*>& element_pt)
    {
      ...
      {
        dynamic_cast<RefineableSphericalNavierStokesEquations*>(element_pt[e])
  ...
  public:
                                 ...
                                 Vector<double>& values)
    {
      ...
      {
        values[i] = interpolated_u_spherical_nst(s, i);
      }
                             ...
                             const int& n_value) const
    {
      if (n_value == 3)
      {
        return this->pshape_spherical_nst(s, psi);
      }
      else
      {
