...
//=======================================================================
  ...
  Vector<double> &residuals, DenseMatrix<double> &jacobian, unsigned flag)
 {
  ...
  int local_eqn_real=0, local_eqn_imag=0, 
      local_unknown_real=0, local_unknown_imag=0;
        ...
        if(local_eqn_real >= 0)
         {
          ...
          residuals[local_eqn_real] += 
           (-omega_sq.real()*interpolated_u[a].real()
            +omega_sq.imag()*interpolated_u[a].imag()
            -b[a].real())*psi(l)*r*W;
            ...
            residuals[local_eqn_real] += 
             (
              mu.real()*(
               ...
               n*psi(l)/r*(n*ur.real()/r+
                           duthdr.imag()-uth.imag()/r)
               +dpsidx(l,1)*(durdz.real()+duzdr.real())+
               2.0*psi(l)/pow(r,2)*(ur.real()-n*uth.imag())
               )
              +mu.imag()*(
               -2.0*durdr.imag()*dpsidx(l,0)+
               n*psi(l)/r*(-n*ur.imag()/r+duthdr.real()-uth.real()/r)
               -dpsidx(l,1)*(durdz.imag()+duzdr.imag())-2.0*psi(l)/pow(r,2)*
               (ur.imag()+n*uth.real())
               )
              +lambda.real()*(
               durdr.real()+ur.real()/r-n/r*uth.imag()+duzdz.real()
               )*(dpsidx(l,0)+psi(l)/r)
              -lambda.imag()*(
               durdr.imag()+ur.imag()/r+n/r*uth.real()+duzdz.imag()
               )*(dpsidx(l,0)+psi(l)/r)
              )*r*W;
           }    
            ...
            residuals[local_eqn_real] +=
             (
              mu.real()*(
               ...
               (dpsidx(l,0)-psi(l)/r)  
               +2.0*n*psi(l)/pow(r,2)*(n*uth.real()+ur.imag())+
               dpsidx(l,1)*(duthdz.real()-n/r*uz.imag())
               )
              +mu.imag()*(
               (-duthdr.imag()+uth.imag()/r-n*ur.real()/r)*
               (dpsidx(l,0)-psi(l)/r)  
               +2.0*n*psi(l)/pow(r,2)*(-n*uth.imag()+ur.real())+
          ...
          if(flag)
           {
            ...
            for(unsigned l2=0;l2<n_node;l2++)
             {
              // define complex terms used to obtain entries of current row in the Jacobian:
              ...
              if(a==0)
               {
                G_r = (mu*(2.0*dpsidx(l2,0)*dpsidx(l,0)+
                       ...
                       (dpsidx(l,0)+psi(l)/r)-omega_sq*psi(l2)*psi(l))*r*W;

                G_z=(mu*dpsidx(l2,0)*dpsidx(l,1)+
                     lambda*dpsidx(l2,1)*(dpsidx(l,0)+psi(l)/r))*r*W;
 
                G_theta=(-I*(mu*(n/r*dpsidx(l2,0)*psi(l)-3.0*n/rsq*psi(l2)*psi(l))-
                             lambda*n/r*psi(l2)*(dpsidx(l,0)+psi(l)/r)))*r*W;
               }
              // terms for rows of Jacobian matrix corresponding to z-equation
              else if(a==1)
               {
                ...
                G_z=(mu*(dpsidx(l2,0)*dpsidx(l,0)+nsq/rsq*psi(l2)*psi(l)
                         +2.0*dpsidx(l2,1)*dpsidx(l,1))+
                     lambda*dpsidx(l2,1)*dpsidx(l,1)-omega_sq*psi(l2)*psi(l))*r*W;
 
                G_theta=(-I*(mu*n/r*dpsidx(l2,1)*psi(l)-
                             lambda*n/r*dpsidx(l,1)*psi(l2)))*r*W;
               }
              // terms for rows of Jacobian matrix corresponding to theta-equation
              else if(a==2)
               {
                G_r=(-I*(mu*(-n/r*psi(l2)*dpsidx(l,0)+3.0*n/rsq*psi(l2)*psi(l))+
                         lambda*n/r*(dpsidx(l2,0)+psi(l2)/r)*psi(l)))*r*W;

                G_z=(-I*(-mu*n/r*psi(l2)*dpsidx(l,1)+
                         lambda*n/r*dpsidx(l2,1)*psi(l)))*r*W;

                G_theta=(mu*((dpsidx(l2,0)-psi(l2)/r)*(dpsidx(l,0)-psi(l)/r)+
                             2.0*nsq/rsq*psi(l2)*psi(l)+dpsidx(l2,1)*dpsidx(l,1))+
                         lambda*nsq/rsq*psi(l2)*psi(l)-omega_sq*psi(l2)*psi(l))*r*W;
               }
              ...
              for(unsigned c=0;c<3;c++)
               {
                ...
                local_unknown_real=this->nodal_local_eqn(l2,u_nodal_index[c].real());
                local_unknown_imag=this->nodal_local_eqn(l2,u_nodal_index[c].imag());
                ...
                if(local_unknown_real >= 0)
                 {
                  if(c==0)
                   {
                    jacobian(local_eqn_real,local_unknown_real) += G_r.real();
                   }
                  else if(c==1)
                   {
                    jacobian(local_eqn_real,local_unknown_real) += G_z.real();
                   }
                  else if(c==2)
                   {
                    jacobian(local_eqn_real,local_unknown_real) += G_theta.real();
                   }
                 }
                ...
                if(local_unknown_imag >= 0)
                 {
                  if(c==0)
                   {
                    jacobian(local_eqn_real,local_unknown_imag) += -G_r.imag();
                   }
                  else if(c==1)
                   {
                    jacobian(local_eqn_real,local_unknown_imag) += -G_z.imag();
                   }
                  else if(c==2)
                   {
                    jacobian(local_eqn_real,local_unknown_imag) += -G_theta.imag();
                   }
                 }
               }
             }
        ...
        if(local_eqn_imag >= 0)
         {
          ...
          residuals[local_eqn_imag] += 
           (-omega_sq.real()*interpolated_u[a].imag()
            -omega_sq.imag()*interpolated_u[a].real()
            -b[a].imag())*psi(l)*r*W;
          ...
          if(a==0)
           {
            residuals[local_eqn_imag] += 
             (
              ...
              +mu.imag()*(
               2.0*durdr.real()*dpsidx(l,0)+
               n*psi(l)/r*(n*ur.real()/r+duthdr.imag()-uth.imag()/r)
               +dpsidx(l,1)*(durdz.real()+duzdr.real())+
               2.0*psi(l)/pow(r,2)*(ur.real()-n*uth.imag())
               )
              +lambda.real()*(
                ...
                if(local_unknown_real >= 0)
                 {
                  if(c==0)
                   {
                    jacobian(local_eqn_imag,local_unknown_real) += G_r.imag();
                   }
                  else if(c==1)
                   {
                    jacobian(local_eqn_imag,local_unknown_real) += G_z.imag();
                   }
                  else if(c==2)
                   {
                    jacobian(local_eqn_imag,local_unknown_real) += G_theta.imag();
                   }
                 }
                ...
                if(local_unknown_imag >= 0)
                 {
                  if(c==0)
                   {
                    jacobian(local_eqn_imag,local_unknown_imag) += G_r.real();
                   }
                  else if(c==1)
                   {
                    jacobian(local_eqn_imag,local_unknown_imag) += G_z.real();
                   }
                  else if(c==2)
                   {
  ...
  for (unsigned iplot=0;iplot<num_plot_points;iplot++)
   {
    ...
    this->interpolated_x(s,x);
    this->
     interpolated_u_time_harmonic_fourier_decomposed_linear_elasticity(s,u);
   ...
   for(unsigned i=0;i<3;i++) 
    {
     fprintf(file_pt,"%g ",
             this->
             interpolated_u_time_harmonic_fourier_decomposed_linear_elasticity(s,i).real());
    }
   for(unsigned i=0;i<3;i++) 
    {
     fprintf(file_pt,"%g ",
             this->
             interpolated_u_time_harmonic_fourier_decomposed_linear_elasticity(s,i).imag());
    }
  }
 fprintf(file_pt,"\n");
 ...
 for(unsigned ipt=0;ipt<n_intpt;ipt++)
  {
   ...
   for(unsigned i=0;i<3;i++)
    {
     norm+=(exact_soln[i]*exact_soln[i]+exact_soln[i+3]*exact_soln[i+3])*W;
     error+=(
      (exact_soln[i]-
       this->
       interpolated_u_time_harmonic_fourier_decomposed_linear_elasticity(s,i).real())*
