...
void TimeHarmonicFourierDecomposedLinearElasticityTractionElement<ELEMENT>::
 traction(const Vector<double>& s, Vector<std::complex<double> >& traction)
 {
  ...
  for(unsigned ipt=0;ipt<n_intpt;ipt++)
   {
    ...
    switch(n_dim)
     ...
     default:
      throw 
       OomphLibError(
        "Wrong dimension in TimeHarmonicFourierDecomposedLinearElasticityTractionElement",
        "TimeHarmonicFourierDecomposedLinearElasticityTractionElement::fill_in_contribution_to_residuals()",
        OOMPH_EXCEPTION_LOCATION);
     }
      ...
      for(unsigned i=0;i<n_dim+1;i++)
       {
        ...
        if(local_eqn >= 0)
         {
          ...
          residuals[local_eqn] -= traction[i].real()*psi(l)*interpolated_x[0]*W;
         }
        ...
        if(local_eqn >= 0)
         {
          //Add the loading terms to the residuals
          residuals[local_eqn] -= traction[i].imag()*psi(l)*interpolated_x[0]*W;
         }
