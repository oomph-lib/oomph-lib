...
//     index[i] == -1: signifies that the corresponding eigenvector is stored in two vectors: the real part in the i-1 column of Evecs and the negative imaginary part in the i column of Evecs
        ...
        if (index_vector[i]==0)
         ...
         {
          oomph_info << "eigenvector " << i << " is complex and stored in cols i and i+1.\n";
          for (unsigned j = 0; j < nrow_local; j++)
           {
            eigenvector[i][j] = std::complex<double>((*evecs)(i, j),(*evecs)(i+1, j));
           }
         }
        else if (index_vector[i]==-1)
         {
          oomph_info << "eigenvector " << i << " is complex and stored in cols i-1 and i.\n";
          for (unsigned j = 0; j < nrow_local; j++)
           {
            eigenvector[i][j] = std::complex<double>((*evecs)(i-1, j),-(*evecs)(i, j));
           }
         }
        else
         {
          oomph_info << "Never get here: index_vector[ " << i << " ] = " << index_vector[i] << std::endl;
          abort();
         }
      }
