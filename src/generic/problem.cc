...
namespace oomph
{
  ...
  void Problem::newton_solve()
  {
    ...
    do
    {
      ...
      if (n_dofs==0)
      {
        oomph_info << "\n\n\nThis is a bit bizarre: The problem has "
                   << "no dofs.\nI'll just return from the Newton solver "
                   << "without doing anything." << std::endl;
                            ...
                            DoubleVector &z)
  {
    ...
    do
    {
      ...
      *parameter_pt -= dparam;
      ...
      if (!Shut_up_in_newton_solve)
      {
        oomph_info << "\nContinuation Step " << count << ":  Maximum residuals "
                   << maxres << "\n" << std::endl;
      }
      ...
      const unsigned &max_adapt)
  {
    ...
    if (!Shut_up_in_newton_solve)
    {
      oomph_info << "Total time for pseudo-arc-length continuation (ndof="
                 << n_dofs << ") [sec]: " << TimingHelpers::timer()-t_start
                 << "\n" << std::endl;
    }
                                 ...
                                 const bool &shift_values)
  {
    ...
    do
    {
      ...
      if (adaptive_flag && !reject_timestep)
      {
        ...
        if (error>epsilon)
        {
          oomph_info << "Estimated timestepping error "
          ...
          oomph_info << "Note: This behaviour can be adjusted by changing the "
                     << "protected boolean\n\n"
  ...
  void Problem::read(std::ifstream& restart_file, bool& unsteady_restart)
  {
...
#else
    // When Problems are dumped with MPI enabled, the dumped data file (at this
    // point) becomes different to that which would be found with MPI disabled.
    // In particular, information about base elements are stored in the file
    // (which need to be ignored now if MPI is disabled).

    // Get the current position in the file
    int len=restart_file.tellg();

    // Read line...
    getline(restart_file,input_string);

    // ...and now jump back to the position we were in before calling getline(...)
    restart_file.seekg(len,std::ios_base::beg);

    // What is the string we need to search for in this line?
    std::string search_string="base elements";

    // Search for the string above in the extracted line
    std::size_t found=input_string.find(search_string);

    // If we found it managed to find it we need to ignore the appropriate
    // number of lines in the file and carry as normal (with a warning
    // issued to the user just in case something goes wrong...)
    if (found!=std::string::npos)
    {
      ...
      warning_message_stream << "It looks like you dumped the problem with MPI "
                             << "enabled but\nnow you're loading it in without "
                             << "MPI enabled. I'm going to\ngo ahead and just "
                             << "ignore the extra stuff (which shouldn't be\n"
                             << "necessary now). Everything else *should* be "
                             << "fine but if\nsomething goes wrong you should "
                             << "try dumping the problem\n(again) with MPI "
                             << "disabled!" << std::endl;
      ...
      // Loop over the base elements
      for (unsigned e=0; e<n_base_element_read_in; e++)
      {
        // Read in the e-th base element
        getline(restart_file,input_string);
      }
      ...
      if (restart_file_is_open)
      {
        // Make sure that tmp has the right value
        if (tmp!=8888)
        {
          // Create an output stream
          ...
          error_message_stream << "Error in reading restart data: Target proc for "
                               << "base\nelements should be followed by 8888."
                               << std::endl;
