...
namespace oomph
{
  ...
  ///    where \f$ \xi^m \f$ and \f$ \xi^s \f$ are the nodal values at the
    ...
    const int &value_id, const int &my_edge, std::ofstream &output_hangfile)
  {
    ...
    if (h_type_slave || p_type_slave)
    {
      ...
      Orthpoly::gll_nodes(
        slave_element_nnode_1d, slave_nodal_position, slave_weight);
      ...
      // slave elements. Where this is not the case, the vertex matching
      // condition must be enforced by constraining the value of the unknown at
      // the node on the slave side to be the same as the value at that location
      // in the master.
      ...
      if (n_slave_nodes - n_mortar_vertices > 0)
      {
        ...
        for (unsigned i = 0; i < n_slave_nodes - n_mortar_vertices; i++)
        {
          ...
          psi[i] =
            pow(-1.0, int((slave_element_nnode_1d - 1) - i - 1)) *
        ...
        for (unsigned q = 0; q < (*quadrature_weight).size(); q++)
        {
          ...
          {
            // Check if denominator is zero
            if (std::fabs(slave_nodal_position[non_vertex_pos[k]] -
                          s_on_mortar[0]) >= 1.0e-08)
            {
              // We're ok
              psi[k] =
                pow(-1.0, int((slave_element_nnode_1d - 1) - k - 1)) *
                Orthpoly::dlegendre(slave_element_nnode_1d - 1,
                                    s_on_mortar[0]) /
        ...
        for (unsigned i = 0; i < n_slave_nodes - n_mortar_vertices; i++)
        {
          ...
          for (unsigned m = 0; m < hang_info_pt[i]->nmaster(); m++)
          {
            ...
            if (hang_info_pt[i]->master_node_pt(m) ==
                slave_node_pt[non_vertex_pos[i]])
            {
          ...
          {
            slave_node_pt[non_vertex_pos[i]]->set_hanging_pt(hang_info_pt[i],
                                                             -1);
          }
        }
      ...
      for (unsigned i = 0; i < n_slave_nodes; i++)
      {
        ...
        if (slave_node_pt[i]->is_hanging())
        {
          ...
          for (unsigned m = 0; m < slave_node_pt[i]->hanging_pt()->nmaster();
               m++)
          {
        ...
        else
        {
          ...
          if (!is_master)
          {
            ...
            error_string << "This node in the slave element is neither"
                         << std::endl
    ...
    const Vector<double> &s_fraction)
  {
    ...
    for (unsigned j = 0; j < n_face; j++)
    {
      ...
      if (neigh_pt != 0)
      {
        ...
        {
          throw OomphLibError("Not a quad element!",
                              "PRefineableQElement<3,INITIAL_NNODE_1D>::node_"
                              "created_by_neighbour()",
                              OOMPH_EXCEPTION_LOCATION);
        }
        for (unsigned vnode = 0; vnode < neigh_obj_pt->nvertex_node(); vnode++)
    ...
    const int &value_id, const int &my_face, std::ofstream &output_hangfile)
  {
    ...
    if (h_type_slave || p_type_slave || true)
    {
      ...
      for (unsigned i = 0; i < 4; i++)
      {
        ...
        if (!my_edge_is_master)
        {
          ...
          // and slave elements. Where this is not the case, the vertex matching
          // condition must be enforced by constraining the value of the unknown
          // at the node on the slave side to be the same as the value at that
          ...
          for (unsigned v = 0; v < n_mortar_vertices; v++)
          {
            ...
            if (!node_is_shared)
            {
              ...
              // Make node hang
              slave_node_pt[vertex_pos[v]]->set_hanging_pt(explicit_hang_pt,
                                                           -1);
          ...
          // Check that there are actually slave nodes for which we still need
          // to construct a hanging scheme. If not then there is nothing more to
          // do.
          if (n_slave_nodes - n_mortar_vertices > 0)
          {
            // Assemble mass matrix for mortar
            ...
            // n_slave_nodes, N_m = n_master_nodes. (Use pointers for the
            ...
            for (unsigned q = 0; q < (*quadrature_weight).size(); q++)
            {
              ...
              for (unsigned k = 0; k < n_slave_nodes - n_mortar_vertices; k++)
              {
                ...
                // Check if numerator is zero
                           ...
                           slave_element_nnode_1d - 1, s_on_mortar[0])) <
                         1.0e-8)
                {
                  // We can use l'Hopital's rule
                  psi[k] =
            ...
            for (unsigned i = 0; i < n_slave_nodes - n_mortar_vertices; i++)
            {
              ...
              if (!node_is_really_shared)
              {
                slave_node_pt[non_vertex_pos[i]]->set_hanging_pt(
                  hang_info_pt[i], -1);
    ...
    if (h_type_slave || p_type_slave)
    {
      ...
      // std::cout << "Slave nodes on face: " << OcTree::Direct_string[my_face]
      // << std::endl; for(unsigned i=0; i<slave_node_pt.size(); i++)
      // {
      //  std::cout << i << ": "
      //            << slave_node_pt[i]->x(0) << "  "
      //            << slave_node_pt[i]->x(1) << "  "
      ...
      Orthpoly::gll_nodes(
        slave_element_nnode_1d, slave_nodal_position_1d, slave_weight_1d);
      ...
      // Storage for 2D
      Vector<Vector<double>> slave_nodal_position(slave_element_nnode_1d *
                                                  slave_element_nnode_1d);
      ...
      Vector<unsigned> non_vertex_pos((slave_element_nnode_1d - 2) *
                                      (slave_element_nnode_1d - 2));
      unsigned nvindex = 0;
      ...
      if (n_slave_nodes - n_mortar_vertices > 0)
      {
        ...
        for (unsigned i = 0; i < n_slave_nodes - n_mortar_vertices; i++)
        {
          ...
          {
            unsigned index1d =
              (dir == 0) ? i : i % (slave_element_nnode_1d - 2);
            // Use L'Hosptal's rule:
            psi[i] *=
              pow(-1.0, int((slave_element_nnode_1d - 1) - index1d - 1)) *
              -Orthpoly::ddlegendre(
                slave_element_nnode_1d - 1,
        ...
        for (unsigned q = 0; q < (*quadrature_weight).size(); q++)
        {
          ...
          for (unsigned k = 0; k < n_slave_nodes - n_mortar_vertices; k++)
          {
            ...
            {
              unsigned index1d =
                (dir == 0) ? k : k % (slave_element_nnode_1d - 2);
              // Check if denominator is zero
              if (std::fabs(slave_nodal_position[non_vertex_pos[k]][dir] -
                            s_on_mortar[dir]) >= 1.0e-08)
              {
                ...
                psi[k] *=
                  ...
                  Orthpoly::dlegendre(slave_element_nnode_1d - 1,
                                      s_on_mortar[dir]) /
                  (slave_nodal_position[non_vertex_pos[k]][dir] -
                   s_on_mortar[dir]);
              }
              // Check if numerator is zero
              else if (std::fabs(Orthpoly::dlegendre(slave_element_nnode_1d - 1,
        ...
        for (unsigned v = 0; v < n_mortar_vertices; v++)
        {
          ...
          for (unsigned i = 0; i < 3; i++)
          {
            ...
            {
              s_fraction[i] =
                0.5 * (slave_nodal_position[vertex_pos[v]][0] + 1.0);
            }
            else if (i == active_coord_index[1])
            {
              s_fraction[i] =
                0.5 * (slave_nodal_position[vertex_pos[v]][1] + 1.0);
            }
            else
            {
              s_fraction[i] = slave_node_s_fraction[vertex_pos[0]][i];
      ...
      for (unsigned i = 0; i < n_slave_nodes; i++)
      {
        ...
        if (slave_node_pt[i]->is_hanging())
        {
          ...
          {
            weight_sum += slave_node_pt[i]->hanging_pt()->master_weight(m);
            if (std::fabs(slave_node_pt[i]->hanging_pt()->master_weight(m)) <
                1.0e-14)
            {
              zero_weight = true;
              oomph_info << "In the hanging scheme for slave node " << i
                         << ", master node " << m << " has weight "
                         << slave_node_pt[i]->hanging_pt()->master_weight(m)
                         << " < 1.0e-14" << std::endl;
            }
          }
            ...
            if (slave_node_pt[i]->hanging_pt()->master_node_pt(m)->is_hanging())
            {
                   ...
                   mm++)
              {
                    ...
                    new_nod_pt->hanging_pt()->master_node_pt(mm))
                {
                  std::cout << "++++++++++++++++++++++++++++++++++++++++"
                            << std::endl;
                  std::cout << "          Slave node: " << slave_node_pt[i]
                            << " = " << slave_node_pt[i]->x(0) << " "
                            << slave_node_pt[i]->x(1) << " "
                  ...
                  std::cout
                    ...
                    << " "
                    << slave_node_pt[i]->hanging_pt()->master_node_pt(m)->x(2)
                    << " " << std::endl;
                  ...
                  // Print out hanging scheme
                  std::cout << "Hanging node: " << slave_node_pt[i]->x(0)
                            << "  " << slave_node_pt[i]->x(1) << "  "
                            << slave_node_pt[i]->x(2) << "  " << std::endl;
                  for (unsigned m_tmp = 0;
                       m_tmp < slave_node_pt[i]->hanging_pt()->nmaster();
                       m_tmp++)
                  {
                    std::cout
                      << "   m = " << m_tmp << ": "
                      ...
                      << "  "
                      << "w = "
                      << slave_node_pt[i]->hanging_pt()->master_weight(m_tmp)
                      << "  " << std::endl;
                  }
