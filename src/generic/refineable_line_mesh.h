...
namespace oomph
{
 ...
 /// specified in the PRefineableMesh class for meshes that contain the
 /// p-refineable variant of QElement s [The class ELEMENT provided as the
 /// template parameter must be of type PRefineableQElement<1,INITIAL_NNODE_1D>].
 /// 
 /// Mesh adaptation/refinement is implemented by BinaryTree procedures
 /// and any concrete implementation of this class needs to provide a
 /// BinaryTreeForest representation of the initial (coarse) mesh.
 //===========================================================================
 template <class ELEMENT>
  class PRefineableLineMesh : public virtual TreeBasedPRefineableMesh<ELEMENT>, 
  public virtual LineMeshBase
  {
   ...
   /// Broken copy constructor
   PRefineableLineMesh(const PRefineableLineMesh& dummy) 
    { 
     BrokenCopy::broken_copy("PRefineableLineMesh");
    } 
   
   /// Broken assignment operator
   void operator=(const PRefineableLineMesh&) 
    {
     BrokenCopy::broken_assign("PRefineableLineMesh");
    }
   
   /// Destructor:
   virtual ~PRefineableLineMesh() {}
