 ...
 Vector<unsigned>& element_domain_on_this_proc)
{
 ...
 std::map<GeneralisedElement*,std::set<unsigned> > global_eqns_on_each_element;
     ...
     for (unsigned i=0; i<n_dof; i++)
      {
       ...
       elements_connected_with_global_eqn_on_this_proc[eqn_no].insert(el_pt);
       global_eqns_on_this_proc.insert(eqn_no);
       global_eqns_on_each_element[el_pt].insert(eqn_no);
      }
     count_non_halo++;
    }
  }

 oomph_info << "compare count_non_halo " << count_non_halo << " "
            << global_eqns_on_each_element.size() << std::endl;
   ...
   if (!el_pt->is_halo())
    {
     ...
     for (std::set<unsigned>::iterator it=
           global_eqns_on_each_element[el_pt].begin();
          it!=global_eqns_on_each_element[el_pt].end(); it++)
      {
       unsigned eqn_no=*it;
 ...
 MPI_Gatherv(&number_of_eqn_numbers[0], count_non_halo, MPI_INT,
             ...
             &start_index[0], MPI_INT, root_processor, comm_pt->mpi_comm());
 ...
 MPI_Gatherv(&eqn_numbers_with_elements_on_this_proc[0], 
             ...
             &start_eqns_index[0], MPI_INT, root_processor, comm_pt->mpi_comm());
 ...
 if (my_rank==root_processor)
  {
   ...
   std::map<unsigned,std::set<unsigned> > 
    elements_connected_with_global_eqn_on_root_processor;
     ...
     for (unsigned n=0; n<n_eqn_no; n++)
      {
       ...
       count_all++;
       elements_connected_with_global_eqn_on_root_processor[eqn_no].insert(e);
   ...
   for (std::set<unsigned>::iterator it=all_global_eqns_root_processor.begin();
        it!=all_global_eqns_root_processor.end();it++)
    {
     ...
     std::set<unsigned> elements=
      elements_connected_with_global_eqn_on_root_processor[*it];
