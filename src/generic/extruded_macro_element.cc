...
namespace oomph
{
      ...
      Vector<double>& r)
  {
    ...
    Extruded_domain_pt->macro_element_boundary(t,Macro_element_number,
    ...
    for (unsigned i=0; i<n_dim; i++)
    {
      ...
      point_up=corner_LU[i]+(corner_RU[i]-corner_LU[i])*0.5*(s[0]+1.0);

      // Calculate the point on the lower edge
      point_down=corner_LD[i]+(corner_RD[i]-corner_LD[i])*0.5*(s[0]+1.0);
      ...
      point_up=corner_LUB[i]+(corner_RUB[i]-corner_LUB[i])*0.5*(s[0]+1.0);

      // Calculate the point on the lower edge of the back "rectangular slice"
      point_down=corner_LDB[i]+(corner_RDB[i]-corner_LDB[i])*0.5*(s[0]+1.0);
      ...
      // Calculate the point on the upper edge of the back "rectangular slice"
      point_up=corner_LUF[i]+(corner_RUF[i]-corner_LUF[i])*0.5*(s[0]+1.0);

      // Calculate the point on the lower edge of the back "rectangular slice"
      point_down=corner_LDF[i]+(corner_RDF[i]-corner_LDF[i])*0.5*(s[0]+1.0);
      ...
      // Final map
      //----------
      r[i]=slice_mid+0.5*(1.0+s[2])*diff_front+0.5*(1.0-s[2])*diff_back;
    }
  } // End of macro_map
  ...
  //=================================================================
  void QExtrudedMacroElement<3>::
  output_macro_element_boundaries(std::ostream& outfile,const unsigned& nplot)
  {
    ...
    for (unsigned idirect=L; idirect<=F; idirect++)
    {
      ...
      for (unsigned i=0; i<nplot; i++)
      {
        ...
        {
          // Calculate the first local coordinate associated with this plot point
          s[0]=-1.0+2.0*double(j)/double(nplot-1);

          // To make the extrusion machinery consistent with the Domain machinery
          // a time level argument has to be provided. For our purposes we set
          // this to zero to ensure that the appropriate output is provided.
          unsigned t=0;
