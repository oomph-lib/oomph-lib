...
//////////////////////////////////////////////////////////////////////////
 ...
 /// globally.
 //=========================================================================
 int NewMumpsSolver::Default_workspace_scaling_factor=5;
...
//=========================================================================
bool NewMumpsSolver::Suppress_incorrect_rhs_distribution_warning_in_resolve
=false;

//=============================================================================
/// Constructor: Call setup
//=============================================================================
 NewMumpsSolver::NewMumpsSolver()
 {
  Doc_stats=false;
...
//=============================================================================
 void NewMumpsSolver::initialise_mumps()
 {
...
/// Shutdown mumps
//=============================================================================
 void NewMumpsSolver::shutdown_mumps()
 {
...
void NewMumpsSolver::factorise(DoubleMatrixBase* const &matrix_pt)
{
   ...
   if (this->distribution_pt()->communicator_pt()->mpi_comm()!=
       MPI_COMM_WORLD)
    {
     ...
     error_message_stream 
      ...
      << "         paranoia or setting \n\n"
      << "         NewMumpsSolver::suppress_warning_about_MPI_COMM_WORLD()\n\n"
      << "         to true.\n";
 ...
 if (cr_matrix_pt != 0)
  {
   ...
   if (!cr_matrix_pt->built())
    {
     throw OomphLibError
      ("To apply NewMumpsSolver to a CRDoubleMatrix - it must be built",
       "NewMumpsSolver::factorise()",OOMPH_EXCEPTION_LOCATION);
    }
#endif
   ...
   // else the CRDoubleMatrix is not distributed
    ...
    {     
     std::ostringstream error_message_stream;
     error_message_stream << "NewMumpsSolver only works for a "
 ...
 // Otherwise throw an error
  ...
  {
   std::ostringstream error_message_stream;
   error_message_stream << "NewMumpsSolver implemented only for "
                        << "distributed CRDoubleMatrix. \n";
   throw OomphLibError(error_message_stream.str(),
                       "NewMumpsSolver::factorise()",
                       OOMPH_EXCEPTION_LOCATION);
    ...
    if(!Suppress_incorrect_rhs_distribution_warning_in_resolve)
     {
      ...
      warning_stream
       ...
       << "or ii) Set the flag \n"
       << " NewMumpsSolver::Suppress_incorrect_rhs_distribution_warning_in_resolve\n"
       << "       to be true\n\n";
...
//=========================================================================
/// Clean up the memory allocated for the NewMumpsSolver solver -- 
/// don't think this is needed any more... hierher check
//=========================================================================
void NewMumpsSolver::clean_up_memory()
{
 shutdown_mumps();
}
...
/// matrix_pt->clean_up_memory() will be used to wipe the matrix data.
//=========================================================================
void NewMumpsSolver::solve(DoubleMatrixBase* const &matrix_pt,
 ...
 if ((Doc_time) && (this->distribution_pt()->communicator_pt()->my_rank()==0))
  {
   ...
   oomph_info << std::endl 
              << "Time for NewMumpsSolver solve [sec]       : "
              << t_end-t_start << std::endl << std::endl;
  }
...
//==================================================================
void NewMumpsSolver::solve(Problem* const &problem_pt, DoubleVector &result)
{
 ...
 if ((Doc_time) && (my_rank==0))
  {
   double t_end = TimingHelpers::timer();
   oomph_info << std::endl << "Total time for NewMumpsSolver " << "(np=" 
...
//===============================================================
 void NewMumpsSolver::resolve(const DoubleVector &rhs, DoubleVector &result)
 {
 ...
 if ((Doc_time) && (this->distribution_pt()->communicator_pt()->my_rank()==0))
  {
   oomph_info << "Time for NewMumpsSolver solve [sec]: "
