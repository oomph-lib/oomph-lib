...
//=================================================================
				     ...
				     bool& in_neighbouring_tree) const
 {
  ...
  // Find neighbour
  OcTree* return_pt=gteq_face_neighbour(direction,s_difflo,s_diffhi,diff_level,
					in_neighbouring_tree,max_level,
					orig_root_pt);
  ...
  if (Father_pt!=0)
  {
   ...
   if (Is_adjacent(direction,Son_type))
   {
    next_el_pt=dynamic_cast<OcTree*>(Father_pt)->
     gteq_face_neighbour(direction,s_difflo,s_diffhi,diff_level,
			 in_neighbouring_tree,max_level,orig_root_pt);
   }
				     ...
				     OcTreeRoot* orig_root_pt) const
 {
  ...
  else
  {
   ...
   if (n_neigh>0)
   {
    ...
    return_el_pt=dynamic_cast<OcTree*>(edge_neighbour_pt[i_root_edge_neighbour]);
   }
   else
   {
    return_el_pt=0;
  ...
  for (unsigned long i=0;i<num_nodes;i++)
  {
   ...
   if(el_pt->object_pt()->nodes_built())
   {
	...
	mismatch_file.open("mismatch.dat");
       ...
       mismatch_file << "ZONE" << std::endl;
       mismatch_file << x_large[0] << " "
		     << x_large[1] << " "
		     << x_large[2] << "  2 " << std::fabs(error) << " \n";
       mismatch_file << x_small[0] << " "
		     << x_small[1] << " "
  ...
  for (unsigned long i=0;i<num_nodes;i++)
  {
   ...
   if (el_pt->object_pt()->nodes_built())
   {
	...
	output_corners(neighbours_file,Colour[direction]);
      ...
      Vector<int> faces_attached_to_edge=faces_of_common_edge(direction);
	...
	mismatch_file.open("mismatch.dat");
     ...
     else
     {
      if (neighbours_file.is_open())
      {
       ...
       neighbours_file << "#---------------------------------" << std::endl;
       neighbours_file << "# No neighbour in direction: "
		       << Direct_string[direction]
		       << std::endl;
       neighbours_file << "#---------------------------------" << std::endl;
  ...
  for (unsigned i=0;i<numtrees;i++)
  {
   ...
   if (neigh_pt!=0)
   {
    ...
    if(direction!=Tree::OMEGA)
    {
     ...
     octree_pt(i)->set_up_equivalent(neigh_pt,OcTree::Reflect_face[direction]);
     ...
     octree_pt(i)->set_right_equivalent(
      neigh_pt,OcTree::get_the_other_face(nod1,nod2,nnode1d,direction));
    }
    else     
