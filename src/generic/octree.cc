...
namespace oomph
{
                                         ...
                                         DenseMatrix<int>& mat)
  {
    ...
    switch (axis)
    {
      ...
      default:
        throw OomphLibError(
          "Bad Axis", OOMPH_CURRENT_FUNCTION, OOMPH_EXCEPTION_LOCATION);
    }
    for (i = 0; i < 3; i++)
    {
      for (j = 0; j < 3; j++)
                                                ...
                                                OcTree* edge_neigh_pt) const
  {
    ...
    switch (edge)
    {
      case LB:
        ...
        face = L;
        face_neigh_pt = gteq_face_neighbour(
          face, translate_s, s_sw, s_ne, reflected_face, diff_level);
      ...
      if (el_pt->object_pt()->nodes_built())
      {
        ...
        for (int direction = L; direction <= F; direction++)
        {
          ...
          if ((neighb_pt != 0) && (neighb_pt->object_pt()->nodes_built()))
          {
            ...
            double error = sqrt(pow(x_small[0] - x_large[0], 2) +
          ...
          else
          {
            if (neighbours_file.is_open())
            {
              neighbours_file << "#---------------------------------"
                              << std::endl;
              neighbours_file
                << "#no Neighbour in direct: " << Direct_string[direction]
                << std::endl;
              neighbours_file << "#---------------------------------"
                              << std::endl;
