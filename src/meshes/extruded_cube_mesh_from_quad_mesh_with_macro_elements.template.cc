...
namespace oomph
{
  ...
  Vector<std::pair<unsigned,int> > ExtrudedCubeMeshFromQuadMesh<ELEMENT>::
  ...
  //========================================================================
  template<class ELEMENT>
  void ExtrudedCubeMeshFromQuadMesh<ELEMENT>::
  build_mesh(QuadMeshBase* quad_mesh_pt,TimeStepper* time_stepper_pt)
  {
    ...
    Vector<Vector<std::pair<unsigned,int> > >
    boundary_information(n_element_in_quad_mesh);
        ...
        (n_node_1d*n_node_1d!=quad_mesh_pt->finite_element_pt(0)->nnode()))
    {
      ...
      // Create the error message
      error_message << "Extrude2DQuadMeshTo3DCubeMesh needs the number of "
    ...
    // Get the total number of nodes in the input mesh
    unsigned long n_node_in_cube_mesh=n_node_in_quad_mesh*n_node_in_z_direction;
    ...
    for (unsigned i=0; i<Nz; i++)
    {
      ...
      std::map<std::pair<Node*,double>,Node*> quad_corner_node_to_cube_node_map;
      ...
      for (unsigned e=0; e<n_element_in_quad_mesh; e++)
      {
        ...
        {
          // Calculate the z-value at the n_2-th (z-)node in the i-th element slice
          ...
          if ((i>0)&&(n_2==0))
          {
            ...
            FiniteElement* copy_cube_el_pt=finite_element_pt(copy_element_index);

            // Storage for the local node number in the element that we're copying
            ...
            for (unsigned n_1=0; n_1<n_node_1d; n_1++)
            {
              ...
              for (unsigned n_0=0; n_0<n_node_1d; n_0++)
              {
                ...
                // copying it from)
                copy_cube_local_node_number=cube_local_node_number+
          ...
          else
          {
            ...
            // What is the last edge? Convert this to an int so the compiler doesn't
            // complain about comparisons between unsigned and signed integers...
            ...
            for (int i_edge=QuadTreeNames::N; i_edge<last_edge; i_edge++)
            {
              ...
              default:
                ...
                // Create the error message
                error_message_stream << "Input is " << i_edge
                                     << " but it can only\n"
                                     << "be either N/E/S/W, i.e. "
                                     << QuadTreeNames::N << ","
                                     << QuadTreeNames::E << ","
                                     << QuadTreeNames::S << " or "
              ...
              // Create the edge associated with the i_edge-th edge of this element
              ...
              if (it!=edge_to_nodes_map.end())
              {
                ...
                for (unsigned i_node=0; i_node<n_node_1d; i_node++)
                {
                  ...
                  case QuadTreeNames::N:
                    ...
                    quad_local_node_number=((n_node_1d*n_node_1d)-1)-i_node;

                    // Break
                    break;
                  case QuadTreeNames::E:
                    // Calculate this Node's quad local node number
                    quad_local_node_number=(n_node_1d-1)+(i_node*n_node_1d);
                  ...
                  cube_local_node_number=quad_local_node_number+
                                         (n_node_1d*n_node_1d*n_2);
              ...
              for (unsigned n_0=0; n_0<n_node_1d; n_0++)
              {
                ...
                if (((i==0)&&(n_2==0))||((i==Nz-1)&&(n_2==n_node_1d-1)))
                {
                  ...
                  else if ((i==Nz-1)&&(n_2==n_node_1d-1))
                  {
                    ...
                    // Add the node to the appropriate boundary
                    add_boundary_node(n_boundary_in_quad_mesh+1,cube_node_pt);
                  } // if ((i==0)&&(n_2==0))

                  // Get a pointer to the matching quad mesh node
                  Node* quad_node_pt=quad_el_pt->node_pt(quad_local_node_number);
                ...
                if (el_boundary_information.size()>0)
                {
                  ...
                  for (unsigned i_bound=0; i_bound<n_boundaries; i_bound++)
                  {
                    ...
                    case -1:
                      ...
                      // boundary the node can only lie on the boundary if n_0=0.
                      if (n_0==0)
                      {
                        // Now add the node to the appropriate boundary
                        add_boundary_node(
                          boundary_and_face_index.first,cube_node_pt);
                      }

                      // Break
                      break;
                    case 1:
                      // If the s[0]=1 boundary of the element lies on the boundary
                      // the node can only lie on the boundary if n_0=n_node_1d-1
                      if (n_0==n_node_1d-1)
                      {
                        // Now add the node to the appropriate boundary
                        add_boundary_node(
                      ...
                      break;
                    case -2:
                      // If the s[1]=-1 boundary of the element lies on the boundary
                      // the node can only lie on the boundary if n_1=0.
                      if (n_1==0)
                      {
                        // Now add the node to the appropriate boundary
                        add_boundary_node(
                    ...
                    case 2:
                      // If the s[1]=1 boundary of the element lies on the boundary
                      // the node can only lie on the boundary if n_1=n_node_1d-1.
                      if (n_1==n_node_1d-1)
                      {
                        // Now add the node to the appropriate boundary
                        add_boundary_node(
            ...
            //--------------------------------------------------------------------
            // Now store the corner nodes of the element at the current node slice
            // in the map quad_corner_node_to_cube_node_map.
            //--------------------------------------------------------------------
            // Loop over the corner nodes
            for (unsigned i_corner=0; i_corner<n_corner; i_corner++)
            {
      ...
      if (node_count!=(1+(n_node_1d-1)*(i+1))*n_node_in_quad_mesh)
      {
        ...
        int expected_n_node=(1+(n_node_1d-1)*(i+1))*n_node_in_quad_mesh;

        // Difference in node count
        int node_diff=expected_n_node-node_count;

        // Create the error message
        error_message_stream << "There are meant to be " << expected_n_node
                             << " nodes in the extruded mesh by the\nend of slice "
                             << i << " but you have " << node_count << ". "
    ...
    for (unsigned b=0; b<n_boundary_in_quad_mesh; b++)
    {
      ...
      for (unsigned n=0; n<n_boundary_node; n++)
      {
        ...
        if (all_quad_boundary_nodes_pt.find(
              quad_mesh_pt->boundary_node_pt(b,n))==
            all_quad_boundary_nodes_pt.end())
        {
          // Count this node
    ...
    n_expected_boundary_node+=(((n_node_1d-1)*Nz)-1)*n_quad_mesh_boundary_node;
    ...
    if (MeshExtrusionHelpers::Mesh_extrusion_helper.doc_mesh_setup_time())
    {
      ...
      oomph_info << "Time taken to extrude mesh [sec]: "
                 << TimingHelpers::timer()-t_mesh_construct_start << std::endl;
    }
    ...
    for (unsigned e=0; e<n_element_in_quad_mesh; e++)
    {
      ...
      QElementBase* quad_el_pt=dynamic_cast<QElementBase*>
      ...
      if (quad_el_pt->macro_elem_pt()!=0)
      {
        ...
        for (unsigned i=0; i<Nz; i++)
        {
          ...
          // this quad element?
          unsigned macro_id=(i*n_macro_element+quad_el_pt->
                             macro_elem_pt()->macro_element_number());
