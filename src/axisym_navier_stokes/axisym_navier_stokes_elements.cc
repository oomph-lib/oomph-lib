...
namespace oomph
{
      ...
      const unsigned& which_one)
  {
    ...
    for (unsigned ipt = 0; ipt < n_intpt; ipt++)
    {
      ...
      for (unsigned l = 0; l < n_node; l++)
      {
        ...
        {
          local_eqn = nodal_local_eqn(l, momentum_index_axi_nst(l, i));
                                                       ...
                                                       const unsigned& t)
  {
    ...
    for (unsigned iplot = 0; iplot < num_plot_points; iplot++)
    {
      ...
      for (unsigned i = 0; i < 3; i++)
      {
        ...
        {
          interpolated_u[i] +=
            nodal_value(t, l, this->u_index_axi_nst(l, i)) * psi[l];
        }
      }
    ...
    const Vector<double>& s, DenseMatrix<double>& strainrate) const
  {
    ...
    for (unsigned l = 0; l < n_node; l++)
    {
      ...
      ur += nodal_value(l, this->u_index_axi_nst(l, 0)) * psi[l];
      uz += nodal_value(l, this->u_index_axi_nst(l, 1)) * psi[l];
      uphi += nodal_value(l, this->u_index_axi_nst(l, 2)) * psi[l];

      durdr += nodal_value(l, this->u_index_axi_nst(l, 0)) * dpsidx(l, 0);
      durdz += nodal_value(l, this->u_index_axi_nst(l, 0)) * dpsidx(l, 1);

      duzdr += nodal_value(l, this->u_index_axi_nst(l, 1)) * dpsidx(l, 0);
      duzdz += nodal_value(l, this->u_index_axi_nst(l, 1)) * dpsidx(l, 1);

      duphidr += nodal_value(l, this->u_index_axi_nst(l, 2)) * dpsidx(l, 0);
      duphidz += nodal_value(l, this->u_index_axi_nst(l, 2)) * dpsidx(l, 1);
    }
      ...
      unsigned flag)
  {
    ...
    for (unsigned ipt = 0; ipt < Nintpt; ipt++)
    {
      ...
      for (unsigned l = 0; l < n_node; l++)
      {
        ...
        for (unsigned i = 0; i < 3; i++)
        {
          ...
          const double u_value =
            this->raw_nodal_value(l, this->u_index_axi_nst(l, i));
      ...
      for (unsigned l = 0; l < n_node; l++)
      {
        ...
        local_eqn = nodal_local_eqn(l, momentum_index_axi_nst(l, 0));
        ...
        if (local_eqn >= 0)
        {
          ...
          if (flag)
          {
            ...
            {
              local_unknown = nodal_local_eqn(l2, u_index_axi_nst(l2, 0));
              ...
              local_unknown = nodal_local_eqn(l2, u_index_axi_nst(l2, 1));
              if (local_unknown >= 0)
              {
                jacobian(local_eqn, local_unknown) -=
              ...
              local_unknown = nodal_local_eqn(l2, u_index_axi_nst(l2, 2));
              if (local_unknown >= 0)
              {
                // Convective terms
        ...
        local_eqn = nodal_local_eqn(l, momentum_index_axi_nst(l, 1));
        // If it's not a boundary condition
        if (local_eqn >= 0)
        {
          // Add the user-defined body force terms
        ...
        local_eqn = nodal_local_eqn(l, momentum_index_axi_nst(l, 2));
        if (local_eqn >= 0)
        {
          // Add the user-defined body force terms
          // Remember to multiply by the density ratio!
      ...
      for (unsigned l = 0; l < n_pres; l++)
      {
        local_eqn = continuity_local_eqn(l);
      ...
      if (nod_pt->has_auxiliary_node_update_fct_pt())
      {
        ...
        for (unsigned p = 0; p < 2; p++)
        {
          ...
          for (unsigned i = 0; i < 3; i++)
          {
            // Evaluate
            d_U_dX(p, q, i) =
              (*(nod_pt->value_pt(this->u_index_axi_nst(q, i))) - u_ref[i]) /
              eps_fd;
          }
        ...
        for (unsigned p = 0; p < 2; p++)
        {
          ...
          for (unsigned i = 0; i < 3; i++)
          {
            ...
            for (unsigned k = 0; k < 2; k++)
            {
              ...
              {
                aux += this->raw_nodal_value(j, this->u_index_axi_nst(j, i)) *
      ...
      for (unsigned l = 0; l < n_node; l++)
      {
        ...
        if (local_eqn >= 0)
        {
          ...
          for (unsigned l3 = 0; l3 < n_node; l3++)
          {
            ...
            local_freedom = nodal_local_eqn(l3, u_index_axi_nst(l3, 0));
            if (local_freedom >= 0)
            {
            ...
            local_freedom = nodal_local_eqn(l3, u_index_axi_nst(l3, 1));
            if (local_freedom >= 0)
            {
              double temp = 0.0;
            ...
            local_freedom = nodal_local_eqn(l3, u_index_axi_nst(l3, 2));
            if (local_freedom >= 0)
            {
              double temp = 0.0;
