...
//        = new LinearAlgebraDistribution(comm_pt(),
...
//        fluid_sub_vec_pt[i]->clear();
      ...
      DoubleVectorHelpers::split_without_communication(another_temp_vec,fluid_sub_vec_pt);
      another_temp_vec.clear();
...
/// \short Function to get the scaling Sigma of the preconditioner
  ...
  /// \short The total number of dof types
  unsigned N_doftypes;
...
///////////////////////////////////////////////////////////////////////////////
  ...
  { // encapsulating temp vectors.
    ...
    for(unsigned mesh_i = 0; mesh_i < nmesh; mesh_i++)
    {
      ...
      for(unsigned l_i = spatial_dim; l_i < N_doftype_in_mesh[mesh_i]; l_i++)
      {
...
//  Exact_prec_pt->setup(full_jac_concat_pt,this->comm_pt());
  ...
  DenseMatrix<CRDoubleMatrix* > w_pt(1,N_lagrange_doftypes,0);
...
//    pause("got all mass matrices"); 
  ...
  if (n_mm == 0) 
  {
    ...
    unsigned long l_i_nrow_global = mm_pt[0]->nrow();
    ...
    w_pt(0,l_i) = new CRDoubleMatrix(new_distribution_pt);
...
//        mmfile.close();
      ...
      w_pt(0,l_i)->build(l_i_nrow_global,
                          w_i_diag_values,
                           w_i_column_indices,
                           w_i_row_start);
...
//      pause("dum dum "); 
    ...
    else
    {
      ...
      mm_pt[0]->multiply((*mm_pt[0]),(*w_pt(0,l_i)));
      
      // Square the other mass matrices and add the result to w_pt(0,l_i).
      for (unsigned mm_i = 1; mm_i < n_mm; mm_i++)
      {
        ...
        add_matrices(temp_mm_sqrd_pt,w_pt(0,l_i));
        delete temp_mm_sqrd_pt;
      }

      // Now multiply by the Scaling sigma:
      double* current_w_values = w_pt(0,l_i)->value();
      long unsigned current_w_nnz = w_pt(0,l_i)->nnz();
      ...
      get_diag(w_pt(0,l_i),inv_w_i_diag_values);
...
//                      f_subblock_pt(row_i,N_velocity_doftypes));
   ...
   /*
    for(unsigned v_i = 0; v_i < N_fluid_doftypes; v_i++)
    {
      ...
      {
        std::cout << "block" << v_i<<v_j
                  <<", size " << f_subblock_pt(v_i,v_j)->nrow() << ", " 
                              << f_subblock_pt(v_i,v_j)->ncol() << std::endl; 
        
//        stringstream v_block_name;
//        v_block_name << "f_1aug_" << v_i << v_j;
//        f_subblock_pt(v_i,v_j)->sparse_indexed_output(v_block_name.str());
      }
    }
    pause("done!!!!"); 
    
    // */
    //std::cout << "HELLO SUMMONER" << std::endl; 
    
    //std::cout << "f_subblock_pt 00 nlocal: " << f_subblock_pt(0,0)->nrow_local() << std::endl;
    // pause("summoners"); 
     

    CRDoubleMatrix* f_aug_pt = new CRDoubleMatrix;

    //cat(f_subblock_pt,f_aug_pt);
    // NEWRAYRAYRAY
    Vector<LinearAlgebraDistribution*> f_dist_pt(f_subblock_pt.nrow(),0);
    for(unsigned f_i = 0; f_i < f_subblock_pt.nrow(); f_i++)
    {
      f_dist_pt[f_i] = f_subblock_pt(f_i,0)->distribution_pt();
    }
    CRDoubleMatrixHelpers::concatenate_without_communication
...
//    std::cout << "f_aug_pt nlocal: " << f_aug_pt->nrow_local() << std::endl; 
//    pause("do!"); 
...
///////////////////////////////////////////////////////////////////////////////
  ...
  for(unsigned l_i = 0; l_i < N_lagrange_doftypes; l_i++)
  {
    if(Using_superlu_w_preconditioner)
    {
      ...
      W_preconditioner_pts[l_i]->setup(w_pt(0,l_i), comm_pt());
    }
    else
    {
    ...
    for (unsigned e = 0; e < n_el; e++)
     {
      ...
      GeneralisedElement* el_pt=Mesh_pt[0]->element_pt(e);
