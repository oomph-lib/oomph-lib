...
{  
 ...
 /// The setup function...
 //============================================================================
 void BlockPrecQTaylorHoodSpaceTimeElement::
			      ...
			      dof_lookup_list) const
 {
  ...
  {    
   ...
   error_message_stream << "Can only deal with Navier-Stokes elements which "
			<< "use quadratic interpolation at the moment. Using "
			<< n_node_1d << " nodes in each direction" << std::endl;
  ...
  for (unsigned j=0;j<n_node;j++)
  {
   ...
   if (!(this->node_pt(j)->is_a_copy()))
   {
    ...
    // If we're in the second time slice (already excluded the first time slice)
    else if (j<2*n_node_1d*n_node_1d)
    {
     // In the second local time slice
     i_local=1;
    }
    // If we're in the final time slice (already excluded the first/second slice)
    ...
    for (unsigned i=0;i<n_dof;i++)
    {
     ...
     if (local_eqn_number>=0)
     {
      ...
      if (i_local==0)
      {
       // Here i can range only between 0 and 2 (but it's also an unsigned so
       // we only need to check if it's strictly greater than 2)
       if (i>2)
       {
	// Create an output stream
	std::ostringstream error_message_stream;
	...
	// Create an error message
	error_message_stream << "Don't know what to do when i=" << i
	...
	throw OomphLibError(error_message_stream.str(),
       ...
       // which node we're on in the time-direction). We've already covered
       // 2 velocities and 1 pressure so the dof shift is 3.
	...
	throw OomphLibError(error_message_stream.str(),
       ...
       // The local dof shift given which local time slice we're on (i.e.
       // which node we're on in the time-direction). We've already covered
       // 4 velocities and 1 pressure so the dof shift is 5.
