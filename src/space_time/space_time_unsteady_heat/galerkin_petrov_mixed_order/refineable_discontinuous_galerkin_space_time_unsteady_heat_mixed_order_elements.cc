...
namespace oomph
{
 ...
 void RefineableSpaceTimeUnsteadyHeatMixedOrderEquations<SPATIAL_DIM>::
						...
						const unsigned& flag)
 {
  ...
  for (unsigned ipt=0;ipt<n_intpt;ipt++)
  {
   ...
   double J=this->dshape_and_dtest_eulerian_at_knot_ust_heat(ipt,psi,dpsidx,
							     test,dtestdx);
   ...
   for (unsigned l=0;l<n_node;l++)
   {
    ...
    if (is_node_hanging)
    {
     // Get the hang info pointer associated with the l-th node in the element
    ...
    for (unsigned m=0;m<n_master;m++)
    {
     ...
     if (local_eqn>=0)
     {
      ...
      residuals[local_eqn]+=((source+alpha_local*interpolated_dudt)*
	...
	residuals[local_eqn]-=(alpha_local*mesh_velocity[k]*
			       interpolated_dudx[k]*test(l)*W*hang_weight);
       }
      } // if (!ALE_is_disabled)
	...
	for (unsigned m2=0;m2<n_master2;m2++)
	{
	 ...
	 if (local_unknown>=0)
	 {
	  ...
	  for (unsigned k=0;k<SPATIAL_DIM;k++)
	  {
	   // Add the Laplacian contribution to the elemental Jacobian
	   jacobian(local_eqn,local_unknown)+=(beta_local*
					       dpsidx(l2,k)*dtestdx(l,k)*
	  ...
	  if (!(this->ALE_is_disabled))
	  {
	   ...
	   for (unsigned k=0;k<SPATIAL_DIM;k++)
	   {
	    ...
	    jacobian(local_eqn,local_unknown)-=(alpha_local*mesh_velocity[k]*
						dpsidx(l2,k)*test(l)*
						W*hang_weight*hang_weight2);
	   }
	  } // if (!(this->ALE_is_disabled))
