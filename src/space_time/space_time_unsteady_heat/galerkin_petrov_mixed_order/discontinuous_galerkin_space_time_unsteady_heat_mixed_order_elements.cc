...
namespace oomph
{
  ...
  double SpaceTimeUnsteadyHeatMixedOrderEquations<SPATIAL_DIM>::Default_alpha_parameter=1.0;

  /// Default value for Beta parameter (thermal conductivity)
  template<unsigned SPATIAL_DIM>
  double SpaceTimeUnsteadyHeatMixedOrderEquations<SPATIAL_DIM>::Default_beta_parameter=1.0;

  //======================================================================
  // Set the data for the number of variables at each node
  //======================================================================
  template<unsigned SPATIAL_DIM, unsigned NNODE_1D>
  const unsigned QUnsteadyHeatMixedOrderSpaceTimeElement<SPATIAL_DIM,NNODE_1D>::Initial_Nvalue=1;
      ...
      const unsigned& flag)
  {
    ...
    for (unsigned ipt=0; ipt<n_intpt; ipt++)
    {
      ...
      double J=dshape_and_dtest_eulerian_at_knot_ust_heat(ipt,psi,dpsidx,
               test,dtestdx);
      ...
      for (unsigned l=0; l<n_node; l++)
      {
        ...
        if (local_eqn>=0)
        {
          ...
          residuals[local_eqn]+=(source+alpha_local*interpolated_dudt)*test(l)*W;

          // Loop over the coordinate directions
          for (unsigned k=0; k<SPATIAL_DIM; k++)
          {
            // Add in the contribution from the Laplace operator
            residuals[local_eqn]+=beta_local*interpolated_dudx[k]*dtestdx(l,k)*W;
          }
          ...
          if (flag)
          {
            ...
            for (unsigned l2=0; l2<n_node; l2++)
            {
              ...
              if (local_unknown>=0)
              {
                ...
                jacobian(local_eqn,local_unknown)+=(alpha_local*test(l)*
                                                    dpsidx(l2,SPATIAL_DIM)*W);

                // Laplace operator
                for (unsigned i=0; i<SPATIAL_DIM; i++)
                {
                  // Add the test function contribution to the Jacobian
                  jacobian(local_eqn,local_unknown)+=(beta_local*dpsidx(l2,i)*
                                                      dtestdx(l,i)*W);
                }
              } // if (local_unknown>=0)
            } // for (unsigned l2=0;l2<n_node;l2++)
  ...
  //======================================================================
  template <unsigned SPATIAL_DIM>
  void SpaceTimeUnsteadyHeatMixedOrderEquations<SPATIAL_DIM>::output(std::ostream& outfile,
  ...
  //======================================================================
  template <unsigned SPATIAL_DIM>
  void SpaceTimeUnsteadyHeatMixedOrderEquations<SPATIAL_DIM>::output(FILE* file_pt,
      const unsigned& nplot)
  {
    // Vector of local coordinates
    Vector<double> s(SPATIAL_DIM+1,0.0);
  ...
  template <unsigned SPATIAL_DIM>
  void SpaceTimeUnsteadyHeatMixedOrderEquations<SPATIAL_DIM>::
  output_element_paraview(std::ofstream& file_out,const unsigned& nplot)
  {
