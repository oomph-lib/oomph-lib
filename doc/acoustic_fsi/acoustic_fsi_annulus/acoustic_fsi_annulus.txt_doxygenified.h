...
\image html coated_pipe.gif "Sketch of the model problem: Forced oscillations of a circular cylinder (black) deform an elastic coating layer (pink) which is surrounded by a compressible fluid. " 
\image latex coated_pipe.eps "Sketch of the model problem: Forced oscillations of a circular cylinder (black) deform an elastic coating layer (pink) which is surrounded by a compressible fluid. " width=0.6\textwidth
...
(2), respectively.
The displacement boundary condition (3) on the inner surface
...
Sommerfeld radiation condition (6) on the outer boundary
...
The boundary conditions (4) and 
(5) at the fluid-solid interface are 
...
(4), while the normal derivative of the
displacement potential is given in terms of the solid displacement via 
equation (5). Note that corresponding points on the
...
In the computations the integral in (8) is evaluated 
along the outer boundary of the computational domain. 


\image html trace.gif "Radiated power as function of the FSI parameter Q for an axisymmetrically oscillating coating. Markers: computed results; continuous line: analytical result. " 
\image latex trace.eps "Radiated power as function of the FSI parameter Q for an axisymmetrically oscillating coating. Markers: computed results; continuous line: analytical result. " width=0.6\textwidth
...
(4), the FSI flux boundary condition (5)
for the Helmholtz equation, and the Sommerfeld radiation condition 
(6), respectively, using helper functions discussed
below.
...
(4) on the outer boundary (boundary 2) of the solid
mesh:
...
(5) on the inner boundary (boundary 0),
and the Sommerfeld radiation condition (6) 
...
that impose the FSI traction boundary condition (4),
...
adjacent to (the Gauss points of) the \c FaceElements 
that impose the FSI flux boundary condition (5),
...
- Equation (8) for the time-averaged radiated power 
