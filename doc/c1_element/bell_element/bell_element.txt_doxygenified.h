...
After introducing the weight or test function \f$ \phi(x_i) \f$ and changing the differential equation (1) into the integral form, we have
\f[
0 = r = \int_{\Omega}{\left( \frac{\partial^4 u}{\partial x_1^4} + 2\frac{\partial^4 u}{\partial x_1^2\partial x_2^2} + \frac{\partial^4 u}{\partial x_2^4} - f(x_i)\right) \phi(x_i) d\Omega.} \ \ \ (2)
\f] 
...
conditions, \f$ \left. \phi \right|_{\partial \Omega} = 0 = \left. \frac{\partial \phi}{\partial n}\right|_{\partial \Omega}, \f$ the integrals for the last two terms of equation (3) vanish.
...
\f[
u(x_i) = \sum_{l=1}^{N} U_{l}\psi_l(x_i), \ \ \ \ (5)
\f]
and,
\f[
\phi(x_i) = \sum_{k=1}^{N} \Phi_{k}\psi_k(x_i), \ \ \ \ (6)
\f]
...
We have that (7) must hold for any value of the coefficients \f$ \Phi_k \f$, so the coefficients \f$ U_j \f$ must satisfy the equations
...
(9) vanish. This is a consequence of every
...
\f[ \psi_{1}(s_1,s_2) = s_1, \ \ \ \psi_{2}(s_1,s_2) = s_2, \ \ \ \psi_{3}(s_1,s_2) = 1-s_1-s_2. \ \ \ \ \ (10)\f]
...
\image html triangle.gif "The geometry of a triangle which has three nodes. " 
\image latex triangle.eps "The geometry of a triangle which has three nodes. " width=0.55\textwidth

The barycentric coordinates of point \f$ {\bf p} = (x,y)\f$ in the triangle
are simply defined to be ratios of triangular areas as,
\f[ \lambda_1 = \frac{A_1}{A}, \ \ \ \lambda_2 = \frac{A_2}{A}, \ \ \ \lambda_3 = \frac{A_3}{A}. \ \ \ \ (12)
\f]
...
\image html bell_element.gif "(Left)The Bell element with 18 degrees of freedom; 6 dofs at vertices. (Right) The description of all parameters utilised within the derivation of the Bell shape functions. " 
\image latex bell_element.eps "(Left)The Bell element with 18 degrees of freedom; 6 dofs at vertices. (Right) The description of all parameters utilised within the derivation of the Bell shape functions. " width=0.75\textwidth
...
\lambda_2^2\right) \f]
\f[ + b_3c_3\lambda_1^2\lambda_2\left( \lambda_2\lambda_3 - \lambda_3\lambda_1 - \lambda_1\lambda_2 - \lambda_3^2\right) - 5\lambda_1^2\lambda_2\lambda_3\left( b_2c_2l^{\prime}_2\lambda_3 + b_3c_3l_3\lambda_2\right). \ \ \ \ (13)\f] 

These are shape functions associated with the first node with its 6 degrees of freedom. The shape functions at another two nodes can be obtained by performing a cyclic permutation of the Barycentric coordinates in (13). The parameters \f$  b_i, c_i \forall i = 1,2,3, \f$ appearing in the equations can be obtained from
...
and the derivatives of area coordinates with respect to global coordinates can be obtained from considering (12) with the substitutions of \f$ b_i, c_i; i=1,2,3. \f$ They are expressed as follows:
...
\image html shapes.gif "The Bell shape functions that correspond to the 6 degrees of freedom defined previously at the first node. Here the shape functions are evaluated at x1 which show the appreciation of the crucial properties in the Bell shape functions. " 
\image latex shapes.eps "The Bell shape functions that correspond to the 6 degrees of freedom defined previously at the first node. Here the shape functions are evaluated at x1 which show the appreciation of the crucial properties in the Bell shape functions. " width=1.05\textwidth
...
The linear Lagrange polynomials defined in (10)
...
In order to approximate variables in the \c BellElement, the Bell shape functions defined in (13) will be overloaded from \c BellElementShape::Bshape(...). This function will compute the basis functions at local coordinate \f$ s \f$ and it provides \f$ C^1 \f$-continuity.
...
element can be worked out from (14) and are expressed as
...
\image html Biharmonic_Bell_Soln.gif "The finite element solution of the Biharmonic equation obtained from the BellElement. " 
\image latex Biharmonic_Bell_Soln.eps "The finite element solution of the Biharmonic equation obtained from the BellElement. " width=0.55\textwidth

\image html Biharmonic_Bell_Err.gif "Absolute error between the exact and the finite element solutions of the Biharmonic equation obtained from the BellElement. " 
\image latex Biharmonic_Bell_Err.eps "Absolute error between the exact and the finite element solutions of the Biharmonic equation obtained from the BellElement. " width=0.55\textwidth
...
\image html Bell_Err_log.gif "A log-plot between error and the element size from solving the Biharmonic equation with the BellElement in a rectangular domain. " 
\image latex Bell_Err_log.eps "A log-plot between error and the element size from solving the Biharmonic equation with the BellElement in a rectangular domain. " width=0.55\textwidth
