...
- \ref two_point_zero_to_three_point_zero
  - \ref new_functionality_three_point_zero
...
\subsection two_point_zero_to_three_point_zero Changes between version 2.0 and 3.0.*

\subsubsection new_functionality_three_point_zero Major new functionality/changes

- Moved to C++17 standard
  \n\n
- Completely re-implemented the build machinery which is now based on CMake.
  - the equivalent of the old <code>autogen.sh</code> script
    is <code>oomph_build.py</code>
  - renamed the <code>bin</code> directory <code>scripts</code>
  - moved the <code>self_test</code> directory into <code>demo_drivers</code>
  - changed the way the template instantiation in the
    <code>meshes</code> directory is done: <code>*.template.h</code>
    becomes <code>*.h</code>.
  - third-party libraries are now pulled in from their GitHub
    repositories.
  .
.
\n\n
...
- Many new detailed tutorials explaining:\n\n
  - How to use \c oomph-lib in parallel and how to (optionally) distribute 
    ...
    \n\n
  - How to use \c oomph-lib's block preconditioning framework. 
    \n\n
...
- Provided a new mechanism that allows multiple \c FaceElements to be
  ...
  separate tutorial.</a>
  \n\n
- Significantly extended \c oomph-lib's bifurcation tracking routines
