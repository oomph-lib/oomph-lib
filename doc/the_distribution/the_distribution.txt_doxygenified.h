...
See the <a href="../../change_log/html/index.html">change log page</a> for an overview of the changes made since the previous release.</p>
...
\subsection tar_file Option 1: Get a tar file


You can download a tar file of the latest distribution
...
<code>oomph-lib-*</code> where the "*" indicates the version/revision 
number (the same as in the tar file).

\subsection subversion Option 2: Get the distribution from the subversion repository

You can also get hold of the library from our 
...
tutorial.</a> When checking out from the repository, you can specify
where the distribution is placed. By default subversion will place it
into a directory called <code>trunk</code>.
...
If you're an experienced linux/unix user, you may prefer to control the
build/install process manually, using the standard \c configure; \c make;
\c make \c install; \c make \c check; sequence. Note that this will *only*
work when building the tar file version (i.e. not the version from svn
which still requires the configure script to be generated). In this case,
the library will be compiled with GNU's default compiler options ("-O3 -g")
and the libraries and include files will be installed in \c /usr/local/
unless you specify a different directory with the \c --prefix argument to
the \c configure command. Here's an example of a manual installation
 
- Unpack the distribution (you may have to replace the "*" with the 
  actual revision number)
  \code
  tar xvfz oomph-lib-*.tar.gz
  \endcode 
- Change into the oomph-lib home directory (again, you may have to
  replace the "*" with the actual revision information)
  \code 
  cd oomph-lib-*
  \endcode
...
\subsubsection no_doc Suppressing the build of the pdf version of the documentation:
...
\subsection dont_have_auto_tools Writing/linking user code without autotools
