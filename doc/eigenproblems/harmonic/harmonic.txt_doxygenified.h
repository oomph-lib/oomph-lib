...
the library. An interface is also provided to the serial version of 
ARPACK, an iterative solver, but we are not allowed to 
redistribute the source code, so it
must be downloaded separately. That said, much of 
the functionality of ARPACK has
been superseded by the ANASAZI solver that is part of The Trilinos
Project. Thus, if you want to solve small problems, use the default QZ
...
\f[ J_{ij} V_{j} = \lambda M_{ij} V_{j}, \ \ \ \ \ \ \ \ (3) \f]
 ...
 above all solve systems of the form (3), and within \c
...
The general solution of equation (5) takes the form \f$
...
functional form of the solution means that equation (5)
becomes
...
in equation (3). The solution is said to be linearly stable if the real
...
\subsection weak Weak formulation of the problem
 ...
 If we wish to solve the problem (1) using finite elements,
...
\endcode
 ...
 \f$M_{ij}\f$ in equation (3). The residuals need not be
...
Jacobian and mass matrices defined in equations (6) in the
...
fill_in_contribution_to_jacobian_and_mass_matrix 
 ...
 (6). The residuals vector is not filled in and does not
...
the three approaches. Note that if you do not have Trilinos or ARPACK
installed then only the LAPACK_QZ solver will do anything. 
