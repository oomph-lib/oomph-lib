\chapter{The spatially-\/adaptive solution of the azimuthally Fourier-\/decomposed equations of 3D time-\/harmonic linear elasticity on unstructured meshes}
\hypertarget{index}{}\label{index}\index{The spatially-\/adaptive solution of the azimuthally Fourier-\/decomposed equations of 3D time-\/harmonic linear elasticity on unstructured meshes@{The spatially-\/adaptive solution of the azimuthally Fourier-\/decomposed equations of 3D time-\/harmonic linear elasticity on unstructured meshes}}
...
The figure below shows the problem considered in this tutorial\+: an annular elastic body that occupies the region $ r_{\rm min}\leq
r\leq r_{\rm max}, z_{\rm min}\leq
z\leq z_{\rm max} $ is loaded by a time-\/harmonic pressure load acting on its inner surface (at $ r = r_{\rm min} $). The upper and lower ends of the hollow cylinder (at $ z = z_{\rm min} $ and $ z = z_{\rm min} $) are held at a fixed position.
...
\DoxyCodeLine{\ snprintf(filename,\ \textcolor{keyword}{sizeof}(filename),\ \textcolor{stringliteral}{"{}\%s/norm.dat"{}},doc\_info.directory().c\_str());\ \ \ }
\DoxyCodeLine{\ some\_file.open(filename);\ \ \ }
