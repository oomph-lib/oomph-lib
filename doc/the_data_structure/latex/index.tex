...
The {\ttfamily Node} is now fully functional and (by default) all values that are stored with it are free (i.\+e. not pinned).
\begin{DoxyEnumerate}
...
Once this has been done (for all {\ttfamily Meshes}, if there are multiple ones), the function 
\begin{DoxyCode}{0}
...
The {\ttfamily Problem} class also has member functions which assemble the global residual vector and the global Jacobian matrix.
...
For multiply hanging nodes ({\ttfamily Nodes} 17 and 18 in the above sketch), the weights of the ultimate master nodes are determined recursively, e.\+g. $ \omega_{17\ 4} = \omega_{17\ 9} \times \omega_{9 \ 4}. $~\newline
