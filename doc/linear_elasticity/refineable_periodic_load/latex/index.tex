\chapter{Example problem\+: The deformation of an elastic strip by a periodic traction -- this time with spatial adaptivity}
\hypertarget{index}{}\label{index}\index{Example problem: The deformation of an elastic strip by a periodic traction -- this time with spatial adaptivity@{Example problem: The deformation of an elastic strip by a periodic traction -- this time with spatial adaptivity}}
...
\DoxyCodeLine{\textcolor{comment}{}\ \mbox{\hyperlink{classRefineablePeriodicLoadProblem}{RefineablePeriodicLoadProblem}}(\textcolor{keyword}{const}\ \textcolor{keywordtype}{unsigned}\ \&nx,\ \textcolor{keyword}{const}\ \textcolor{keywordtype}{unsigned}\ \&ny,\ }
...
\DoxyHorRuler{0}
 \DoxyHorRuler{0}
\hypertarget{index_constructor}{}\doxysection{\texorpdfstring{The problem constructor – applying periodic boundary condition in spatially adaptive computations.}{The problem constructor – applying periodic boundary condition in spatially adaptive computations.}}\label{index_constructor}
...
\end{DoxyCode}
 Once this function is called for a {\ttfamily Boundary\+Node}, the {\ttfamily Boundary\+Node} shares its {\ttfamily Data} with the {\ttfamily Node} specified as the argument to that function. This "{}wraps the solution around the domain"{}.
...
\DoxyCodeLine{\textcolor{comment}{///\ Problem\ constructor:\ Pass\ number\ of\ elements\ in\ the\ coordinate}\textcolor{comment}{}}
\DoxyCodeLine{\textcolor{comment}{///\ directions\ and\ the\ domain\ sizes.}}
...
\DoxyCodeLine{\ \ \ \ right\_root\_pt[i]-\/>neighbour\_pt(E)\ =\ left\_root\_pt[i];}
\DoxyCodeLine{\ \ \ \ right\_root\_pt[i]-\/>set\_neighbour\_periodic(E);\ \ \ \ \ }
...
\end{DoxyCode}
 to obtain a pointer to the {\ttfamily j} -\/th element on the {\ttfamily Mesh\textquotesingle{}s} {\ttfamily b} -\/th boundary.\hypertarget{index_exercises}{}\doxysubsection{\texorpdfstring{Exercises}{Exercises}}\label{index_exercises}
