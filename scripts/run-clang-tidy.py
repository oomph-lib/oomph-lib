...
# PM: Need to fully understand -header-filter and -source-filter but the following
# does do something...
# python3 scripts/run-clang-tidy.py -config-file=.clang-tidy -p build -clang-tidy-binary=$(which clang-tidy) -clang-apply-replacements-binary=$(which clang-apply-replacements) -header-filter='^(?!src/generic/(?:cfortran\.h|lapack_qz\.h)$)src/.*\.h$' -source-filter='.*src/.*\.(cc|tpp)$' -j 1
