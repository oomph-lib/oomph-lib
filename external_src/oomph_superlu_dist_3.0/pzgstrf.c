 ...
 *
 * <pre>
 * -- Distributed SuperLU routine (version 3.0) --
 ...
 )
{
    ...
    int_t *iperm_u;
#endif
    int *msgcnt, **msgcnts, **msgcntsU, k0, kk0, kk1, kk2, jj0, iukp0, rukp0, flag0, flag1,
        *factored, *factoredU, nnodes, *sendcnts, *sdispls, *recvcnts, *rdispls, *srows, *rrows;
    ...
    *info = 0;
	...
	ABORT("Malloc fails for send_reqs[].");
    ...
    for( i=0; i<=num_look_aheads; i++ ) {
      if ( !(recv_reqs_u[i] =
        ...
        ABORT("Malloc fails for recv_req_u[].");
      if ( !(send_reqs_u[i] =
         (MPI_Request *) SUPERLU_MALLOC(2*Pr*sizeof(MPI_Request))))
        ABORT("Malloc fails for send_req_u[].");
      if ( !(send_reqs[i] =
      ...
      send_reqs[i][0] = send_reqs[i][1] = MPI_COMM_NULL;
      recv_reqs[i][0] = recv_reqs[i][1] = MPI_COMM_NULL;
    }
    ...
    * ======================== */
    ...
    look_ahead_l = SUPERLU_MALLOC( nsupers * sizeof(int_t) );
    look_ahead   = SUPERLU_MALLOC( nsupers * sizeof(int_t) );
    ...
    MPI_Allreduce( look_ahead_l , look_ahead , nsupers, mpi_int_t, MPI_MAX, grid->comm );
...
#define IRECV_U
	...
	if ( ToRecv[k] == 2 ) { /* Recv block row U(k,:). */
	    ...
	    MPI_Irecv( Usub_buf, Llu->bufmax[2], mpi_int_t, krow,
	              2%NTAGS, scp->comm, &recv_reqs_u[0][0] );
	    MPI_Irecv( Uval_buf, Llu->bufmax[3], SuperLU_MPI_DOUBLE_COMPLEX, krow,
	              3%NTAGS, scp->comm, &recv_reqs_u[0][1] ); 
	}
    } 
#endif
	 ...
	 * ======== look-ahead the new columns ======== *
         * ============================================ */
	/* tt1 = SuperLU_timer_(); */
        if( k0 == 0 ) { /* look-ahead all the column in the window */
          kk1 = k0+1;
          kk2 = SUPERLU_MIN( k0+num_look_aheads, nsupers-1 );
        } else { /* look-ahead the new column */
	...
	for( kk0 = kk1; kk0<=kk2; kk0++ ) {
		...
		/* ttt1 = SuperLU_timer_(); */
		  ...
		  if ( ToSendR[lk][pj] != EMPTY ) {
		    ...
		    MPI_Isend( lsub1, msgcnt[0], mpi_int_t, pj,
			      (4*kk0)%NTAGS, scp->comm, &send_req[pj] );
		    MPI_Isend( lusup1, msgcnt[1], SuperLU_MPI_DOUBLE_COMPLEX, pj,
			      (4*kk0+1)%NTAGS, scp->comm, &send_req[pj+Pc] );
		  } 
	        } 
	    ...
	    if ( myrow != krow ) {
	      if ( ToRecv[kk] == 2 ) { /* post iRecv block row U(k,:). */
	        ...
	        MPI_Irecv( Usub_buf, Llu->bufmax[2], mpi_int_t, krow,
	                  (4*kk0+2)%NTAGS, scp->comm, &recv_reqs_u[look_id][0] );
	        MPI_Irecv( Uval_buf, Llu->bufmax[3], SuperLU_MPI_DOUBLE_COMPLEX, krow,
	                  (4*kk0+3)%NTAGS, scp->comm, &recv_reqs_u[look_id][1] ); 
  	      }
	    }
#endif
		...
		if ( myrow == krow ) {
			...
			for (pi = 0; pi < Pr; ++pi) {
				...
				MPI_Isend( usub, msgcnt[2], mpi_int_t, pi,
					 (4*kk0+2)%NTAGS, scp->comm, &send_reqs_u[look_id][pi]);
				MPI_Isend( uval, msgcnt[3], SuperLU_MPI_DOUBLE_COMPLEX, pi,
					 (4*kk0+3)%NTAGS, scp->comm, &send_reqs_u[look_id][pi+Pr]);
#if ( VAMPIR>=1 )
				VT_end(3);
#endif
#if ( PROFlevel>=1 )
 ...
 )
{
    ...
    *info = 0;
	...
	tag = ((k0<<2)+2)%NTAGS;
	tag = (4*(nsupers+k0)+2)%NTAGS;
        MPI_Recv(ublk_ptr, (nsupc*(nsupc+1))>>1, SuperLU_MPI_DOUBLE_COMPLEX,
		 krow, tag, comm, &status);
