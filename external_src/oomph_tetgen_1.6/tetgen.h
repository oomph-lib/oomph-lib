...
public:
  ...
  // // face of T. TetGen attaches tetrahedra (one-to-one) to such faces. All
  // such // tetrahedra contain an "infinite vertex" (which has no geometric
  // coordinates//
  ...
  // // that has no boundary in 3d. It has the nice property that every
  // triangular // face is shared by exactly two tetrahedra. //
  //                                                                            //
  ...
  //    (1/3)    9 (1/0)                                // face 2 |   2 (2/3) 6
  //    (2/1)   10 (2/0)                                // face 3 |   3 (3/0) 7
  //    (3/1)   11 (3/2)                                //
  //                                                                            //
...
#define fastlookup(pool, index) \
  ...
  // reported. This test is a combination of n 3D orientation tests (3 < n <
  // 9).// It uses the robust orient3d() test to make the branch decisions. //
  //                                                                            //
  ...
  int tri_edge_tail(
    point, point, point, point, point, point, REAL, REAL, int, int*, int*);
  ...
  // // ary flips. For example, a 4-to-4 flip, which replaces two coplanar
  // edges,  // combines a 2-to-3 flip and a 3-to-2 flip. Note that the first
  // 2-to-3 flip  // will temporarily create a degenerate tetrahedron removed
  // immediately by    // the followed 3-to-2 flip.  More generally, an n-to-m
  // flip, where n > 3,    // m = (n - 2) * 2, which removes an edge, can be
  // done by first performing a  // sequence of (n - 3) 2-to-3 flips followed by
  // a 3-to-2 flip.                //
  //                                                                            //
  ...
  // // arbitrary order).  The success is guaranteed when the Delaunay
  // tetrahedra- // lization is constructed incrementally by adding one vertex
  // at a time.      //
  //                                                                            //
  ...
  // // tetrahedron in DT, it finds the destination by visit one tetrahedron at
  // a  // time, randomly chooses a tetrahedron if there are more than one
  ...
  // // tional Geometry, 211-219, 2003. On the other hand, Liu and Snoeyink
  // showed // that the point location could be made in constant time if the
  // points are   // pre-sorted so that the nearby points in space have nearby
  ...
  // // Triangulation and the Delaunay Graph in Medium Dimension," In
  // Proceedings  // of the 25th ACM Symposium on Computational Geometry, 2009.
  // It first       // randomly sorts the points into subgroups using the Biased
  // Randomized       // Insertion Ordering (BRIO) of Amenta et al 2003, then
  // sorts the points in   // each subgroup along the 3D Hilbert curve.
  // Inserting points in this order  // ensure a randomized "sprinkling" of the
  // points over the domain, while      // sorting of each subset provides
  // locality.                                  //
  //                                                                            //
  //============================================================================//
  ...
  int hilbert_split(point* vertexarray,
                    int arraysize,
                    int gc0,
                    int gc1,
                    REAL,
                    ...
                    REAL,
                    REAL);
  void hilbert_sort3(point* vertexarray,
                     int arraysize,
                     int e,
                     int d,
                     REAL,
  ...
  enum locateresult locate(point searchpt,
                           triface* searchtet,
                           int chkencflag = 0);
  ...
  // // domain).  A crucial difference between a CDT and a DT is that triangles
  // in // the PLC's polygons are not required to be locally Delaunay, which
  // frees    // the CDT to respect the PLC's polygons better. CDTs have optimal
  // properties // similar to those of DTs. //
  //                                                                            //
  ...
  // // Some extra points, so-called "Steiner points" are needed to form a
  // tetrah- // edralization of such polyhedron.  A Steiner CDT of a 3D PLC is a
  // CDT       // containing Steiner points. TetGen generates Steiner CDTs. //
  //                                                                            //
  ...
  // DT.  // The running time of this algorithm is proportional to the number of
  // // Steiner points. //
  //                                                                            //
  ...
  // // vertices might need Steiner points. It is discussed in the paper of Si,
  // H.,// and  Shewchuk, J., "Incrementally Constructing and Updating
  ...
  enum interresult scoutsegment(
    point, point, face*, triface*, point*, arraypool*);
  ...
  int checkflipeligibility(int fliptype,
                           point,
                           point,
                           point,
                           point,
  ...
  int recoveredgebyflips(
    point, point, face*, triface*, int fullsearch, int& idir);
  ...
  int recoverfacebyflips(
    point, point, point, face*, triface*, int&, point*, point*);
  ...
  // // new Steiner points to achieve this property. The questions are (1) how
  // to  // choose the Steiner points? and (2) how to insert them? //
  //                                                                            //
  ...
  // // Symposium on Computational Geometry, 86-95, 1998.  It guarantees that
  // all  // tetrahedra of the output mesh have a "radius-edge ratio"
  ...
  // // The mesh has good quality (same as Shewchuk's Delaunay refinement
  // algori-  // thm) in the bulk of the mesh domain. Moreover, it supports the
  // generation  // of adaptive mesh according to a (isotropic) mesh sizing
  // function.          //
  //                                                                            //
  //============================================================================//
  ...
  bool split_subface(
    face* splitfac, point encpt, REAL* ccent, REAL*, int, int, int*);
...
inline void terminatetetgen(tetgenmesh* m, int x)
{
  ...
  switch (x)
  {
    ...
    case 2: // Encounter an internal error.
      ...
      break;
    case 3:
      printf("The input surface mesh contain self-intersections. Program "
             "stopped.\n");
    ...
    case 200:
      printf(
        "Boundary contains Steiner points (-YY option). Program stopped.\n");
      break;
  } // switch (x)
  exit(x);
#endif // #ifdef TETLIBRARY
