...
oomph-lib's extensive documentation is generated by doxygen and the full set of webpages (containing many detailed tutorials) is hosted on Github. The webpages are re-generated automatically whenever the main branch of the official oomph-lib repository is updated. It is therefore important to be able to test that any changes to the documentation are rendered correctly before commiting to main.

For this purpose, the relevant workflow can be triggered manually from any cloned oomph-lib repository where it overwrites a local version of the webpages (rather than the official ones). This requires the following steps:
